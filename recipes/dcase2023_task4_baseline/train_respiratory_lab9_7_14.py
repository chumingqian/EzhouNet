...
from desed_task.nnet.EzhouNet_v9_7_9 import GraphRespiratory
from local.respira_trainer_v9_7_10 import RespiraSED_lab
...
# v9-7-4;  修改 区间的 初始学习率为 2e-3;
# 并且修改   Node level loss  的权重为0.2；
# 修改 num triplet = 40; 预热 50 epoch;


# v9-7-6; 使用双优化器， 切换为手动优化，
# 节点和 区间的学习率的衰减速度不同， 并且最低值也不同；



# v9-7-8, 去除 节点和区间中的 位置信息；
# 将区间的定位损失 替换为 smoth l1 loss


# v9-7-10,
#  修改区间GT的 置信度从 0.2 -->0.5;  这导致了节点分类的性能下降
#  修改interval学习率为  1e-4;


# v9-7-12,
#  修改区间GT的 置信度保留为 0.2 ；interval学习率为  1e-4;
#  修改的边的属性特征为 压缩方式；  后续 通过调整 interval  param= [-20, 20]  来进行研究调整；


# v9-7-14;
#  修改区间GT的 置信度保留为 0.2 ；   interval学习率为  1e-4;
#  保留边的属性特征为顺序方式；  后续通过调整 interval  param= [-1.5, 1.5]；
