...
class  DynamicFeatureExtractor(nn.Module):
        def __init__(self,
                     ...
                     temperature=31,
                     pool_dim='freq',
                     node_fea_dim = 512):  # node dim depends  on the frames
            ...
            self.spec2node = nn.Sequential(
                ...
                #
                nn.Linear(1* 256 * 1 * 2, node_fea_dim),
        ...
        def forward(self, x):  # x size : [bs=1, chan=3, frames=5, freqs=84]
            x = self.dynamic_cnn(x)  #  out: (1, 256, 1,2)

            flatten5frames = torch.flatten(x, start_dim= 1)

            gen_signle_node = self.spec2node(flatten5frames)

            signle_node_fea = gen_signle_node.squeeze(0)

            return  signle_node_fea
