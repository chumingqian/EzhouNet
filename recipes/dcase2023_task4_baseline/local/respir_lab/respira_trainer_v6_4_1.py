...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        if len(actual_abnormal_indices) > 0:
            ...
            num_normal_nodes_to_sample = max(1, num_abnormal_nodes // 3)  # Adjust denominator based on desired ratio
            ...
            if len(predicted_abnormal_indices) == 0:
                ...
                num_nodes_to_sample = min(32, total_nodes)  # Adjust the number as needed
        ...
        self.training_step_outputs['record_binary_loss_ce'].append(record_binary_loss_ce)
        self.training_step_outputs['record_binary_loss_fbeta'].append(record_binary_loss_fbeta)
        self.training_step_outputs['classification_loss'].append(classification_loss)
        self.training_step_outputs['node_loss'].append(node_loss)
        self.training_step_outputs['detection_loss'].append(detection_loss)
    ...
    def training_epoch_end(self, outputs):
        ...
        # Process node-level predictions and labels
        if len(self.training_node_preds) > 0:
            ...
            # print(f"\n Node-level Training Confusion Matrix:\n{node_cm}")
            # Compute node-level confusion matrix
              # Extract label indices from the OrderedDict
            ...
            # print_matrix_with_alignment(node_cm_ratio, "Node-level Training Confusion Matrix (Ratio Format)")
            print_matrix_with_labels(node_cm_ratio, classes_labels, "Node-level Training  Confusion Matrix (Ratio Format)")
        ...
        self.log("train/record_binary_class_loss/epoch", epoch_metrics['record_binary_loss_ce'],
                 on_epoch=True)
        self.log("train/record_binary_fp_fn_loss/epoch", epoch_metrics['record_binary_loss_fbeta'],
                 on_epoch=True)
        self.log("train/weight_classification_loss/epoch", epoch_metrics['classification_loss'],
                 on_epoch=True)
        self.log("train/node_loss/epoch", epoch_metrics['node_loss'],
                 on_epoch=True)
        self.log("train/weight_detection_loss/epoch", epoch_metrics['detection_loss'],
                 on_epoch=True)
        self.log("train/total_loss/epoch", epoch_metrics['total_loss'],
                 on_epoch=True)
        ...
        # Update best_training_f1
        if training_f1_score > self.best_training_f1:
            self.best_training_f1 = training_f1_score

        # Start adjusting if training F1-score reaches 0.98
        if training_f1_score >= 0.98 and not self.adjustment_started:
    ...
    def validation_step(self, batch_data, batch_indx):
        ...
        if validation_method == "method1":  # note, 输入到检测网络中的样本是原始批次中所有的样本，
            ...
            num_normal_nodes_to_sample = min(32, len(normal_node_indices))  # Adjust as needed
    ...
    def validation_epoch_end(self, outputs):
        ...
        avg_val_accuracy = torch.stack(
            [x['val_acc_binary_cls'].clone().detach() if isinstance(x['val_acc_binary_cls'], torch.Tensor)
             else torch.tensor(x['val_acc_binary_cls'])
             for x in self.binary_val_acc]).mean().item()
        ...
        if len(self.validation_node_preds) > 0:
            ...
            # Compute node-level confusion matrix
            labels = list(classes_labels.values())  # Extract label indices from the OrderedDict
            node_cm = confusion_matrix(all_node_targets.numpy(), all_node_preds.numpy(), labels=labels)
            print_matrix_with_labels(node_cm, classes_labels, "Node-level Validation Confusion Matrix")

            # Compute ratio format of the confusion matrix
            with np.errstate(divide='ignore', invalid='ignore'):
                node_cm_ratio = node_cm.astype('float') / node_cm.sum(axis=1, keepdims=True)
                node_cm_ratio = np.nan_to_num(node_cm_ratio)  # Replace NaN and Inf with zero
        ...
        #
        #     # Specify all possible labels (classes) to include in the confusion matrix
        #     labels = [0, 1, 2, 3, 4, 5, 6]  # Adjust based on your actual classes
        #
        #     # Compute node-level confusion matrix
        #     node_cm = confusion_matrix(all_node_targets.numpy(), all_node_preds.numpy(), labels=labels)
        #     print(f"\n Node-level Validation Confusion Matrix:\n{node_cm}")
        #
        #     # Compute ratio format of the confusion matrix
        #     with np.errstate(divide='ignore', invalid='ignore'):
        #         node_cm_ratio = node_cm.astype('float') / node_cm.sum(axis=1, keepdims=True)
        ...
        #
        #     #print(f"\n Node-level Validation Confusion Matrix (Ratio Format):\n{node_cm_ratio}")
        #     print_matrix_with_alignment(node_cm_ratio, "Node-level Validation Confusion Matrix (Ratio Format)")
        #
        # else:
        #     print("\n No node-level predictions to compute confusion matrix.")
        ...
        # validation_method = self.hparams.get("validation_method", "method1")  # Can be 'method1' or 'method2'
        # print(f"\n Current using validation method {validation_method}")
        ...
        try:
            ...
            event_segment_scored = DEFAULT_EVENT_SEGMENT_SCORED
            # Handle the error
