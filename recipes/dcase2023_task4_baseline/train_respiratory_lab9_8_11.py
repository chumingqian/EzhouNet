...
from desed_task.dataio.datasets_resp_v9_8_2 import RespiraGnnSet
from desed_task.nnet.EzhouNet_v9_8_11 import GraphRespiratory
from local.respira_trainer_v9_8_11 import RespiraSED_lab
...
# v9-8-3, 将不同的IOU阈值，Stridor 与 Rhonchi 使用相同的阈值， 与稀有类别的数据增强一起使用；
# 降低 推理阶段 NMS_IOU, 只要满足0.5 or 0.4 就去除；


# v9-8-4,  定制化IOU， 并且同时对所有的异常类进行数据增强;
#  但是使用概率不同， {"Stridor", "Rhonchi"} 为0.5， 其余两类异常为0.2;

# v9-8-8,  weight param 的设置为固定的参数， require 梯度设置为0；

# v9-8-11,  weight param 的设置为固定的参数， require 梯度设置为0；
# 区间 [-0.5, 0.5],   并且区间的最低学习率 5e-4
