...
class RespiraSED_lab(pl.LightningModule):
    ...
    def compute_consistency_loss(self, node_logits_refined, edge_logits_refined, batch_graph,outputs_detection, consistency_loss_weight=1.0):
        ...
        # 节点与边之间的一致性损失；
        # If your pipeline is fully argmax-based or you do a two‐stage process
        # (i.e., first finalize node predictions, then decide edge predictions purely in a discrete sense),
        # the hard approach (Method 1) might suffice.
        # Convert refined logits to predicted classes
        node_pred_classes_refined = torch.argmax(node_logits_refined, dim=-1)  # (N,)
        edge_pred_classes_refined = torch.argmax(edge_logits_refined, dim=-1)  # (E,)If your pipeline is fully argmax-based or you do a two‐stage process (i.e., first finalize node predictions, then decide edge predictions purely in a discrete sense), the hard approach (Method 1) might suffice.
        ...
        node_edge_consistency_loss = inconsistent_edges.float().mean() * consistency_loss_weight


        # 子图与节点的一致性损失；
        sub_graph_node_consistency_loss = 0.0
        for i in range(len(outputs_detection['subgraph_predictions'])):
            subgraph_pred = outputs_detection['subgraph_predictions'][i]  # Shape: (1, num_classes)
            node_preds = outputs_detection['subgraph_node_predictions'][i]  # Shape: (num_nodes_in_subgraph, num_classes)

            # Expand subgraph prediction to match node predictions
            subgraph_pred_expanded = subgraph_pred.expand_as(node_preds)

            # Consistency loss (e.g., CrossEntropy between node_preds and subgraph_pred)
            # Since subgraph_pred is same for all nodes in subgraph, we can define this loss in various ways.

            # Option 1: KL Divergence
            subgraph_pred_prob = F.log_softmax(subgraph_pred_expanded, dim=1)
            node_pred_prob = F.softmax(node_preds, dim=1)
            kl_loss = F.kl_div(subgraph_pred_prob, node_pred_prob, reduction='batchmean')
