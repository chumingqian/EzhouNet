...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.node_conf_cls_head_params = (  list(self.sed_net.node_interval_pred.node_class_heads.parameters() )  )

        self.interval_unify_head_params = list(self.sed_net.node_interval_pred.interval_refine.interval_unify_head.parameters())
        ...
        node_head_set = set(self.node_conf_cls_head_params)
        interval_unify_head_set = set(self.interval_unify_head_params)
        self.shared_params = []
        for i, p in enumerate(self.sed_net.parameters()):
            # print(f"Param {i}: shape={p.shape}, device={p.device}, requires_grad={p.requires_grad}")
            if p not in node_head_set and p not in interval_unify_head_set:
                self.shared_params.append(p)
            # else:
            #     print(f"Skipping param: {p.shape}")
        print("Shared params assigned")
        ...
        print(f"Confidence params count: {len(self.node_conf_cls_head_params)}")
        print(f"Classification params count: {len(self.interval_unify_head_params)}")
        ...
        self.node_conf_loss_weight = 20.0
        self.node_loss_weight = 30.0
        ...
        self.interval_conf_loss_weight =  8.0
        self.interval_cls_loss_weight =   5.0
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        if self.current_epoch < 60:
            ...
            self.node_conf_loss_weight = 30.0
            self.node_loss_weight = 20.0

            self.interval_conf_loss_weight = 8.0
            self.interval_cls_loss_weight = 5.0
            self.interval_location_loss_weight = 0.02

        elif self.current_epoch < 100:
            transition_factor = (self.current_epoch - 60) / 40.0
            self.node_conf_loss_weight = 30.0 - transition_factor * 15.0
            self.node_loss_weight = 20.0 - transition_factor * 10.0

            self.interval_conf_loss_weight = 8.0  + transition_factor * 32.0
            self.interval_cls_loss_weight  = 5.0   + transition_factor * 45.0
            self.interval_location_loss_weight = 0.02 + transition_factor * 0.98

        else:
            self.node_conf_loss_weight = 15.0
    ...
    def configure_optimizers(self):
        ...
        lr_interval_initial = 1e-5
        lr_shared_initial = 1e-3

        optimizer = torch.optim.Adam([
            {'params': self.node_conf_cls_head_params,  'lr': lr_node_initial,     'name': 'node_conf_cls'},
            {'params': self.interval_unify_head_params, 'lr': lr_interval_initial, 'name': 'interval_unify'},
            {'params': self.shared_params,              'lr': lr_shared_initial,   'name': 'shared'}
        ])

        # Learning rate schedule function
        def lr_lambda(epoch, group_name):
            if epoch < 60:  # Phase 1: Confidence focus
                return {
                    'node_conf_cls': 1.0,  # 1e-3
                    'interval_unify': 1.0,  # 1e-5
                    'shared': 1.0  # 1e-3
                }[group_name]
            elif epoch < 100:  # Phase 2: Transition
                transition_factor = (epoch - 60) / 40.0
                return {
                    'node_conf_cls': 1.0 - transition_factor * 0.9,  # From 1e-3 to 1e-4
                    'interval_unify': 1.0 + transition_factor * 99.0,  # From 1e-5 to 1e-3
                    'shared': 1.0  # Constant 1e-3
                }[group_name]
            else:  # Phase 3: Classification focus
                return {
                    'node_conf_cls': 0.1,  # 1e-4
                    'interval_unify': 100.0,  # 1e-3
                    'shared': 1.0  # 1e-3
                }[group_name]
    ...
    #     lr_node_conf_cls_initial = 1e-3
    #     lr_interval_unify_initial = 1e-5  # 1e-5
    #     lr_shared_initial = 1e-3
    #
    #     optimizer = torch.optim.Adam([
    #         {'params': self.confidence_params, 'lr': lr_node_conf_cls_initial, 'name': 'confidence'},
    #         {'params': self.classification_params, 'lr': lr_interval_unify_initial, 'name': 'classification'},
    #         {'params': self.shared_params, 'lr': lr_shared_initial, 'name': 'shared'}
    #     ])
    #
    ...
    def on_train_epoch_start(self):
        ...
        lr_node_conf_cls = optimizer.param_groups[0]['lr']  # confidence
        lr_interval_unify = optimizer.param_groups[1]['lr']  # classification
        lr_shared = optimizer.param_groups[2]['lr']  # shared

        print(f"Epoch {self.current_epoch}: lr_node_conf_cls={lr_node_conf_cls}, "
              f"lr_interval_unify={lr_interval_unify}, lr_shared={lr_shared}")

        self.log('lr_node_conf_cls', lr_node_conf_cls, on_epoch=True)
        self.log('lr_interval_unify', lr_interval_unify, on_epoch=True)
