...
# # v8-13_6,   基于8-13-2， 纠正 alpha 权重比率 ，
# note,  开始引入 time shift  数据增强， 使用零填充对齐；
# time shift  应该对异常事件的检测， 起到很好帮助作用；

# v8-13_7, 使用 time shift, 但是使用的是音频自身的数据进行对齐；
#  并且考虑 timestamp warping 的情况， which event span the begaining and the end of the  audio
...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='reorganize audio info,' )):
            ...
            cur_dict["assignments"] = assignments  # Cache the original assignments
    ...
    def __getitem__(self, idx):
        ...
        # Check if augmentation should be considered: only for training and abnormal samples
        is_time_shifted = False

        if self.train_flag and binary_label == "Abnormal":
            # Decide on time shift augmentation first (50% probability)

            if self.audio_aug and random.random() > 0.5:
                # Calculate maximum allowable shifts to prevent event splitting
                if vad_timestamps:
                    onsets = [vad["start"] for vad in vad_timestamps]
                    offsets = [vad["end"] for vad in vad_timestamps]
                    min_onset = min(onsets)  # Earliest event start time
                    max_offset = max(offsets)  # Latest event end time

                    max_forward_shift = min_onset  # Max shift left (events move earlier)
                    max_backward_shift = audio_dur - max_offset  # Max shift right (events move later)

                    # if  use the following ,will avoid the timestamp wrapping,
                    # this will prevent the  event  span the audio at the  begian and end of the audio
                    """
                    max_forward_shift = minimum of all events’ event["offset"].
                    max_backward_shift = minimum of all events’ audio_dur - event["onset"].
                    
                    """

                else:
                    # If no events, limit shift to half the duration in either direction
                    max_forward_shift = audio_dur / 2
                    max_backward_shift = audio_dur / 2

                # Sample a random shift within the allowable range (in seconds)
                shift_seconds = random.uniform(-max_forward_shift, max_backward_shift)
                shift_samples = int(shift_seconds * self.fs)  # Convert to samples using sampling frequency

                # Shift the audio waveform with zero padding
                audio_len = audio.size(0)
                if shift_samples > 0:  # Right shift (events move later)
                    if shift_samples >= audio_len:
                        # If shift is larger than audio length, keep the audio unchanged (full loop)
                        audio_shifted = audio
                    else:
                        # Move the last 'shift_samples' to the beginning (looping backward)
                        audio_shifted = torch.cat([audio[-shift_samples:], audio[:-shift_samples]], dim=0)
                elif shift_samples < 0:  # Left shift (events move earlier)
                    k = -shift_samples
                    if k >= audio_len:
                        # If shift is larger than audio length, keep the audio unchanged (full loop)
                        audio_shifted = audio
                    else:
                        # Move the first 'k' samples to the end (looping forward)
                        audio_shifted = torch.cat([audio[k:], audio[:k]], dim=0)
                else:
                    audio_shifted = audio  # No shift
                # cur_audio_dict["audio_data"] = audio_shifted
                audio = audio_shifted


                # Update events with wrapping
                updated_events = []
                for event in cur_audio_dict["events"]:
                    if pd.isna(event["onset"]) or pd.isna(event["offset"]):
                        continue  # Skip events with NaN timestamps
                    new_onset = event["onset"] + shift_seconds
                    new_offset = event["offset"] + shift_seconds

                    # Handle wrapping around the audio duration
                    wrapped_events = []
                    if new_onset < 0 or new_offset > audio_dur:
                        # Normalize timestamps to [0, audio_dur] using modulo
                        new_onset = new_onset % audio_dur
                        new_offset = new_offset % audio_dur

                        if new_onset > new_offset:
                            # Event wraps around: split into two parts
                            # Part 1: from new_onset to audio_dur
                            if new_onset < audio_dur:
                                wrapped_event_1 = event.copy()
                                wrapped_event_1["onset"] = new_onset
                                wrapped_event_1["offset"] = audio_dur
                                wrapped_events.append(wrapped_event_1)
                            # Part 2: from 0 to new_offset
                            if new_offset > 0:
                                wrapped_event_2 = event.copy()
                                wrapped_event_2["onset"] = 0
                                wrapped_event_2["offset"] = new_offset
                                wrapped_events.append(wrapped_event_2)
                        else:
                            # No split needed, just wrapped
                            wrapped_event = event.copy()
                            wrapped_event["onset"] = new_onset
                            wrapped_event["offset"] = new_offset
                            wrapped_events.append(wrapped_event)
                    else:
                        # No wrapping needed
                        if new_onset < new_offset:  # Keep only valid events
                            updated_event = event.copy()
                            updated_event["onset"] = new_onset
                            updated_event["offset"] = new_offset
                            wrapped_events.append(updated_event)

                    updated_events.extend(wrapped_events)
                cur_audio_dict["events"] = updated_events

                # Update vad_timestamps with wrapping
                updated_vad = []
                for vad in vad_timestamps:
                    ...
                    if new_start < 0 or new_end > audio_dur:
                        ...
                        if new_start > new_end:
                            # Event wraps around: split into two parts
                            # Part 1: from new_start to audio_dur
                        ...
                        # No wrapping needed
                        if new_start < new_end:  # Keep only valid intervals
                ...
                vad_timestamps = updated_vad
                cur_audio_dict["vad_timestamps"] = updated_vad

                # Update labels_df with wrapping
                updated_labels_df_rows = []
                for _, row in labels_df.iterrows():
                    if pd.isna(row["onset"]) or pd.isna(row["offset"]):
                        continue  # Skip rows with NaN timestamps
                    new_onset = row["onset"] + shift_seconds
                    new_offset = row["offset"] + shift_seconds
                    ...
                    if new_onset < 0 or new_offset > audio_dur:
                        ...
                        if new_onset > new_offset:
                            ...
                            # Part 1: from new_onset to audio_dur
                            if new_onset < audio_dur:
                                wrapped_row_1 = row.copy()
                                wrapped_row_1["onset"] = new_onset
                                wrapped_row_1["offset"] = audio_dur
                                wrapped_rows.append(wrapped_row_1)
                            # Part 2: from 0 to new_offset
                            if new_offset > 0:
                                wrapped_row_2 = row.copy()
                                wrapped_row_2["onset"] = 0
                                wrapped_row_2["offset"] = new_offset
                                wrapped_rows.append(wrapped_row_2)
                        else:
                            # No split needed, just wrapped
                            wrapped_row = row.copy()
                            wrapped_row["onset"] = new_onset
                            wrapped_row["offset"] = new_offset
                            wrapped_rows.append(wrapped_row)
                    else:
                        # No wrapping needed
                        if new_onset < new_offset:  # Keep only valid events
                            updated_row = row.copy()
                            updated_row["onset"] = new_onset
                            updated_row["offset"] = new_offset
                            wrapped_rows.append(updated_row)

                    updated_labels_df_rows.extend(wrapped_rows)

                # Convert updated rows back to DataFrame
                if updated_labels_df_rows:
                    labels_df = pd.DataFrame(updated_labels_df_rows)
                else:
                    labels_df = pd.DataFrame(columns=labels_df.columns)
                cur_audio_dict["labels_df"] = labels_df

                is_time_shifted = True


            # 注意， 此时的 audio 即有可能经过 time shift , 或者是原始的音频；
            # If time shift was not applied, consider general audio augmentation
        ...
        # Use cached assignments unless a time shift occurred
        if is_time_shifted:
            # Compute assignments based on current vad_timestamps (updated if augmented)
            gt_intervals = [(anno["start"], anno["end"]) for anno in vad_timestamps]
            gt_labels = [anno["event_label"] for anno in vad_timestamps]
            gt_boxes = torch.tensor(gt_intervals) if gt_intervals else torch.empty((0, 2))
            anchor_boxes = anchor_intervals  # Assumed to be pre-scaled to audio_dur
            ...
            if len(gt_boxes) == 0 or gt_boxes.dim() < 2:
                ...
                for a_idx in range(len(anchor_boxes)):
                    if max_ious[a_idx] >= self.iou_threshold:
                        ...
                        assignments.append({
                            "conf": 1.0 if gt_label != "Normal" else 0.0,
                            "cls": interval_4labels[gt_label],  # Assumes interval_4labels is a class attribute
                            "box": gt_intervals[gt_idx]
                        })
                    else:
                        assignments.append({
            ...
            #  note, represent time shift happened,
            # so  need to update each anchor interval's  assignments;
            cur_audio_dict["assignments"] = assignments  # use update assignments;
        else:
    ...
    # def compute_assignments(self, anchor_intervals, vad_timestamps):
    #     assignments = []
    #     for anchor in anchor_intervals:
    #         max_iou = 0
    #         best_event = None
    #         anchor_start, anchor_end = anchor
    #
    #         for vad in vad_timestamps:
    #             event_start, event_end = vad['start'], vad['end']
    #             intersect = min(anchor_end, event_end) - max(anchor_start, event_start)
    #             union = (anchor_end - anchor_start) + (event_end - event_start) - intersect
    ...
    #         if max_iou > 0.5:
    #             assignments.append({
    #                 'box': [best_event['start'], 0, best_event['end'], 1],
    #                 'cls': 1,
    #                 'conf': 1.0
    #             })
    #         else:
