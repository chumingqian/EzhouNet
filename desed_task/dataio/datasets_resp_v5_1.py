...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='generate the frame labels,' )):
            ...
            labels = cur_dict.get("events", [])
            # Determine record_multi_labels for current audio
            event_labels = [event["event_label"] for event in labels]
            unique_labels = set(event_labels)

            # if unique_labels == {"Normal"}:
            #     # record_multi_labels = "Normal"
            #     record_multi_labels = ["Normal"]
            # else:
            #     non_normal_labels = unique_labels - {"Normal"}
            #     record_multi_labels = ", ".join(sorted(non_normal_labels)) if non_normal_labels else "Normal"
            #
            # # Update the current example with the generated multi-label
            # self.examples[audio_name]["record_multi_labels"] = record_multi_labels
            #
            # # Count the number of samples for each category
            # if record_multi_labels not in multi_label_counts:
            #     multi_label_counts[record_multi_labels] = 0
            # multi_label_counts[record_multi_labels] += 1


            if unique_labels == {"Normal"}:
                record_multi_labels = ["Normal"]
            else:
                non_normal_labels = list(unique_labels - {"Normal"})
                record_multi_labels = sorted(non_normal_labels) if non_normal_labels else ["Normal"]

            # Update the current example with the generated multi-label
            self.examples[audio_name]["record_multi_labels"] = record_multi_labels

            # Count the number of samples for each category
            label_key = ", ".join(record_multi_labels)
            if label_key not in multi_label_counts:
                multi_label_counts[label_key] = 0
            multi_label_counts[label_key] += 1




        # Display the final count for each category
        print(f" \n The  multi label  on the  record Level:")
        for label, count in multi_label_counts.items():
            ...
            # 最终，没有这样实施的原因是， 在audio 数据增强后， 音频的长度仍是发生偏移，导致部分音频的帧数增加或减少；
            # 这表明的当前的一些数据增强， 确实改变了音频的长度，虽然微小， 但确实存在；
...
class StratifiedMultiLabelBatchSampler(Sampler):
    """
    Sampler that ensures each batch contains at least N samples per exception type.
    """
    def __init__(self, dataset, batch_size, samples_per_exception=6):
        """
        Args:
            dataset: The dataset to sample from. Must have an attribute 'record_multi_labels'.
            batch_size: The desired batch size.
            samples_per_exception: Minimum number of samples per exception type in each batch.
        """
        self.dataset = dataset
        self.samples_per_exception = samples_per_exception

        # Define the set of individual exception types
        self.exception_types = ['Normal', 'Stridor', 'Coarse Crackle', 'Fine Crackle', 'Wheeze', 'Rhonchi', 'Wheeze+Crackle']
        self.num_exception_types = len(self.exception_types)

        # Build a mapping from exception type to sample indices
        self.exception_type_to_indices = {etype: [] for etype in self.exception_types}

        # For each sample, get its exception types
        for idx in range(len(self.dataset)):
            # record_labels = self.dataset.record_multi_labels[idx]
            # train_dataset.examples[train_dataset.examples_list[32]]["record_multi_labels"]
            ...
            for label in labels:
                if label in self.exception_types:
                    self.exception_type_to_indices[label].append(idx)

        # Check if there are enough samples for each exception type
        for etype in self.exception_types:
            if len(self.exception_type_to_indices[etype]) < self.samples_per_exception:
                print(f"Warning: Not enough samples for exception type '{etype}'. "
                      f"Required: {self.samples_per_exception}, Available: {len(self.exception_type_to_indices[etype])}")

        # Calculate batch size
        self.batch_size = self.samples_per_exception * self.num_exception_types
    ...
    def __iter__(self):
        # Create iterators for exception types
        exception_type_iters = {}
        for etype in self.exception_types:
            indices = self.exception_type_to_indices[etype]
            indices = indices.copy()
            np.random.shuffle(indices)
            exception_type_iters[etype] = iter(indices)

        batches_generated = 0
        while batches_generated < self.num_batches:
            batch_indices = []
            # Collect samples_per_exception samples for each exception type
            for etype in self.exception_types:
                indices_iter = exception_type_iters[etype]
                selected_indices = []
                while len(selected_indices) < self.samples_per_exception:
                    try:
                        idx = next(indices_iter)
                        selected_indices.append(idx)
                    except StopIteration:
                        # Reshuffle and reset iterator
                        indices = self.exception_type_to_indices[etype]
                        if not indices:
                            break  # Skip if no samples for this exception type
                        indices = indices.copy()
                        np.random.shuffle(indices)
                        exception_type_iters[etype] = iter(indices)
                        idx = next(exception_type_iters[etype])
                        selected_indices.append(idx)
                batch_indices.extend(selected_indices)
            # If batch size not reached (due to missing samples), fill with random samples
