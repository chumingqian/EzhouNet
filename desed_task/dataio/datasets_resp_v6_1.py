...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='reorganize audio info,' )):
            ...
            # Skip processing if there are no valid events
            # if labels_df.empty or labels_df["event_label"].isna().all():
            #     continue
...
# class MockEncoder:
#     def __init__(self):
#         self.labels = ['Normal', 'Abnormal1', 'Abnormal2', 'Abnormal3']
#         self.n_frames = 156  # Assuming 156 frames for frame-level label generation
#
#     def encode_variable_df(self, labels_df, cur_audio_frames):
#         # Return a fake encoded label matrix (n_frames, n_labels)
#         # Here we use random values for simplicity, replace with actual encoding logic
#         return np.random.randint(0, 2, size=(cur_audio_frames, len(self.labels)))
#
# # Prepare mock tsv_entries
# mock_data = {
#     'filename': ['audio1.wav', 'audio2.wav'],
#     'onset': [0.0, 1.0],
#     'offset': [5.0, 6.0],
#     'event_label': ['Normal', 'Abnormal1'],
#     'confidence': [0.9, 0.8],
#     'record_bin_label': ['Normal', 'Abnormal'],
#     'record_abnormal_label': ['None', 'Abnormal1']
# }
# tsv_entries = pd.DataFrame(mock_data)
#
# # Initialize a MockEncoder
# encoder = MockEncoder()
#
# # Initialize RespiraGnnSet with mock data
# respira_dataset = RespiraGnnSet(
#     audio_folder='/media/respecting_god/S2/05_soundEventDetection/Respira_SED_LGNN/recipes/dcase2023_task4_baseline/confs',  # Replace with an actual path or a mock path
...
# )
#
# # Use DataLoader to load data from RespiraGnnSet
# loader = DataLoader(respira_dataset, batch_size=2, collate_fn=lambda x: x)
#
...
#             else:
#                 print(f"{key}: {value}")
#     break  # Stop after first batch for testing purposes
