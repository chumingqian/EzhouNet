...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        device = batch_data['spectrograms'].device
        ...
        device = next(self.sed_net.parameters()).device  # Get model's device
    ...
    def custom_collate(self, batch):
        ...
        # Initialize lists for padded spectrograms and masks
        padded_spectrograms = []
        padded_frame_labels = []
        masks = []

        for spec, labels in zip(spectrograms, frame_labels):
            n_frames = spec.shape[1]
            pad_size = max_frames - n_frames

            # Pad spectrogram
            pad_spec = torch.nn.functional.pad(spec, (0, 0, 0, pad_size), mode='constant', value=0)
            padded_spectrograms.append(pad_spec)

            # Pad labels
            pad_labels = torch.nn.functional.pad(labels, (0, pad_size), mode='constant', value=0)
            padded_frame_labels.append(pad_labels)

            # Create mask (1 for valid frames, 0 for padded frames)
            mask = torch.cat([torch.ones(n_frames, dtype=torch.bool),
                              torch.zeros(pad_size, dtype=torch.bool)])
            masks.append(mask)

        # Stack into tensors
        batch_spectrograms = torch.stack(padded_spectrograms)  # Shape: (batch_size, channels, max_frames, n_mels)
        batch_frame_labels = torch.stack(padded_frame_labels)  # Shape: (batch_size, num_classes, max_frames)
        masks = torch.stack(masks)  # Shape: (batch_size, max_frames)

        # Prepare batch data
        batch_data = {
            'spectrograms': batch_spectrograms,
