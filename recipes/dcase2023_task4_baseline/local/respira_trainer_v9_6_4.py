...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_idx):
       ...
       # print(f"total_loss grad_fn: {total_loss.grad_fn is not None}")
        ...
        edge_epochs = 50
        node_conf_op_epochs = 30
        s1_transition_epochs = 20
        node_stage_epoch =  edge_epochs +  node_conf_op_epochs + s1_transition_epochs
        ...
        # optimize the node  confidence : 0-50 epoch
        if self.current_epoch < edge_epochs:
            total_loss = edge_bin_loss


        # optimize the node  confidence : 50-80 epoch
        elif edge_epochs <  self.current_epoch < edge_epochs +  node_conf_op_epochs:
            total_loss = node_conf_loss

        # optimize the  node classification  : 80-100 epoch
        elif  edge_epochs +  node_conf_op_epochs < self.current_epoch < node_stage_epoch:
            alpha1 = (self.current_epoch - node_conf_op_epochs) / s1_transition_epochs
            total_loss =  alpha1 * (node_conf_loss + edge_bin_loss )  +  node_level_loss
