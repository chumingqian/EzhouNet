...
class GraphRespiratory(nn.Module):
    ...
    def forward(self, batch_data, stage):
        ...
        if stage == "classification":
            ...
            # Aggregate node embeddings to get sample-level embeddings
            # sample_embeddings = []
            # start_idx = 0
            # for num_groups in num_groups_per_sample:
            #     end_idx = start_idx + num_groups
            #     sample_node_embeddings = node_embeddings[start_idx:end_idx]
            #     # Use global mean pooling (or another readout function)
            #     sample_embedding = torch.mean(sample_node_embeddings, dim=0)  # Shape: (feature_dim,)
            #     sample_embeddings.append(sample_embedding)
            #     start_idx = end_idx
            #
            # # Stack sample embeddings
            # sample_embeddings_tensor = torch.stack(sample_embeddings)  # Shape: (batch_size, feature_dim)
            #
            # # Perform record-level classification
            # record_binary_predictions = self.record_classifier(
            #     sample_embeddings_tensor)  # Shape: (batch_size, num_classes)
