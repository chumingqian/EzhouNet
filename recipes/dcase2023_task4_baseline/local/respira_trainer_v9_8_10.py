...
# repira_v9-8-4,  启用数据增强， 修改了AugmentWrapper， note
# 1. 其中的audio_aug_prob 数值会决定是否启用数据增强
# 2. 对区间学习率使用多步骤， 学习率衰减的方式；
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        # Add this to the class initialization (__init__)
        self.category_nms_thresholds = {
            "Stridor": 0.5,
            "Wheeze": 0.4,
            "Crackle": 0.15,
            "Rhonchi": 0.3
    ...
    def inference_one_sample(self,
        ...
        # Compute predicted classes for grouping
        pred_classes = torch.argmax(class_logits, dim=-1)  # (K,)
        ...
        for cls_id in unique_classes:
            ...
            cls_bounds = bounds[cls_mask]
            cls_conf = conf_scores[cls_mask]
            cls_subtypes = pred_classes[cls_mask]  # All same

            if cls_bounds.numel() == 0:
                continue

            # Sort by descending confidence
            sort_inds = torch.argsort(cls_conf, descending=True)
            cls_bounds = cls_bounds[sort_inds]
            cls_conf = cls_conf[sort_inds]
            cls_subtypes = cls_subtypes[sort_inds]

            # Get class-specific NMS threshold
            label = id_to_label.get(cls_id.item(), "Unknown")
            nms_th = self.category_nms_thresholds.get(label, 0.5)  # Default 0.5 if not found

            # Apply NMS
            final_indices = []
            for i in range(len(cls_bounds)):
                keep = True
                for j in final_indices:
                    if self.interval_iou(cls_bounds[i], cls_bounds[j]) > nms_th:
                        keep = False
                        break
                if keep:
            ...
            # Append kept
            final_bounds.append(cls_bounds[final_indices])
            final_conf.append(cls_conf[final_indices])
            final_subtypes.append(cls_subtypes[final_indices])

        # Combine across classes
        if final_bounds:
            final_bounds = torch.cat(final_bounds, dim=0)
            final_conf = torch.cat(final_conf, dim=0)
            final_subtypes = torch.cat(final_subtypes, dim=0)
        else:
            final_bounds = torch.empty((0, 2))
            final_conf = torch.empty((0,))
            final_subtypes = torch.empty((0,), dtype=torch.long)

        return final_bounds, final_conf, final_subtypes
    ...
    def training_step(self, batch_data, batch_idx):
       ...
       # print(f"total_loss grad_fn: {total_loss.grad_fn is not None}")
        ...
        interval_level_loss = (conf_loss + interval_cls_loss + loc_loss)
    ...
    def on_validation_epoch_end(self, outputs= None):
        ...
        try:
            ...
            for idx, cur_thread in enumerate(self.hparams["training"]["val_conf_thresholds"]):
                print(f"\nUsing confidence threshold: conf={cur_thread}")
                print("Category-specific NMS IoU thresholds:")
                for cat, th in self.category_nms_thresholds.items():
                    print(f"  {cat}: {th}")
        ...
        if node_event_thread_f_score > self.best_val_score:
            ...
            # Save best_val_score to conf-specific directory
            conf_save_dir = os.path.join(self.hparams["log_dir"], f"conf{idx}_pred_interval_event_metrics")
            os.makedirs(conf_save_dir, exist_ok=True)
            with open(os.path.join(conf_save_dir, "best_val_score.txt"), "w") as f:
                f.write(f"{self.best_val_score}\n")

            # Save best_val_score to best directory
            best_save_dir = os.path.join(self.hparams["log_dir"], f"best_pred_interval_event_metrics")
            os.makedirs(best_save_dir, exist_ok=True)
            with open(os.path.join(best_save_dir, "best_val_score.txt"), "w") as f:
                f.write(f"{self.best_val_score}\n")
    ...
    def train_dataloader(self):
        ...
        # note, here audio_aug_prob this value will determine weather use the  data augment for the respiratory audio;
        train_dataset = AugmentWrapper(combined_dataset, audio_aug_prob=0.0)
    ...
    # def val_dataloader(self):
    #     val_dataset = AugmentWrapper(self.valid_data, audio_aug_prob=0.0)
    #     self.val_loader = DataLoader(
    #         val_dataset,
    #         batch_size=self.hparams["training"]["batch_size_val"],
    #         num_workers=self.num_workers,
