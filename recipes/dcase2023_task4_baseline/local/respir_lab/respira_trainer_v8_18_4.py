...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.node_loss_weight_initial = 5.0
    ...
    def training_step(self, batch_data, batch_indx, optimizer_idx):
        ...
        conf_loss_value = conf_loss.item()  # Scalar value of confidence loss
        interval_conf_loss_threshold = 0.20
        min_conf_loss = 0.10
        initial_weight = 1.0
        max_weight = 10.0

        if conf_loss_value < interval_conf_loss_threshold:
            if conf_loss_value <= min_conf_loss:
                dynamic_weight = max_weight
            else:
                # Linear interpolation between initial_weight (1.0) and max_weight (1.5)
                factor = (interval_conf_loss_threshold - conf_loss_value) / (interval_conf_loss_threshold - min_conf_loss)
                dynamic_weight = initial_weight + factor * (max_weight - initial_weight)
        else:
            dynamic_weight = initial_weight

        # Apply dynamic weight to interval_cls_loss
        #  interval_cls_loss_weighted = dynamic_weight * self.interval_cls_loss_weight * interval_cls_loss
    ...
    def configure_optimizers(self):
        ...
        optimizer_node = torch.optim.Adam(node_params, lr=self.node_lr_initial)
        optimizer_interval = torch.optim.Adam(interval_params, lr=self.interval_lr_initial)

        # Scheduler for node parameters: Exponential decay
        scheduler_node = {
            'scheduler': torch.optim.lr_scheduler.LambdaLR(
                optimizer_node,
                lr_lambda=lambda epoch: 0.99 ** epoch  # Replaces self.compute_lr_factor
            ),
            'interval': 'epoch',
            'frequency': 1,
        ...
        # Scheduler for interval parameters: Cosine Annealing (unchanged)
        scheduler_interval = {
            'scheduler': torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer_interval,
                T_max=400  # Adjust based on total epochs
            ),
            'interval': 'epoch',
            'frequency': 1,
        ...
        print(f"\n node parameter {optimizer_interval}")
        ...
        print(f"\n inteval parameter {optimizer_interval}")
        ...
        #     {'params': node_params, 'lr': self.node_lr_initial, 'name': 'node_cls'},
        #     {'params': interval_params, 'lr': self.interval_lr_initial, 'name': 'interval_tasks'}  # Larger
        # ])
        #
        ...
        #     optimizer,
        #     lr_lambda=[
        #         lambda epoch: self.compute_lr_factor(epoch, 'node_cls'), ]
        # )
        #
        # scheduler_config = {
        #     'scheduler': scheduler,
        ...
        return [optimizer_node, optimizer_interval], [scheduler_node, scheduler_interval]
    ...
    def on_train_epoch_start(self):
        ...
        # 用于记录学习率的调整
        # Log learning rate for node parameters
        lr_node = self.optimizers()[0].param_groups[0]['lr']
        self.log('lr_node', lr_node, on_epoch=True)

        # Log learning rate for interval parameters
        lr_interval = self.optimizers()[1].param_groups[0]['lr']
