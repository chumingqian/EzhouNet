...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.interval_unify_head_params = (list(self.sed_net.node_interval_pred.interval_refine.parameters())
        ...
        self.node_conf_loss_weight = 4.0
        self.node_loss_weight = 2.0

        self.interval_conf_loss_weight =  50.0
    ...
    #def training_step(self, batch_data, batch_idx):
        ...
        # ---------------------  node  confidence, cls,  ---------------
        # node label
        node_type_labels = outputs_detection['node_type_labels']  # Shape: (total_nodes,)  values  from -1, 0-3; # Shape: [1711], includes -2 for virtual nodes
        node_confidences = outputs_detection['node_confidences']  # Shape: (total_nodes,) ,# Shape: [1699], regular nodes only
        node_predictions = outputs_detection['node_predictions']  # Shape: (total_nodes, 5), # Shape: [1711, num_classes]

        # np.set_printoptions(threshold=np.inf)  # Show all elements
        # # # Print full edge_index
        # print(node_type_labels.cpu().numpy())
        ...
        # Node confidence loss
        node_conf_mask = (node_type_labels != -2)  # True for regular nodes
        node_conf_logits = node_predictions[node_conf_mask, 0] # exclude the  regular nodes;
        # Virtual nodes don’t have confidence values, as they don’t correspond to audio chunks.
        node_conf_targets = node_confidences
        node_conf_loss = self.node_conf_loss_fn(node_conf_logits, node_conf_targets)

        # Node classification loss
        node_fg_mask = (node_type_labels != -1) & (node_type_labels != -2)
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        # Node confidence loss
        node_conf_mask = (node_type_labels != -2)
        node_conf_logits = node_predictions[node_conf_mask, 0]
    ...
    def configure_optimizers(self):
        ...
        node_params = self.shared_params + self.node_conf_cls_head_params
        interval_params = self.interval_unify_head_params

        # Create optimizers for each parameter group
        optimizer_node = torch.optim.Adam(node_params, lr=lr_node_initial)
        optimizer_interval = torch.optim.Adam(interval_params, lr=lr_interval_initial)
        ...
        # min_lr_factor = 5e-5 / lr_node_initial  # e.g., 1e-5 / 1e-3 = 0.01,
        # Scheduler for node parameters: Exponential decay
        scheduler_node = {
            'scheduler': torch.optim.lr_scheduler.LambdaLR(
                optimizer_node,
                #lr_lambda=lambda epoch:max(min_lr_factor, 0.99 ** epoch)  # Enforce minimum
    ...
    def on_train_epoch_start(self):
        ...
        lr_node_conf_cls = optimizer[0].param_groups[0]['lr']  # confidence
        lr_interval_unify = optimizer[1].param_groups[0]['lr']  # classification
        # lr_shared = optimizer.param_groups[2]['lr']  # shared

        print(f"Epoch {self.current_epoch}: lr_node_conf_cls={lr_node_conf_cls}, "
              f"lr_interval_unify={lr_interval_unify}, ")
