...
# import torch_sparse
from desed_task.nnet.DCNN_v3_2 import  DynamicFeatureExtractor
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.node_fea_proj2_vad =  nn.Linear(edge_vad_fea, 1)
        self.edge_fea_proj2_vad =  nn.Linear( 2* edge_vad_fea, 1)
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            node_features, node_vad = self.node_fea_generator(all_chunks_tensor)  # Output: (total_nodes, feature_dim)


            # should compute loss  with  node_vad_flags_all: (total_nodes ),
            node_vad_logit =  self.node_fea_proj2_vad(node_vad)  # tensors: (total_nodes, fea_dim=1)

            # Add VAD flag to node features
            node_features = torch.cat([node_features, node_vad_logit], dim=1)
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                sample_node_vad = node_vad[start_idx:end_idx]
                ...
                if num_nodes_sample > 1:
                    ...
                    combined_edge_features = torch.cat([src_node_vad, dst_node_vad],
                                                       dim=1)  # [E, 2 * feature_dim] (191, 2*32)

                    edge_vad_logit = self.edge_fea_proj2_vad(combined_edge_features)  # Output: (total_nodes, fea_dim=1)
                    # Encode edge attributes to learnable embeddings
                    edge_features = self.edge_encoder(edge_vad_logit)  # 这里边的属性，直接使用一个可学习参数；
                ...
                all_edge_attr.append(edge_vad_logit)
                start_idx = end_idx
            ...
            # all_edge_attr is a list of Tensors of shape [E_i, 1]
            edge_vad_attr_concat = torch.cat(all_edge_attr, dim=0)
            ...
            outputs = {
                ...
                'edge_vad_attr': edge_vad_attr_concat.squeeze(1), #  边的携带 vad 的属性； 与 edge label 形成loss
