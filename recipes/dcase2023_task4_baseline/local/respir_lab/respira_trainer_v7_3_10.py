...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.vad_loss_fn = nn.BCEWithLogitsLoss()  # For VAD prediction
        ...
        self.classification_loss_weight_final = 20.0
        ...
        self.edge_attr_loss_weight =  1.0  #  时间属性；
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        edge_vad_attr_pred = outputs_detection['edge_vad_attr']  # (batch_edge_nums)
        ...
        #         f"Shape mismatch: edge_vad_attr_pred.shape = {edge_vad_attr_pred.shape}, edge_labels.shape = {edge_labels.shape}")
        if isinstance(edge_labels, list):
            edge_labels = torch.as_tensor(edge_labels, dtype=torch.float32, device=device)
        elif isinstance(edge_labels, torch.Tensor):
            edge_labels = edge_labels.to(dtype=torch.float32, device=device)

        edge_attr_loss = self.vad_loss_fn(edge_vad_attr_pred, edge_labels)
        ...
        self.log("train/edge_attr_loss", edge_attr_loss, prog_bar=True)
    ...
    def validation_step(self, batch_data, batch_indx):
        ...
        edge_vad_attr_pred = outputs_detection['edge_vad_attr']  # (E, edge_num_classes)
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        self.valid_edge_attr_loss = self.vad_loss_fn(edge_vad_attr_pred, edge_labels)


        self.valid_total_loss =   (self.valid_node_loss   + self.valid_edge_loss
                                   + self.consistency_loss
                                   + self.valid_node_vad_loss+ self.valid_edge_attr_loss)
        ...
        self.log("valid/edge_attri_loss", self.valid_edge_attr_loss, prog_bar=True)
    ...
    def configure_optimizers(self):
        ...
        node_learing_params = (
            ...
            list(self.sed_net.edge_fea_proj2_vad.parameters()) +
