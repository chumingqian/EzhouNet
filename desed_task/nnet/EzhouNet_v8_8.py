...
#  v8-8,  直接使用 anchor + delta + offset logit;
...
class EnhancedTimeAwareFDR(nn.Module):
    ...
    def forward(
        ...
        # Combine with global abnormal features.
        interval_gen_input = torch.cat([query_features, global_abnormal_feature],
                                       dim=-1)  # (max_intervals, node_embedding_dim+2)
        # Instead of directly predicting centers and widths, the network now predicts offsets.
        delta_params = self.interval_generator(interval_gen_input)  # (max_intervals, 4)
        delta_center = delta_params[:, 0]
        delta_width = delta_params[:, 1]
        # (The remaining two outputs can be used for existence or type if needed.)

        # Retrieve fixed anchor intervals.
        anchors = self.anchor_intervals.to(device)  # shape (max_intervals, 2): [center, width]
        # Adjust the anchors with the predicted offsets.
        # Here, the predicted center is computed by a simple addition.
        # For width, an exponential transformation (as in YOLO) ensures positive values.
        pred_centers = (anchors[:, 0] + delta_center).clamp(0, 1)
        pred_widths = (anchors[:, 1] * torch.exp(delta_width)).clamp(0, 1)
