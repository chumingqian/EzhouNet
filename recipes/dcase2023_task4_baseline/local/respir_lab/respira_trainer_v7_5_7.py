...
from .utils import  batched_node_vad_decode_preds,  compute_event_based_metrics
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.node_vad_loss_weight = 20.0   # 时间属性
        ...
        self.val_buffer_vad_abnormal_node= {  # 为不同阈值创建多个 DataFrame 缓冲区，用于存储合成数据和测试数据的验证结果。
            k: pd.DataFrame() for k in self.hparams["training"]["val_thresholds"]
        }
    ...
    def validation_step(self, batch_data, batch_indx):
        ...
        node_vad_pred_labels = torch.argmax(node_vad_pred, dim=1)  # Shape: (num_nodes,)
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        self.valid_total_loss =   (self.valid_node_loss   + self.valid_edge_loss
                                   + self.consistency_loss + self.valid_node_emb_vad_loss
        ...
        scores_postprocessed_node_pred, decoded_node_pred,decoded_abnormal_pred, refine_pred \
            = batched_node_vad_decode_preds(
            node_predictions,
            node_vad_pred_labels,
            filenames_synth,
        ...
        for th in self.val_buffer_vad_abnormal_node.keys():
            self.val_buffer_vad_abnormal_node[th] = pd.concat(
                [self.val_buffer_vad_abnormal_node[th], refine_pred[th]],
                ignore_index=True,
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        try:
            ...
            print("\n Using Node Vad  & combine thread-based to compute")
            node_combine_edge_scored = compute_event_based_metrics(
                self.val_buffer_vad_abnormal_node[cur_thread],
                self.hparams["data"]["valid_tsv"],
                save_dir=os.path.join(self.hparams["log_dir"], "node_event_level_metrics"),
            )
        ...
        node_event_thread_f_score = node_event_segment_scored[0] + node_combine_edge_scored[0]
        ...
        self.val_buffer_vad_abnormal_node = {th: pd.DataFrame() for th in self.val_buffer_vad_abnormal_node.keys()}
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2, reduction='mean', epsilon=1e-8):
        ...
        # Handling alpha: ensuring it's a tensor and on the same device as inputs
    ...
    def forward(self, inputs, targets):
        ...
        # Gather log probs and probs for the target class
        log_probs = log_probs.gather(1, targets)  # Shape: (batch_size, 1)
        probs = probs.gather(1, targets)  # Shape: (batch_size, 1)

        # Handle alpha tensor: Ensure it is on the same device as inputs
        if self.alpha is not None:
