...
class RespiraGnnSet(Dataset):
    ...
    def generate_anchor_intervals(self, audio_length):
        ...
        # [0.5, 0.75, 1s,  1.25s,  1.6s]   new five scale，
        # # 0.75s, 1s most number;  0.5s, second     last 2 kind has the least number ；
        # # set weight param 为 [-100, 100],    use clmap  to ensure the range ;
        desired_durations = torch.tensor([0.5, 0.75,  1.0, 1.25,  1.65])
        ...
        weights = torch.tensor([0.5, 1.0, 1.0, 0.5, 0.5])  # Weights for scales 1, 2, and 3
        ...
        anchor_intervals_list = []
        for k in range(len(num_centers)):
            n_k = num_centers[k]
