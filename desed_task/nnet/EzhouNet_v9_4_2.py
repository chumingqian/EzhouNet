...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        self.unify_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(2 * node_embedding_dim + 35, 256),
                nn.ReLU(),
                nn.Dropout(0.2),
                nn.Linear(256, 256),
                nn.ReLU(),
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            shared_feature = torch.cat([
                ...
                pos_embedding.repeat(num_intervals_scale, 1) #  here  add the respira position  infor
            ], dim=1)
...
class Node_Interval_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
        ...
        self.pos_linear = nn.Linear(position_dim, 32)  # pos_dim = 32
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop,location_one_hot   ): # pass the batch data
        ...
        for i in range(num_audios):
            ...
            respir_posi = location_one_hot[mask][0]  # Same for all nodes in audio,
            pos_embedding = self.pos_linear(respir_posi)
