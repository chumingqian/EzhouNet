...
class RespiraGnnSet(Dataset):
    ...
    def spec_extract_features(self, audio, binary_label):
        ...
        clip_mel_resized = F.interpolate(clip_mel, size=(self.n_filters , clip_mel.shape[3]),
        ...
        mel_standard = standard_mel_spec.unsqueeze(0)  # (1, 84, 626)
        ...
        cqt_standard = standard_cqt_spec.unsqueeze(0)
        ...
        gamma_standard = standard_gamma_spec.unsqueeze(0)
        ...
        if self.train_flag and binary_label == "Abnormal"  and spec_Aug > 0.5  :
            mel_standard   = self.spec_aug(mel_standard)
            cqt_standard   = self.spec_aug(cqt_standard)
            gamma_standard = self.spec_aug(gamma_standard)

        # (chan =3, n_mels=84,  frames= variable)
        hybrid_spectrogram  = torch.cat((cqt_standard, gamma_standard, mel_standard,), dim=0)
        # 对语谱图特征在通道维度上进行标准化；
