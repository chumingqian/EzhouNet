...
# hf_lab3_6,  分离头， 将音频重采样至8k,  保持原始的 nfft 配置进行训练；
...
class Interval_Refine(nn.Module):
    ...
    def forward(   # change  back  to  process  one  sample
        ...
        for scale_idx in range(self.num_scales):
            ...
            temporal_encoding = scale_factor * torch.sin(
                time_positions * freqs)  # Shape: (num_nodes, node_embedding_dim)
            x = node_embeddings + temporal_encoding  # Shape: (num_nodes, node_embedding_dim)

            # print("\n Node embeddings mean:", node_embeddings.abs().mean().item())
