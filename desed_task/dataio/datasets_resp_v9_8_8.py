...
def gen_augmented(original, sample_rate):
    ...
    augment_list = [
        ...
        # naa.NoiseAug(),
        # 1. Controlled additive noise (time-safe, freq-safe)
        lambda x: add_noise(x, noise_factor_range=(0.001, 0.01)),

        # 2. Loudness / gain change (time-safe, freq-safe)
        lambda x: naa.LoudnessAug(factor=(0.8, 1.2)).augment(x)[0],

        # 3. Small VTLP warp (time-safe if kept mild)
        lambda x: naa.VtlpAug(sampling_rate=sample_rate, zone=(0.0, 1.0), fhi=(0.9, 1.1)).augment(x)[0],

        # 4. Small pitch shift without time stretch (time-safe, mostly freq-safe if mild)
        lambda x: naa.PitchAug(sampling_rate=sample_rate, factor=(-0.5, 0.5)).augment(x)[0],
    ]
    ...
    aug_fn = random.choice(augment_list)
    aug_data = aug_fn(original)
    return  aug_data
...
class RespiraGnnSet(Dataset):
    ...
    def __getitem__(self, idx,apply_general_aug=False ):
        ...
        if self.train_flag and binary_label == "Abnormal" and  apply_general_aug:
            ...
            elif not contains_rare_event  and random.random() > 0.8:  # augment for other two abnormal;
                #  print("\n 0.2 prob Augmentation for (crackle or wheeze) ")
                audio_np = audio.numpy() if not isinstance(audio, np.ndarray) else audio
                audio_np = gen_augmented(audio_np, self.fs)
                audio = torch.from_numpy(np.array(audio_np, dtype=np.float32)).to(torch.float32)
