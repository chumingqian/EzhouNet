...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.interval_unify_head_params = (list(self.sed_net.node_interval_pred.interval_refine.interval_refine_head.parameters()) +
                                           list(self.sed_net.node_interval_pred.interval_refine.interval_conf_cls_heads.parameters())
                                           )
        ...
        print(f"Node conf/cls params count: {len(self.node_conf_cls_head_params)}")
        print(f"Interval unify params count: {len(self.interval_unify_head_params)}")
        ...
        self.classes_labels = node_cls_labels
        ...
        self.interval_conf_loss_fn = FocalLossBinary(alpha=0.70, gamma=2.0, reduction='mean')
        ...
        self.node_loss_weight =  1.0 #2.0


        self.interval_conf_loss_weight =   50.0  # 40.0
        self.interval_cls_loss_weight =    15.0 #15.0    #50.0
    ...
    def configure_optimizers(self):
        ...
        T_max = 28000  # Reach minimum at 28k steps
        eta_min = 1e-8  # Minimum learning rate, close to 1.5e-8
        # Scheduler for node parameters: Step-based cosine annealing
        scheduler_node = {
            'scheduler': torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer_node,
                T_max=T_max,
        ...
        # Scheduler for interval parameters: Step-based cosine annealing
        scheduler_interval = {
            'scheduler': torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer_interval,
                T_max=T_max,
