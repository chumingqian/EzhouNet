...
class Interval_Refine(nn.Module):
    ...
    def forward(
        ...
        starts = cur_anchor_intervals[:, 0]
        ends = cur_anchor_intervals[:, 1]
        ...
        refined_bounds = []
        time_pos_1d = time_positions.squeeze(-1) * audio_len.item() # back to the auctual time length;
       ...
       # print(f"the number of current sample's  prior anchor intervals : {self.num_anchor_intervals} ")
        ...
        for i in range(len(cur_anchor_intervals)):
            ...
            for layer in self.refinement_layers:
                query = torch.cat([
                    local_feat,
                    center.view(1)/audio_len.item() , # use normalized
                    width.view(1)/audio_len.item(),
                    start.view(1)/audio_len.item(),
                    end.view(1)/audio_len.item(),
                ...
                # note, it's wrong here,  should  be without clamp(0, 1);
