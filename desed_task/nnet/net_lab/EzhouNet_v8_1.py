...
class FDR(nn.Module):
    def __init__(self, num_classes, in_channels=256, hidden_dim=256, max_intervals=5):
        super().__init__()
        ...
        # 初始时间边界预测（虽然支持多区间，但对于正常呼吸音主要使用第一个区间）
        self.initial_boundary = nn.Sequential(
            nn.Linear(in_channels, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, 2 * max_intervals)  # 每个区间预测开始和结束时间
        )

        # 概率分布预测（为每个区间的开始和结束时间生成分布）
        self.start_distribution = nn.Sequential(
            nn.Linear(in_channels, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, 100 * max_intervals)  # 每个区间生成100个点的分布
        )

        self.end_distribution = nn.Sequential(
            nn.Linear(in_channels, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, 100 * max_intervals)  # 每个区间生成100个点的分布
        )

        # Learnable weighting function (D-FINE style) <<<<<<<
        self.weight_params = nn.Parameter(
            torch.linspace(-0.5, 0.5, 100),  # Initialize like original fixed version
            requires_grad=True  # Make parameters learnable
        )

    def forward(self, x):

        # ... (keep existing forward code)
        batch_size = x.size(0)
        ...
        # 预测初始边界
        initial_bounds = self.initial_boundary(x)  # [B, 2*max_intervals]
        initial_bounds = initial_bounds.view(batch_size, self.max_intervals, 2)

        # 生成概率分布
        start_logits = self.start_distribution(x)  # [B, 100*max_intervals]
        end_logits = self.end_distribution(x)  # [B, 100*max_intervals]

        start_logits = start_logits.view(batch_size, self.max_intervals, 100)
        end_logits = end_logits.view(batch_size, self.max_intervals, 100)

        start_dist = F.softmax(start_logits, dim=-1)  # [B, max_intervals, 100]
        end_dist = F.softmax(end_logits, dim=-1)  # [B, max_intervals, 100]

        # 使用加权函数计算最终的边界调整值
        start_offsets = torch.stack([self.weighted_sum(dist) for dist in start_dist.unbind(dim=1)], dim=1)
        end_offsets = torch.stack([self.weighted_sum(dist) for dist in end_dist.unbind(dim=1)], dim=1)

        # 最终的时间边界
        final_start = initial_bounds[..., 0:1] + start_offsets
        final_end = initial_bounds[..., 1:2] + end_offsets
        ...
        return {
            ...
            'final_bounds': torch.cat([final_start, final_end], dim=-1),
            'num_intervals': num_intervals,
            'interval_mask': interval_mask
        }

    def weighted_sum(self, distribution):
        """Modified learnable weighting function (D-FINE style)"""
