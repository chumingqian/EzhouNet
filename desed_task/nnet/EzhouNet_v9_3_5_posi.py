...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        self.interval_refine_head =  nn.ModuleList([
                nn.Sequential(
        ...
        self.interval_conf_cls_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(node_embedding_dim + 1, 64),  # Hidden layer
                nn.ReLU(),
                nn.Dropout(0.2),  # Regularization
                nn.Linear(64,  1 +  self.num_classes - 1)  # Output layer
            ) for _ in range(self.num_scales)
        ])
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            refine_head = self.interval_refine_head[scale_idx]
            weight_params = self.weight_params[scale_idx]
            conf_cls_head  = self.interval_conf_cls_heads[scale_idx]
            ...
            local_content_feat = torch.cat([local_feat, local_abnormal_feature.unsqueeze(-1)], dim=1)

            # septe  head for localization logits, interval confidence, classification

            refine_logit = refine_head(query_feature)
            start_logits = refine_logit[:, :self.dist_bins[scale_idx]]
            end_logits   = refine_logit[:, self.dist_bins[scale_idx]:2 * self.dist_bins[scale_idx]]


            conf_cls_logit =  conf_cls_head(local_content_feat)

            interval_conf_logits_scale = conf_cls_logit[:, 0]
            interval_cls_logits_scale =  conf_cls_logit[:, 1:]
...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, heads=2):
        ...
        self.gat = GATConv(in_channels, out_channels, heads=heads, concat=False, edge_dim= 12 )#16
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.node_edge_proj =  nn.Sequential( nn.Linear(adjusted_in_channels, 128),
        ...
        edge_feature_dim = 12 # if you change this , the GAT layer,edge_dim=2 should also be changed ;
        self.edge_encoder = nn.Linear(32*2, edge_feature_dim) # *2, due to  combine two edge_proj
