...
class EnhancedTimeAwareFDR(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=3,
        ...
        # Thresholds (non-learnable, manually adjusted)
        self.register_buffer('normal_threshold', torch.tensor(0.5))  # Fixed
        self.register_buffer('exist_thresh', torch.tensor(0.7))  # Fixed
        self.register_buffer('abnormal_thresh', torch.tensor(0.5))  # Adjusted by rule
    ...
    def forward(self, node_embeddings, time_positions, node_pred, node_vad, duration_ratio=None):
        """
        Args:
            node_embeddings: Tensor of shape (num_nodes, node_embedding_dim)
            time_positions: Tensor of shape (num_nodes, 1) with normalized time in [0, 1]
            batch_indices: Tensor (num_nodes,) indicating audio membership (unused here)
            node_pred: Tensor of shape (num_nodes, num_classes) raw node prediction scores.
            node_vad: Tensor of shape (num_nodes, 2) logits from node VAD (abnormal vs. normal).
            audio_durations: Optional tensor with audio durations (if using absolute time).
        Returns:
            dict with:
              "final_bounds": Tensor (num_intervals, 2) with refined [start, end] boundaries.
              "interval_mask": Bool Tensor (num_intervals,) marking abnormal intervals (True) vs. normal.
              "num_intervals": Number of candidate intervals.
        """
        device = node_embeddings.device
        num_nodes = node_embeddings.size(0)

        # 1. Duration-aware time embedding.
        # 1. Time embedding with precomputed duration ratio
        if duration_ratio is not None:
            # Expand scalar ratio to match time_positions shape
        ...
        # 2. Learnable smoothing.
        # Reshape node_pred from (num_nodes, num_classes) to (1, num_classes, num_nodes) for conv1d.
        smoothed_pred = F.softmax(self.smoothing_conv(node_pred.T).T, dim=-1)  # shape: (num_nodes, num_classes)
        normal_probs = smoothed_pred[:, self.normal_class_idx]
        abnormal_scores = 1 - normal_probs

        # 3. Global normality check based on node counts.
        # normal_mask = (smoothed_pred.argmax(dim=1) == self.normal_class_idx)
        # ratio_normal = normal_mask.float().mean()
        ratio_normal = normal_probs.mean()
        if ratio_normal > self.normal_threshold:
            return self._empty_output(device)

        # 4. Global feature aggregation (learnable instead of .mean).
        # For abnormal scores.
        abnormal_seq = abnormal_scores.unsqueeze(0).unsqueeze(-1)  # shape: (1, num_nodes, 1)
        _, h_abnormal = self.global_abnormal_rnn(abnormal_seq)  # h_abnormal: (1, 1, 1)
        global_abnormal_scalar = h_abnormal.squeeze()  # scalar
        # For node_vad abnormal probability.
        vad_probs = F.softmax(node_vad, dim=-1)  # (num_nodes, 2)
        vad_abnormal = vad_probs[:, 1]
        vad_seq = vad_abnormal.unsqueeze(0).unsqueeze(-1)  # shape: (1, num_nodes, 1)
        _, h_vad = self.global_vad_rnn(vad_seq)  # h_vad: (1, 1, 1)
        global_vad_scalar = h_vad.squeeze()  # scalar

        # Combine into a global abnormal feature vector.
        ...
        ], dim=-1)  # shape: (max_intervals, 2)

        #self.method_name()
        # Concatenate each learnable query with the global abnormal feature.
        query_scores = torch.matmul(self.interval_queries, node_embeddings.transpose(0,1))
        attn_weights = F.softmax(query_scores, dim=-1)
        query_features = torch.matmul(attn_weights,x)  # (max_intervals, node_embedding_dim)


        interval_gen_input = torch.cat([query_features, global_abnormal_feature],  dim=-1)  # (max_intervals, node_embedding_dim+2)

        interval_params = self.interval_generator(interval_gen_input)  # (max_intervals, 4)
        centers = torch.sigmoid(interval_params[:, 0])  # in [0, 1]
        widths = torch.sigmoid(interval_params[:, 1]) * 0.5  # restrict width (max = 0.5)
        existence = torch.sigmoid(interval_params[:, 2])
        interval_type = torch.sigmoid(interval_params[:, 3])  # predicted probability of abnormal type

        # Compute initial boundaries.
        starts = (centers - widths / 2).clamp(0, 1)
        ends = (centers + widths / 2).clamp(0, 1)

        # Filter candidates by existence threshold.
        #self.exist_thresh = 0.5   # 有效区间，用于判断该区间是否有意义的存在，即该区间只要是正常区间或者是异常区间都是可以的，
        valid_mask = existence > self.exist_thresh
        valid_indices = valid_mask.nonzero(as_tuple=False).squeeze(-1)
        if valid_indices.numel() > self.max_intervals:
            topk = torch.topk(existence, self.max_intervals).indices
            valid_indices = topk
        if valid_indices.numel() == 0:
            valid_indices = torch.tensor([0], device=device)
        candidate_starts = starts[valid_indices]
        candidate_ends = ends[valid_indices]
        candidate_interval_type = interval_type[valid_indices]  # used for final interval type

        # 6. Iterative boundary refinement with learnable local aggregation.
        refined_bounds = []
        time_pos_1d = time_positions.squeeze(-1)
        refined_types = []  # store predicted interval type (0 normal, 1 abnormal)

        intermediate_distributions = []
        final_distributions = []
        # initialize storage for each candidate， for self-distillation
        for i in range(valid_indices.numel()):
            ...
            # Use a learnable aggregator (GRU) for local node features.
            in_interval = (time_pos_1d >= start) & (time_pos_1d <= end)
            if in_interval.sum() > 0:
                local_x = x[in_interval].unsqueeze(0)  # shape: (1, n_interval, node_embedding_dim),  获取当前存在于当前区间的节点特征
                _, h_local = self.local_feat_rnn(local_x)  # h_local: (1, 1, node_embedding_dim)
                local_feat = h_local.squeeze(0).squeeze(0)  # shape: (node_embedding_dim)
                # Similarly, learn to aggregate local abnormal scores.
                local_abnormal_seq = ((abnormal_scores + vad_abnormal) / 2)[in_interval].unsqueeze(0).unsqueeze(-1)
                _, h_local_abnormal = self.local_abnormal_rnn(local_abnormal_seq)  # shape: (1, 1, 1)
                local_abnormal_feature = h_local_abnormal.squeeze()  # scalar
            else:
                # If no nodes fall in the interval, use learned defaults (zeros).
                local_feat = torch.zeros(self.node_embedding_dim, device=device)
                local_abnormal_feature = torch.zeros(1, device=device)

            candidate_layer_outputs = []  # to store intermediate outputs for candidate i
            # Refine boundaries using the refinement layers.
            for layer in self.refinement_layers:
                # Query: concatenate local feature, center, width, start, end, and local abnormal feature.
                query = torch.cat([
                    local_feat,
                    center.view(1),
                    width.view(1),
                ...
                logits = layer(query)  # shape: (2 * dist_bins,)
                start_logits, end_logits = logits.chunk(2, dim=-1)
                # Save the raw logits for distillation
            ...
            refined_bounds.append(torch.stack([start, end]))  #    这里只负责区分出正常区间还是异常区间，至于具体的异常区间的类型，则后续通过节点的预测来判读；
            refined_types.append(1 if candidate_interval_type[i] > self.abnormal_thresh else 0) # 区分正常和异常区间， 后续在统计检测误差时，只对异常区间的边界误差进行计算；

        final_bounds = torch.stack(refined_bounds) if refined_bounds else torch.empty((0, 2), device=device)
        # Force at least one abnormal interval if needed
        if sum(refined_types) == 0 and len(refined_types) > 0:
            best_idx = torch.argmax(candidate_interval_type).item()
            refined_types[best_idx] = 1

        # Create an interval mask: True for abnormal intervals, False for normal intervals.
        interval_mask = torch.tensor([flag == 1 for flag in refined_types], device=device, dtype=torch.bool)

        return {
            "final_bounds": final_bounds,
            "interval_mask": interval_mask,
            "num_intervals": final_bounds.size(0),
            "final_distribution": final_distributions,
            # Last layer output for each candidate (if needed, stack across candidates)
            "intermediate_distributions": intermediate_distributions,
        }


    # def method_name(self, candidate_interval_type, i, refined_types):
    #     refined_types.append(
    #         1 if candidate_interval_type[i] > self.abnormal_thresh else 0)  # 区分正常和异常区间， 后续在统计检测误差时，只对异常区间的边界误差进行计算；
...
class Node_Edge_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, front_vad_logit):
        ...
        for i in range(num_audios):
            ...
            num_candidate_interval = event_output["num_intervals"]
            final_distribution = event_output["final_distribution"]   #candidate_layer_outputs[-1],
            # Last layer output for each candidate (if needed, stack across candidates)
            intermediate_distributions = event_output["intermediate_distributions"]

            cur_pred_interval_num = event_output["interval_mask"].sum().item()

            interval_mask = self.method_name(event_output)
            pred_abnormal_interval = final_bounds[interval_mask]  # (num_true_intervals, 2)

            distill_loss = 0.0
            for candidate in range(num_candidate_interval):
                teacher_start, teacher_end = final_distribution[candidate]  # final predictions for this candidate
                for layer_output in intermediate_distributions[candidate]:
                    student_start, student_end = layer_output
                    ...
                    loss_end = F.kl_div(F.log_softmax(student_end, dim=-1),
                                        ...
                                        reduction='batchmean')
                    distill_loss += (loss_start + loss_end)


            node_predictions_list.append(node_preds)
            node_vad_pred_list.append(fusion_vad_pred)
            all_node_labels.append(cur_node_labels)
            bt_pred_interval_num.append(cur_pred_interval_num)


            if len(pred_abnormal_interval) == 0:
                event_bounds_list.append([])
            else:
                event_bounds_list.append(pred_abnormal_interval)
            bt_distill_loss.append(distill_loss)
        ...
        if bt_distill_loss:
            total_distill_loss = sum(bt_distill_loss) / len(bt_distill_loss)
        else:
        ...
        return [node_predictions,  edge_predictions, node_vad_pred_batch, event_bounds_list, total_distill_loss, bt_pred_interval_num ]

    # def method_name(self):
    #     pass
    #     # Precompute duration ratio for all audios
...
class GraphRespiratory(nn.Module):
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            pred_intervals_num = pred_output[5]
            ...
            outputs = {
                ...
                'pred_intervals_num': pred_intervals_num,
