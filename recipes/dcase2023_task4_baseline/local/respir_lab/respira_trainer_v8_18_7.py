...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.classification_params = list(self.sed_net.node_interval_pred.interval_refine.interval_class_heads.parameters())
        ...
        # Debugging
        print(f"Confidence params count: {len(self.confidence_params)}")
        print(f"Classification params count: {len(self.classification_params)}")
        print(f"Shared params count: {len(self.shared_params)}")
        ...
        #  [Crackle, Wheeze,  Stridor, Rhonchi,]  should  align with the interval_4labels;
    ...
    def training_step(self, batch_data, batch_indx ):
        ...
        # print("conf_preds_cat shape:", conf_preds_cat.shape)
        # print("cls_preds_cat shape:", cls_preds_cat.shape)
        # print("conf_targets_cat shape:", conf_targets_cat.shape)
        # print("cls_targets_cat shape:", cls_targets_cat.shape)
        # print("cls_targets_cat dtype:", cls_targets_cat.dtype)
        # print("cls_targets_cat unique:", torch.unique(cls_targets_cat))


        # # Dynamic Loss Reweighting for interval_cls_loss
        # Adjust loss weights based on epoch
        current_epoch = self.current_epoch
        if current_epoch < 80:  # Phase 1: Confidence focus
            self.interval_conf_loss_weight = 40.0  # High weight
            self.interval_cls_loss_weight = 5.0  # Reduced weight
        elif current_epoch < 140:  # Phase 2: Transition
            transition_factor = (current_epoch - 80) / 60.0
            # self.interval_conf_loss_weight = 40.0 - transition_factor * 35.0  # From 40.0 to 5.0
            self.interval_cls_loss_weight = 5.0 + transition_factor * 45.0  # From 5.0 to 50.0
        else:  # Phase 3: Classification focus
            # self.interval_conf_loss_weight = 5.0  # Low weight
            self.interval_cls_loss_weight = 50.0  # High weight

        conf_loss            = self.interval_conf_loss_weight * conf_loss
        # interval_cls_loss    = dynamic_weight * self.interval_cls_loss_weight  * interval_cls_loss
        ...
        # Log losses
        self.log("train/node_level_loss", node_level_loss,    prog_bar=True,on_step=True, on_epoch=True)
        self.log("train/interval_conf_loss", conf_loss,       prog_bar=True,on_step=True, on_epoch=True)
        self.log("train/interval_cls_loss", interval_cls_loss,prog_bar=True,on_step=True, on_epoch=True)
        self.log("train/interval_loc_loss", loc_loss,         prog_bar=True,on_step=True, on_epoch=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        try:
            ...
            node_event_segment_scored0 = compute_event_based_metrics(
                self.val_buffer_intervals[cur_thread],
                self.hparams["data"]["valid_tsv"],
                save_dir=os.path.join(self.hparams["log_dir"], "conf0_interval_event_metrics"),
            )
        ...
        node_event_thread_f_score = max(node_event_segment_scored0[0],
                                        node_event_segment_scored1[0],node_event_segment_scored2[0])
    ...
    def configure_optimizers(self):
        ...
        lr_confidence_initial = 1e-3
        lr_classification_initial = 1e-5
        lr_shared_initial = 1e-3

        optimizer = torch.optim.Adam([
            {'params': self.confidence_params, 'lr': lr_confidence_initial, 'name': 'confidence'},
            {'params': self.classification_params, 'lr': lr_classification_initial, 'name': 'classification'},
            {'params': self.shared_params, 'lr': lr_shared_initial, 'name': 'shared'}
        ])

        # Learning rate schedule function
        def lr_lambda(epoch, group_name):
            if epoch < 80:  # Phase 1: Confidence focus
                return {
                    'confidence': 1.0,  # 1e-3
                    'classification': 1.0,  # 1e-5
                    'shared': 1.0  # 1e-3
                }[group_name]
            elif epoch < 140:  # Phase 2: Transition
                transition_factor = (epoch - 80) / 60.0
                return {
                    'confidence': 1.0 - transition_factor * 0.9,  # From 1e-3 to 1e-4
                    'classification': 1.0 + transition_factor * 99.0,  # From 1e-5 to 1e-3
                    'shared': 1.0  # Constant 1e-3
                }[group_name]
            else:  # Phase 3: Classification focus
                return {
                    'confidence': 0.1,  # 1e-4
                    'classification': 100.0,  # 1e-3
                    'shared': 1.0  # 1e-3
                }[group_name]

        # Create a list of lambda functions, one for each parameter group
        lr_lambdas = [
            lambda epoch, name=group['name']: lr_lambda(epoch, name)
    ...
    # def configure_optimizers(self):
    #     # Initial learning rates as requested
    #     lr_confidence_initial = 1e-3
    #     lr_classification_initial = 1e-5  # 1e-5
    #     lr_shared_initial = 1e-3
    #
    #     optimizer = torch.optim.Adam([
    #         {'params': self.confidence_params, 'lr': lr_confidence_initial, 'name': 'confidence'},
    ...
    #                 'shared': 1.0  # 1e-3
    #             }
    #         elif epoch < 120:  # Phase 2: Transition
    #             transition_factor = (epoch - 80) / 20.0
    #             return {
    #                 'confidence': 1.0 - transition_factor * 0.99,  # From 1.0 (1e-3) to 0.01 (1e-5)
    #                 'classification': 1.0 + transition_factor * 99.0,  # From 1e-5 to 1e-3
    #                 'shared': 1.0  # Constant 1e-3
    #             }
    #         else:  # Phase 3: Classification focus
    #             return {
    #                 'confidence': 0.01,  # 1e-5 (1e-3 * 0.01)
    #                 'classification': 100.0,  # 1e-3 (1e-5 * 100)
    #                 'shared': 1.0  # 1e-3
    #             }
    #
    #     scheduler = torch.optim.lr_scheduler.LambdaLR(
    #         optimizer,
               # note, this will only use the confidence lr for all 3 groups parameters
    #         lr_lambda=lambda epoch: lr_lambda(epoch)[optimizer.param_groups[0]['name']]
    #     )
    #     return [optimizer], [{'scheduler': scheduler, 'interval': 'epoch', 'frequency': 1}]
    ...
    def on_train_epoch_start(self):
        ...
        lr_confidence = optimizer.param_groups[0]['lr']  # confidence
        lr_classification = optimizer.param_groups[1]['lr']  # classification
        lr_shared = optimizer.param_groups[2]['lr']  # shared

        print(f"Epoch {self.current_epoch}: lr_confidence={lr_confidence}, "
              f"lr_classification={lr_classification}, lr_shared={lr_shared}")

        self.log('lr_confidence', lr_confidence, on_epoch=True)
        self.log('lr_classification', lr_classification, on_epoch=True)
