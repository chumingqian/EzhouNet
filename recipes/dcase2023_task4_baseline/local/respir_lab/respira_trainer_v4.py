...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        # output from the collate fn
        # batch_data = {
        #     'chunks': all_chunks,  # List of tensors
        #     'node_labels': all_node_labels,  # List of tensors
        #     'batch_indices': batch_indices,  # List of ints
    ...
    def custom_collate(self, batch):
        """
        In the custom_collate function, we'll:

        Collect all chunks and node labels from each sample.
        从每个样本中收集所有块和节点标签。

        Flatten the chunks and node labels into a single list.
        将块和节点标签展平为单个列表。

        Maintain a batch_indices list to keep track of
        which sample each chunk belongs to.
        维护一个batch_indices列表来跟踪每个块属于哪个样本。

        This reduces the need for padding and
        allows for more efficient batching.
        Args:
            batch:

        Returns:
            Return a batch_data dictionary containing all necessary information.
            返回包含所有必要信息的batch_data字典。
        """
        ...
        for i, sample in enumerate(batch):
            chunks = sample['chunks']  # List of tensors
            node_labels = sample['node_labels']  # List of tensors
            num_chunks = len(chunks)

            all_chunks.extend(chunks)
            all_node_labels.extend(node_labels)
            batch_indices.extend([i] * num_chunks)
            c_ex_mixtures.append(sample['c_ex_mixture'])
