...
class EnhancedTimeAwareFDR(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=3,
        ...
        for k, scale in enumerate(scales):
            n_k = num_centers[k]  # Number of centers for this scale
            # Centers evenly spaced from near 0 to near 1
            centers_k = (torch.arange(n_k) + 0.5) / n_k
            # Repeat the scale for each center
            scales_k = scale * torch.ones(n_k)
            # Form intervals as [center, scale] pairs
            intervals_k = torch.stack([centers_k, scales_k], dim=1)  # Shape (n_k, 2)
            anchor_intervals_list.append(intervals_k)

        # Combine all intervals into a single tensor
        anchor_intervals = torch.cat(anchor_intervals_list, dim=0)
    ...
    def forward(
       ...
       # print(f"the number of current sample's  prior anchor intervals : {self.num_anchor_intervals} ")

        for i in range(self.num_anchor_intervals):
            ...
            if in_interval.sum() > 0:
                ...
                local_abnormal_seq = abnormal_scores[in_interval].unsqueeze(0).unsqueeze(-1)
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.node_interval_pred = Node_Edge_cls_Module(adjusted_in_channels, hidden_channels, out_channels, node_num_classes=num_classes)
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            pred_output = self.node_interval_pred(batch_graph, audio_durations )
