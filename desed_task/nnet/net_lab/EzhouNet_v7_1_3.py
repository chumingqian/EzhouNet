...
# v7-1-2,   vad 只用于生成生成真实的边的标签， 不用于合成到节点特征 以及 依靠生成边的特征；
# 边的特征，通过两个相邻的节点来生成；
...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, heads=2):
        ...
        self.gat = GATConv(in_channels, out_channels,
                           ...
                           edge_dim=3,
                           add_self_loops= False # Prevent automatic addition of self-loops
                           )

    def forward(self, x, edge_index, edge_attr):
        out, attn_weights = self.gat(x, edge_index, edge_attr=edge_attr, return_attention_weights=True)
        #print(f"GATLayer forward: attn_weights type: {type(attn_weights)}")

        # Check if attn_weights is a tuple
        if isinstance(attn_weights, tuple):
            #print(f"attn_weights has {len(attn_weights)} elements.")
            # for idx, attn in enumerate(attn_weights):
            #     print(f"attn_weights[{idx}] shape: {attn.shape}")

            # Extract the normalized attention weights (assuming it's the second tensor)
            attn_weights = attn_weights[1]
            #print(f"GATLayer forward: Extracted attn_weights shape: {attn_weights.shape}")

        else:
            print(f"GATLayer forward: attn_weights shape: {attn_weights.shape}")
...
class Node_Edge_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, node_num_classes, hidden_dim=32,edge_num_classes=2):
        super(Node_Edge_cls_Module, self).__init__()
        self.gat_model = GATModel(in_channels, hidden_channels, out_channels)

        # Define projection layers for edge embeddings
        # Option 1: Use attention weights to scale node embeddings before combining
        self.edge_embedding_proj = nn.Linear(in_channels, hidden_dim)
        # Edge classification head
        self.edge_classifier = nn.Linear(hidden_dim, edge_num_classes)


        # Option 2: Use a separate attention mechanism (if needed)
        # self.edge_attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=4, batch_first=True)
    ...
    def forward(self, data):
        ...
        for i in range(num_audios):
            ...
            # Debugging: Inspect attn_weights
            # print(f"Type of attn_weights: {type(attn_weights)}")
            # print(f"Number of elements in attn_weights: {len(attn_weights)}")
            #
            # for idx, weight in enumerate(attn_weights):
            #     print(f"Type of attn_weights[{idx}]: {type(weight)}")
            #     if isinstance(weight, torch.Tensor):
            #         print(f"Shape of attn_weights[{idx}]: {weight.shape}")
            #     else:
            #         print(f"attn_weights[{idx}] is not a tensor.")
            ...
            if cur_graph.edge_index.size(1) > 0:
                ...
                x_src = node_embeddings[src]  # (E, hidden_dim)
                x_dst = node_embeddings[dst]  # (E, hidden_dim)

                # Average attention weights across heads
                attn_weights_mean = attn_weights.mean(dim=1, keepdim=True)  # (E, 1)

                # Expand attention weights to match embedding dimensions
                attn_weights_expanded = attn_weights_mean.expand(-1, x_src.size(1))  # (E, hidden_dim)

                # Weighted node embeddings
                weighted_x_src = x_src * attn_weights_expanded  # (E, hidden_dim)
                weighted_x_dst = x_dst * attn_weights_expanded  # (E, hidden_dim)
                combined_edge_embeddings = torch.cat([weighted_x_src, weighted_x_dst], dim=1)  # (E, 2 * hidden_dim)


                # Project combined embeddings to hidden_dim
                edge_embeddings = self.edge_embedding_proj(combined_edge_embeddings)  # (E, hidden_dim)
                edge_embeddings = F.relu(edge_embeddings)  # Non-linearity

                # Alternatively, you can explore other combination methods:
                # - Element-wise multiplication: x_src * x_dst
                # - Difference: x_src - x_dst
                # - Concatenation with other features or interactions
                edge_pred = self.edge_classifier(edge_embeddings)  # (E, edge_num_classes)
    ...
    def flatten_attn_weights(self, attn_weights):
        """
        Recursively flattens a nested tuple/list of attention weights into a flat list of tensors.
        """
        if isinstance(attn_weights, (tuple, list)):
            flat = []
            for item in attn_weights:
                flat.extend(self.flatten_attn_weights(item))
            return flat
        elif isinstance(attn_weights, torch.Tensor):
            return [attn_weights]
        else:
            raise ValueError("attn_weights contains non-tensor elements")
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        num_additional_features = 0  # No extra features now
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                if num_nodes_sample > 1:
                    ...
                    combined_node_features = torch.cat([src_node_features, dst_node_features],
                                                       dim=1)  # [E, 2 * feature_dim] (191, 1024)
            ...
            outputs = {
                ...
                'edge_predictions': edge_predictions, # note, before it's wrong should be edge_predictions ;
