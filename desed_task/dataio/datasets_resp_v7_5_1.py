...
#  V7-5-1:  使用 blank 区域裁剪， 进行线性插值，然后进行归一化；
...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='reorganize audio info,' )):
            ...
            new_items = {"audio_data": audio, "labels_df": labels_df, }
            ...
            # Count each event type individually
    ...
    def __getitem__(self, idx):
        ...
        if self.train_flag and binary_label == 'Abnormal':
            ...
            if self.audio_aug and random.random() > 0.8:
            ...
            if self.audio_aug and random.random() > 0.5:
    ...
    def spec_extract_features(self, audio, binary_label):
        ...
        #plot_spectrogram(mel_spec, "Raw Mel Spectrogram")

        mel_spec = self.powerToDB(mel_spec)
        #plot_spectrogram(mel_spec, "Mel Spectrogram (Amplitude to dB)")

        # mel_norm = standardize_rows(mel_spec)   # note, 对语谱图分别在行维度上进行标准化；
        #plot_spectrogram(mel_norm, "Mel Spectrogram (Standardized Rows)")


        mel_normalization =  (mel_spec - mel_spec.min()) / (mel_spec.max() - mel_spec.min())
        #plot_spectrogram(mel_normalization, "Mel Spectrogram (Normalization)")

        mel_gray =  mel_normalization * 255
        # plot_spectrogram(mel_gray, "Mel Gray")

        # mel_gray[mel_gray < 80] = 0
        mel_gray = torch.clamp(mel_gray - 80, min=0)  # Soft threshold
        mel_revert = torch.flip(mel_gray, dims=[0])

        for row in range(mel_revert.shape[0]):
            black_percent = len(np.where(mel_revert[row,:]==0)[0])/len(mel_revert[row,:])
            if black_percent < 0.80:
                break

        clip_mel = mel_revert[row:, :]
        # revise_clip_mel = torch.flip(clip_mel, dims=[0])
        # plot_spectrogram(revise_clip_mel, "Mel Spectrogram (Clip thread)")

        # Assuming `clip_mel` is a PyTorch tensor and has shape (C, H, W)
        # Where C = number of channels, H = height, W = width (or time dimension in your case)
        # Assuming clip_mel is a 2D tensor, shape: (H, W)
        # Ensure it has a channel dimension (C=1 in this case)
        clip_mel = clip_mel.unsqueeze(0).unsqueeze(0)  # Shape becomes (1, 1, H, W)        # Resize the tensor using interpolate
        clip_mel_resized = F.interpolate(clip_mel, size=(self.n_filters , clip_mel.shape[3]),  mode='bilinear', align_corners=False)

        # mel_resize = cv2.resize(clip_mel, (clip_mel.shape[1], self.n_filters*3), interpolation=cv2.INTER_LINEAR)
        revise_mel_resize =   torch.flip(clip_mel_resized.squeeze(0).squeeze(0), dims=[0])
        # plot_spectrogram(revise_mel_resize, "Mel Resize (interpolation)")

        if (revise_mel_resize.max() - revise_mel_resize.min()) < 1e-8:
            print("\n Mel Handle edge case: add noise or skip the sample" )
            revise_mel_resize += torch.randn_like(revise_mel_resize) * 1e-6


        mel_resize_normalization = (revise_mel_resize - revise_mel_resize.min()) / ( revise_mel_resize.max() - revise_mel_resize.min())
        #plot_spectrogram(mel_resize_normalization, "mel Resize ( resize Normalization)")

        # Perform standardization (mean = 0, variance = 1)
        # mean = mel_resize_normalization.mean()
        # std = mel_resize_normalization.std()
        # standard_mel_spec = (mel_resize_normalization - mean) / ( std + 1e-8)  # Add epsilon to avoid division by zero
        # plot_spectrogram(standard_mel_spec, "mel Resize (Mean 0, Variance 1 Standardzation)")
        mel_norm = mel_resize_normalization.unsqueeze(0)  # (1, 84, 626)
        ...
        #plot_spectrogram(cur_cqt.squeeze(0), "Raw CQT Spectrogram")
        cur_cqt = self.amplitudeToDB(cur_cqt)
        # plot_spectrogram(cur_cqt.squeeze(0), "CQT Spectrogram (Amplitude to dB)")

        cur_cqt = cur_cqt.squeeze(0)
        # cqt_norm = standardize_rows(cur_cqt)
        # plot_spectrogram(cqt_norm, "CQT Spectrogram (Standardized Rows)")

        cqt_normalization =  (cur_cqt - cur_cqt.min()) / (cur_cqt.max() - cur_cqt.min())
        # plot_spectrogram(cqt_normalization, "Cqt Spectrogram (Normalization)")

        cqt_gray =  cqt_normalization * 255
        #plot_spectrogram(cqt_gray, "Cqt Gray")

        # cqt_gray[cqt_gray < 120] = 0
        cqt_gray = torch.clamp(cqt_gray - 120, min=0)  # Soft threshold
        cqt_revert = torch.flip(cqt_gray, dims=[0])

        for row in range(cqt_revert.shape[0]):
            black_percent = len(np.where(cqt_revert[row, :] == 0)[0]) / len(cqt_revert[row, :])
            if black_percent < 0.80:
                break

        clip_cqt = cqt_revert[row:, :]
        # revise_clip_cqt = torch.flip(clip_cqt, dims=[0])
        # plot_spectrogram(revise_clip_cqt, "cqt Spectrogram (Clip thread)")

        # Assuming `clip_cqt` is a PyTorch tensor and has shape (C, H, W)
        # Where C = number of channels, H = height, W = width (or time dimension in your case)
        # Assuming clip_cqt is a 2D tensor, shape: (H, W)
        # Ensure it has a channel dimension (C=1 in this case)
        clip_cqt = clip_cqt.unsqueeze(0).unsqueeze(0)  # Shape becomes (1, 1, H, W)        # Resize the tensor using interpolate
        clip_cqt_resized = F.interpolate(clip_cqt, size=(self.n_filters, clip_cqt.shape[3]),  mode='bilinear', align_corners=False)

        # cqt_resize = cv2.resize(clip_cqt, (clip_cqt.shape[1], self.n_filters*3), interpolation=cv2.INTER_LINEAR)
        revise_cqt_resize = torch.flip(clip_cqt_resized.squeeze(0).squeeze(0), dims=[0])
        # plot_spectrogram(revise_cqt_resize, "cqt Resize (interpolation)")

        if (revise_cqt_resize.max() - revise_cqt_resize.min()) < 1e-8:
            print("\n cqt Handle edge case: add noise or skip the sample" )
            revise_cqt_resize += torch.randn_like(revise_cqt_resize) * 1e-6


        cqt_resize_normalization = (revise_cqt_resize - revise_cqt_resize.min()) / (revise_cqt_resize.max() - revise_cqt_resize.min())
        # plot_spectrogram(cqt_resize_normalization, "cqt Resize ( resize Normalization)")


        # Perform standardization (mean = 0, variance = 1)
        # mean = cqt_resize_normalization.mean()
        # std = cqt_resize_normalization.std()
        # standard_cqt_spec = (cqt_resize_normalization - mean) / ( std + 1e-8)  # Add epsilon to avoid division by zero
        # plot_spectrogram(standard_cqt_spec, "cqt Resize (Mean 0, Variance 1 Standardzation)")

        cqt_norm = cqt_resize_normalization.unsqueeze(0)

        # ------------------------ gamma spec---------------
        cur_gamma = self.gamma_spec(audio)  # (1, n_filters, frames= 626)
        #plot_spectrogram(cur_gamma.squeeze(0), "Raw Gamma Spectrogram")

        cur_gamma = self.powerToDB(cur_gamma)
        # plot_spectrogram(cur_gamma.squeeze(0), "Gamma Spectrogram (Power to dB)")

        # cur_gamma_ap = self.amplitudeToDB(cur_gamma)
        # plot_spectrogram(cur_gamma_ap.squeeze(0), "Gamma Spectrogram (Ampliteude to dB)")


        cur_gamma = cur_gamma.squeeze(0)
        # gamma_norm = standardize_rows(cur_gamma)
        # plot_spectrogram(gamma_norm, "Gamma Spectrogram (Standardized Rows)")

       
        gamma_normalization =  (cur_gamma - cur_gamma.min()) / (cur_gamma.max() - cur_gamma.min())
        # plot_spectrogram(gamma_normalization, "gamma Spectrogram (Normalization)")

        gamma_gray =  gamma_normalization * 255
        # plot_spectrogram(gamma_gray, "gamma Gray")

        # gamma_gray[gamma_gray < 160] = 0
        gamma_gray = torch.clamp(gamma_gray - 160, min=0)  # Soft threshold
        gamma_revert = torch.flip(gamma_gray, dims=[0])

        for row in range(gamma_revert.shape[0]):
            black_percent = len(np.where(gamma_revert[row, :] == 0)[0]) / len(gamma_revert[row, :])
            if black_percent < 0.80:
                break

        clip_gamma = gamma_revert[row:, :]
        # revise_clip_gamma = torch.flip(clip_gamma, dims=[0])
        # plot_spectrogram(revise_clip_gamma, "gamma Spectrogram (Clip thread)")

        # Assuming `clip_gamma` is a PyTorch tensor and has shape (C, H, W)
        # Where C = number of channels, H = height, W = width (or time dimension in your case)
        # Assuming clip_gamma is a 2D tensor, shape: (H, W)
        # Ensure it has a channel dimension (C=1 in this case)
        clip_gamma = clip_gamma.unsqueeze(0).unsqueeze(0)  # Shape becomes (1, 1, H, W)        # Resize the tensor using interpolate
        clip_gamma_resized = F.interpolate(clip_gamma, size=(self.n_filters, clip_gamma.shape[3]),  mode='bilinear', align_corners=False)

        # gamma_resize = cv2.resize(clip_gamma, (clip_gamma.shape[1], self.n_filters*3), interpolation=cv2.INTER_LINEAR)
        revise_gamma_resize = torch.flip(clip_gamma_resized.squeeze(0).squeeze(0), dims=[0])
        # plot_spectrogram(revise_gamma_resize, "gamma Resize (interpolation)")
       
        if (revise_gamma_resize.max() - revise_gamma_resize.min()) < 1e-8:
            print("\n gamma Handle edge case: add noise or skip the sample" )
            revise_gamma_resize += torch.randn_like(revise_gamma_resize) * 1e-6


       
        gamma_resize_normalization =  (revise_gamma_resize - revise_gamma_resize.min()) / (revise_gamma_resize.max() - revise_gamma_resize.min())
        # plot_spectrogram(gamma_resize_normalization, "gamma Resize ( resize Normalization)")


        # Perform standardization (mean = 0, variance = 1)
        # mean = gamma_resize_normalization.mean()
        # std = gamma_resize_normalization.std()
        # standard_gamma_spec = (gamma_resize_normalization - mean) / ( std + 1e-8)  # Add epsilon to avoid division by zero
        # plot_spectrogram(standard_gamma_spec, "Gamma Resize (Mean 0, Variance 1 Standardzation)")
        gamma_norm = gamma_resize_normalization.unsqueeze(0)
