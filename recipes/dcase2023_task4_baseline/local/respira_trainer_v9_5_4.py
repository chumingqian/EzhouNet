...
##  v9-1-1,  数据集：修改先验区间的持续时间为 ， 并且保证尺度2的个数最多，
...
class DebugCallback(Callback):
    """Callback to debug requires_grad status in callback hooks."""
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        print("Callback: After on_train_batch_end, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
        if not all(p.requires_grad for p in pl_module.sed_net.parameters()):
            print("Warning: Parameters frozen in callback on_train_batch_end")
            pl_module.prevent_ema_swap()

    def on_validation_epoch_end(self, trainer, pl_module):
        print("Callback: After on_validation_epoch_end, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
        if not all(p.requires_grad for p in pl_module.sed_net.parameters()):
            print("Warning: Parameters frozen in callback on_validation_epoch_end")
            pl_module.prevent_ema_swap()

    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        print("Callback: After on_save_checkpoint, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
        if not all(p.requires_grad for p in pl_module.sed_net.parameters()):
            print("Warning: Parameters frozen in callback on_save_checkpoint")
            pl_module.prevent_ema_swap()

    def on_train_epoch_end(self, trainer, pl_module):
        print("Callback: After on_train_epoch_end, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
        if not all(p.requires_grad for p in pl_module.sed_net.parameters()):
            print("Warning: Parameters frozen in callback on_train_epoch_end")
            pl_module.prevent_ema_swap()


    def on_validation_start(self, trainer, pl_module):
        print("Callback: After on_validation_start, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
        if not all(p.requires_grad for p in pl_module.sed_net.parameters()):
            print("Warning: Parameters frozen in callback on_validation_start")
            pl_module.prevent_ema_swap()
...
class CustomModelCheckpoint(ModelCheckpoint):
    """Custom ModelCheckpoint to ensure sed_net weights are saved and requires_grad preserved."""
    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        checkpoint["state_dict"] = pl_module.sed_net.state_dict()
        checkpoint["ema_state_dict"] = pl_module.ema_model.state_dict()
        print("CustomModelCheckpoint: Saving sed_net and ema_model state_dicts")
        super().on_save_checkpoint(trainer, pl_module, checkpoint)
        for param in pl_module.sed_net.parameters():
            param.requires_grad = True
        print("CustomModelCheckpoint: After saving checkpoint, sed_net requires_grad:",
              all(p.requires_grad for p in pl_module.sed_net.parameters()))
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        # print("sed_net initialized")
        # for name, param in self.sed_net.named_parameters():
        #     print(f"{name}: {param.shape}")


        # Ensure all parameters in sed_net are trainable
        for param in self.sed_net.parameters():
            param.requires_grad = True
        ...
        self.interval_conf_loss_weight =  5.0
    ...
    def update_ema(self, alpha, global_step):
        """Update the EMA model parameters without affecting the main model's requires_grad."""

        #decay = self.ema_decay
        decay = alpha
        with torch.no_grad():  # Ensure no gradients are computed
            for (name, param), (ema_name, ema_param) in zip(self.sed_net.named_parameters(),
                                                            self.ema_model.named_parameters()):
                assert name == ema_name, f"Parameter mismatch: {name} vs {ema_name}"
                # Update EMA parameters only, preserving requires_grad of sed_net
                ema_param.data.mul_(decay).add_((1 - decay) * param.data)
        # Verify sed_net parameters are unchanged
        for param in self.sed_net.parameters():
            assert param.requires_grad, f"Parameter {param} has requires_grad=False after EMA update"
    ...
    #def training_step(self, batch_data, batch_idx):
        ...
        print("training_step: Entering Lightning wrapper, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))

        print("training_step: Start, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        ...
        if frozen_params:
            ...
            # Group frozen parameters by module
            module_frozen = {}
            for name, p in frozen_params:
                # Extract module name (e.g., 'node_fea_generator', 'gat_model')
                module_name = name.split('.')[0] if '.' in name else 'root'
                if module_name not in module_frozen:
                    module_frozen[module_name] = []
                module_frozen[module_name].append(name)

            # Print frozen parameters by module
            for module_name, param_names in module_frozen.items():
                print(f"Frozen module: {module_name}, Parameters: {len(param_names)}")
        ...
        self.sed_net.train()
        print("sed_net training mode:", self.sed_net.training)
        print("node_fea_generator training mode:", self.sed_net.node_fea_generator.training)
        print("training_step: Before forward pass, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))

        # Forward pass
        outputs_detection = self.sed_net(batch_data_device, level="node")

        print("training_step: After forward pass, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        ...
        assert node_predictions.grad_fn is not None, "node_predictions lacks grad_fn after forward pass"
        ...
        # print(f"Weighted node_level_loss grad_fn: {node_level_loss.grad_fn is not None}")
       ...
       # print(f"total_loss grad_fn: {total_loss.grad_fn is not None}")
        ...
        node_conf_op_epochs = 60
        s1_transition_epochs = 40
        node_stage_epoch = node_conf_op_epochs + s1_transition_epochs

        s2_transition_epochs = 60
        interval_conf_op_epochs = 160
        s3_transition_epochs = 80
        ...
        # joint  optimize the total loss  : 240 epoch   ~
        else:
            total_loss = node_level_loss + node_conf_loss + conf_loss + interval_cls_loss + loc_loss
    ...
    def on_before_zero_grad(self, *args, **kwargs) :
        ...
        #     self.global_step
        # )
        # print("After update_ema, sed_net requires_grad:",
        #       all(p.requires_grad for p in self.sed_net.parameters()))
        print("\n EMA disabled, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        print("After on_train_batch_end, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        if not all(p.requires_grad for p in self.sed_net.parameters()):
            print("Warning: Parameters frozen in on_train_batch_end")
            self.prevent_ema_swap()

    def on_train_batch_start(self, batch, batch_idx):
        print("Before training_step, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        if not all(p.requires_grad for p in self.sed_net.parameters()):
            print("Warning: Parameters frozen before training_step")
            self.prevent_ema_swap()


    def on_save_checkpoint(self, checkpoint):
        checkpoint["sed_net"] = self.sed_net.state_dict()
        ...
        print("on_save_checkpoint: Saving sed_net and ema_model state_dicts")
        for param in self.sed_net.parameters():
            param.requires_grad = True
        print("on_save_checkpoint: After saving, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))

    def on_load_checkpoint(self, checkpoint):
        self.sed_net.load_state_dict(checkpoint["sed_net"])
        self.ema_model.load_state_dict(checkpoint["ema_model"])
        if "best_model" in checkpoint:
            self.best_model_weights = checkpoint["best_model"]
        if "best_ema_model" in checkpoint:
            self.best_ema_weights = checkpoint["best_ema_model"]
        for param in self.sed_net.parameters():
            param.requires_grad = True
        print("on_load_checkpoint: After loading, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))

    def prevent_ema_swap(self):
        for (name, param), (ema_name, ema_param) in zip(self.sed_net.named_parameters(),
                                                        self.ema_model.named_parameters()):
            if not param.requires_grad:
                print(f"Warning: {name} has requires_grad=False, resetting")
                param.requires_grad = True

    def on_after_backward(self):
        """Called after backward pass."""
        print("After on_after_backward, sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        if not all(p.requires_grad for p in self.sed_net.parameters()):
            print("Warning: Parameters frozen in on_after_backward")
            self.prevent_ema_swap()

    # def on_before_optimizer_step(self, optimizer, opt_idx):
    #     """Called before optimizer step."""
    #     print("After on_before_optimizer_step, sed_net requires_grad:",
    #           all(p.requires_grad for p in self.sed_net.parameters()))
    #     if not all(p.requires_grad for p in self.sed_net.parameters()):
    #         print("Warning: Parameters frozen in on_before_optimizer_step")
    #         self.prevent_ema_swap()
    # # Ensure no weight swapping from ema_model to sed_net

    def on_before_optimizer_step(self, optimizer, opt_idx):
        print("Before optimizer step: sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
        super().on_before_optimizer_step(optimizer, opt_idx)
        print("After optimizer step: sed_net requires_grad:",
              all(p.requires_grad for p in self.sed_net.parameters()))
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        s2_transition_epochs = 40
        node_conf_op_epochs = 160
        if self.current_epoch < node_conf_op_epochs:
            total_loss = node_conf_loss + conf_loss
        elif self.current_epoch < node_conf_op_epochs + s2_transition_epochs:
            alpha = (self.current_epoch - node_conf_op_epochs) / s2_transition_epochs
            total_loss = (node_conf_loss + conf_loss) + alpha * (node_level_loss + interval_cls_loss + loc_loss)
        else:
            total_loss = node_level_loss + node_conf_loss + conf_loss + interval_cls_loss + loc_loss
    ...
    def configure_optimizers(self):
        ...
        lr_node_initial = 1e-3
        lr_interval_initial = 1e-3

        # Combine shared_params and node_conf_cls_head_params
        # node_update_conf_cls_param       = self.shared_params + self.node_update_conf_cls_params
        node_update_conf_cls_param = (self.shared_params +
                                      self.node_update_conf_cls_params
                                      # + list(self.sed_net.node_fea_generator.parameters())
                                      )


        interval_conf_cls_refine_params  = self.interval_conf_cls_refine_params

        # Create optimizers for each parameter group
        optimizer_node_update_conf_cls      = torch.optim.Adam(node_update_conf_cls_param, lr=lr_node_initial)
        optimizer_interval_conf_cls_refine  = torch.optim.Adam(interval_conf_cls_refine_params, lr=lr_interval_initial)

        # Scheduler for node parameters:
        # Exponential decay with minimum learning rate for  shared and node param

        #  Note, should  not limit the  min  lr, it destory the  performance;
        # Scheduler for node parameters: MultiStepLR with stepped decay
        scheduler_node_update_conf_cls = {
            'scheduler': torch.optim.lr_scheduler.MultiStepLR(
                optimizer_node_update_conf_cls,
                milestones=[100,  160, 200],  # Adjust based on your training length and needs
                gamma=0.2  # Reduce lr by 10x at each milestone
            ),
            'interval': 'epoch',
            'frequency': 1,
            'name': 'node_cls_scheduler'
        }


        # Scheduler for interval parameters: Cosine Annealing
        scheduler_interval_conf_cls_refine = {
            'scheduler': torch.optim.lr_scheduler.MultiStepLR(
                optimizer_interval_conf_cls_refine,
                milestones=[160, 240, 300],  # Adjust based on your training length and needs
                gamma=0.2  # Reduce lr by 10x at each milestone
                #eta_min= 2e-4,  # set min lr  for  interval
            ),
            'interval': 'epoch',
            'frequency': 1,
            'name': 'interval_tasks_scheduler'
        }

        # Print parameter shapes for debugging
        # for opt in [optimizer_node_update_conf_cls, optimizer_interval_conf_cls_refine]:
        #     for param_group in opt.param_groups:
        #         for param in param_group['params']:
        #             print(f"Param shape: {param.shape}, requires_grad: {param.requires_grad}")

        # Debug: Print parameter shapes and requires_grad status
        for opt_name, opt in [
            ("node_update_conf_cls", optimizer_node_update_conf_cls),
            ("interval_conf_cls_refine", optimizer_interval_conf_cls_refine)
        ]:
        ...
        return [optimizer_node_update_conf_cls, optimizer_interval_conf_cls_refine], [scheduler_node_update_conf_cls, scheduler_interval_conf_cls_refine]

    # def configure_optimizers(self):
    #     optimizer = torch.optim.Adam(self.sed_net.parameters(), lr=1e-3)
    #     scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[80, 120, 250], gamma=0.2)
    #     print("Total params in optimizer:", sum(p.numel() for p in optimizer.param_groups[0]["params"]))
    #     return {"optimizer": optimizer,
    #             "lr_scheduler": {"scheduler": scheduler, "interval": "epoch", "name": "scheduler"}}
    #
    ...
    def on_train_epoch_start(self):
        ...
        # lr_node_interval_conf_cls = optimizer[0].param_groups[0]['lr']  # confidence
        # lr_interval_offset = optimizer[1].param_groups[0]['lr']  # classification
        ...
        #       f"lr_interval_off={lr_interval_offset}, ")
        #
        # self.log('lr_node_interval_conf_cls', lr_node_interval_conf_cls, on_epoch=True)
        # self.log('lr_interval_offset', lr_interval_offset, on_epoch=True)
        # self.log('lr_shared', lr_shared, on_epoch=True)
