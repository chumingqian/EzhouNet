...
class Basic_slide_conv_scale20(nn.Module):
    def __init__(self, embed_dim):
        ...
        # Replace the linear_proj with:
        # self.linear_proj = nn.Sequential(
        #     nn.LazyLinear(embed_dim),  # This will infer the input size
        #     nn.BatchNorm1d(embed_dim),
        #     nn.ReLU(True),
        # )
...
class Interval_Refine(nn.Module):
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            abnormal_scores = smoothed_pred[:, self.normal_class_idx] # here node abnormal scores = node confidence;
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.node_fea_generator = Basic_slide_conv_scale20(embed_dim= 512)
