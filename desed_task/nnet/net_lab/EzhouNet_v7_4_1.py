...
class Node_Edge_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, node_num_classes, edge_num_classes=2, subgraph_num_classes= 5):
        ...
        self.subgraph_classifier = nn.Linear(out_channels, subgraph_num_classes)  # Subgraph classifier
    ...
    def forward(self, data):
        ...
        for i in range(num_audios):
            ...
            ## Step 1: Group nodes into subgraphs ###
            # Use ground truth labels during training or predicted labels during inference
            # For the purpose of the forward method, we'll use node predictions to form subgraphs
            with torch.no_grad():
                node_pred_labels = node_pred.argmax(dim=1)


                # STEP 1: Obtain the raw predicted labels and apply smoothing
                # 2) Double-node smoothing (updated)
                idx = 0
                while idx < len(node_pred_labels) - 2:
                    left = node_pred_labels[idx]
                    mid1 = node_pred_labels[idx + 1]
                    mid2 = node_pred_labels[idx + 2]
                    if (idx + 3) < len(node_pred_labels):
                        right = node_pred_labels[idx + 3]
                        if left == right and mid1 != left and mid2 != left:
                            node_pred_labels[idx + 1] = left
                            node_pred_labels[idx + 2] = left
                            idx += 4
                            continue
                    idx += 1

                # Move smoothed labels back to device
                node_pred_labels = node_pred_labels.to(device)

                # Build a NetworkX graph to find connected components
                G = nx.Graph()
                G.add_nodes_from(range(cur_nodes.shape[0]))
                edges = cur_graph.edge_index.cpu().numpy().T
                G.add_edges_from(edges)

                # Assign node labels as attributes
                for idx in range(node_pred_labels.shape[0]):
                    G.nodes[idx]['label'] = node_pred_labels[idx].item()

                # Find connected components of nodes with the same label
                subgraphs = []
                for sub_nodes in nx.connected_components(G):
                    # Check if all nodes in sub_nodes have the same label
                    labels = set(G.nodes[node]['label'] for node in sub_nodes)
                    if len(labels) == 1:
                        subgraphs.append((sub_nodes, labels.pop()))
                    else:
                        # Split sub_nodes into groups of same label
                        label_to_nodes = {}
                        for node in sub_nodes:
                            label = G.nodes[node]['label']
                            label_to_nodes.setdefault(label, set()).add(node)
                        for label, nodes in label_to_nodes.items():
                            subgraphs.append((nodes, label))

            ### Step 2: Subgraph Classification ###
            for sub_nodes, subgraph_label in subgraphs:
                ...
                # Aggregate node embeddings to get subgraph embedding
                subgraph_embedding = subgraph_node_embeddings.mean(dim=0, keepdim=True)  # Shape: (1, embedding_dim)

                # Classify subgraph
                subgraph_prediction = self.subgraph_classifier(subgraph_embedding)  # Shape: (1, num_classes)
                subgraph_predictions_list.append(subgraph_prediction)
                all_subgraph_labels.append(torch.tensor([subgraph_label], dtype=torch.long, device=device))

                # Store indices of nodes in the subgraph for consistency loss
                subgraph_node_indices_list.append(sub_nodes_tensor)
                subgraph_node_predictions_list.append(node_pred[sub_nodes_tensor])
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        # Make edge_attr_val a learnable parameter:
        self.edge_base_attr = nn.Parameter(torch.tensor([0.0]))
        ...
        if level == "node":
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                if num_nodes_sample > 1:
                    ...
                    # Create edge attributes and labels using the learnable parameter
                    # edge_labels_list = []
                    # num_edges = edge_index.size(1)
                    #
                    # # If either connected node is abnormal, edge is abnormal
                    ...
                    #
                    # edge_labels = torch.tensor(edge_labels_list, dtype=torch.long, device=device)


                    # Generate edge labels based on node labels
                    src_labels = sample_node_labels[edge_index[0]]
                    dst_labels = sample_node_labels[edge_index[1]]

                    # Edge label is 1 if either connected node is abnormal (node label != 0)
                    edge_labels = torch.where((src_labels != 0) | (dst_labels != 0), 1, 0)
                    edge_labels = torch.tensor(edge_labels, dtype=torch.long, device=device)


                    # All edges share the same base attribute
                    # Expand the learnable parameter to match number of edges
                    edge_attr = self.edge_base_attr.expand(num_edges, 1)
                    # Encode edge attributes to learnable embeddings
                    edge_features = self.edge_encoder(edge_attr)  # 这里边的属性，是依据边的标签生成的， 为何不结合节点的特征生成呢，
                ...
                # print("Final node_features shape:", node_features.shape)
