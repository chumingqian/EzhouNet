...
# Since MultiStepLR and CosineAnnealingLR apply to all parameters,
# we need a custom scheduler to handle different groups
class CustomScheduler:
    def __init__(self, optimizer, scheduler_node, scheduler_interval):
        self.optimizer = optimizer
        self.scheduler_node = scheduler_node
        self.scheduler_interval = scheduler_interval

    def step(self):
        self.scheduler_node.step()  # Updates lr for node group
        self.scheduler_interval.step()  # Updates lr for interval group

        #  note,
        # Note, Explicitly set learning rates for each parameter group
        self.optimizer.param_groups[0]['lr'] = self.scheduler_node.get_last_lr()[0]
        self.optimizer.param_groups[1]['lr'] = self.scheduler_interval.get_last_lr()[0]


    def state_dict(self):
        return {
            "scheduler_node": self.scheduler_node.state_dict(),
            "scheduler_interval": self.scheduler_interval.state_dict()
        }

    def load_state_dict(self, state_dict):
        self.scheduler_node.load_state_dict(state_dict["scheduler_node"])
        self.scheduler_interval.load_state_dict(state_dict["scheduler_interval"])
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        #node_conf_loss = self.sed_net.node_conf_loss_fn(node_conf_logits, node_conf_targets)
    ...
    def configure_optimizers(self):
        ...
        # Schedulers for each parameter group
        scheduler_node = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[100, 160, 200], gamma=0.2
        )
        scheduler_interval = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[160, 240, 300], gamma=0.2
        )
        ...
        custom_scheduler = CustomScheduler(optimizer, scheduler_node, scheduler_interval)

        print("Total params in optimizer:",
              sum(p.numel() for p in optimizer.param_groups[0]["params"] + optimizer.param_groups[1]["params"]))
        print("Node group params:", sum(p.numel() for p in optimizer.param_groups[0]["params"]))
        print("Interval group params:", sum(p.numel() for p in optimizer.param_groups[1]["params"]))
    ...
    def on_train_epoch_start(self):
        ...
        # lr_interval = optimizer[1].param_groups[0]['lr']  # classification
