...
class RespiraSED_lab(pl.LightningModule):
    ...
    def validation_step(self, batch_data, batch_indx):
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        # 这三种声音事件检测指标而做准备。
        # note, batch_audio_names  需要更新成 batch detection names， 表示当前batch 中的音频文件名称；
        ...
        # scores_postprocessed_node_pred， batch 个音频， 每个音频中， 所有节点对应的起始，终止时间，以及他们对应分类的logit  数值；
        # decoded_node_pred, 每个阈值下，每份音频预测的事件的起始和终止时间，以及类别； decoded_node_pred, 只保留预测为异常类型的；
    ...
    def validation_epoch_end(self, outputs):
        ...
        try:
            ...
            node_event_segment_scored = compute_event_based_metrics(
                self.val_buffer_abnormal_node[cur_thread], #  这里传入的是当前阈值下，  验证集上，所有音频预测输出的声音事件的起始和终止时间， 以及对应的类型；
