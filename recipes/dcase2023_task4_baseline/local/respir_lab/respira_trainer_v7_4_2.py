...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        edge_cls_weight = torch.tensor([1.0, 8.0], dtype=torch.float, device='cuda')
        self.cls_edge_loss_fn = nn.CrossEntropyLoss(weight=edge_cls_weight)
        ...
        self.training_step_outputs = {
            ...
            'subgraph_cls_loss': [],
            'consis_node_edge_loss': [],
            'consis_sub_graph_node_loss': [],
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        #  subgrapch  class
        if outputs_detection['subgraph_labels'].size(0) > 0:
            subgraph_cls_loss = self.cls_node_loss_fn(outputs_detection['subgraph_predictions'], outputs_detection['subgraph_labels'])
        else:
            subgraph_cls_loss = torch.tensor(0.0, device=device)


        #  边与节点的一致性损失
        node_edge_consistency_loss, sub_graph_node_consistency_loss,  consistency_loss = self.compute_consistency_loss(node_pred_refine, edge_pred_refine, batch_graph_data,outputs_detection,
        ...
        # Total loss
        total_loss = node_level_loss + edge_level_loss + subgraph_cls_loss  +  consistency_loss

        # Log individual losses and total loss
        self.log("train/node_level_loss", node_level_loss)
        self.log("train/edge_level_loss", edge_level_loss)
        self.log("train/subgraph_cls_loss",  subgraph_cls_loss)

        self.log("train/consis_node_edge_loss", node_edge_consistency_loss)
        self.log("train/consis_subgraph_node_loss", sub_graph_node_consistency_loss)

        self.log("train/consis_loss", consistency_loss)
        ...
        self.training_step_outputs['subgraph_cls_loss'].append(subgraph_cls_loss)
        self.training_step_outputs['consis_node_edge_loss'].append(node_edge_consistency_loss)
        self.training_step_outputs['consis_sub_graph_node_loss'].append(sub_graph_node_consistency_loss)
    ...
    def training_epoch_end(self, outputs):
        ...
        if len(self.training_node_preds) > 0:
            ...
            print(f"Weighted Metrics: Precision: {weighted_precision:.4f}, Recall: {weighted_recall:.4f}, F1 Score: {weighted_f1:.4f}\n")
            ...
            if node_f1_score >= 0.85 and not self.adjustment_started:
        ...
        self.log("train/subgraph_cls_loss/epoch", epoch_metrics['subgraph_cls_loss'], on_epoch=True)
        self.log("train/consis_node_edge_loss/epoch", epoch_metrics['consis_node_edge_loss'], on_epoch=True)
        self.log("train/consis_sub_graph_node_loss/epoch", epoch_metrics['consis_sub_graph_node_loss'], on_epoch=True)
    ...
    def validation_step(self, batch_data, batch_indx):
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        node_edge_consistency_loss, sub_graph_node_consistency_loss,  self.consistency_loss = self.compute_consistency_loss(node_pred_refine, edge_pred_refine, batch_graph_data,
                                           outputs_detection, consistency_loss_weight=1.0)

        self.valid_total_loss =   (self.valid_node_loss   + self.valid_edge_loss
                                   + subgraph_cls_loss + self.consistency_loss)

        # Log individual losses and total loss
        self.log("valid/node_loss", self.valid_node_loss, prog_bar=True)
        self.log("valid/edge_loss", self.valid_edge_loss, prog_bar=True)
        self.log("valid/subgraph_cls_loss", subgraph_cls_loss, prog_bar=True)

        self.log("valid/consis_node_edge_loss", node_edge_consistency_loss, prog_bar=True)
        self.log("valid/consis_sub_graph_node_loss", sub_graph_node_consistency_loss, prog_bar=True)
    ...
    def configure_optimizers(self):
        ...
        node_learing_params = (
            ...
            list(self.sed_net.edge_mlp.parameters())
        )
    ...
    def compute_consistency_loss(self, node_logits_refined, edge_logits_refined, batch_graph, outputs_detection, consistency_loss_weight=1.0):
        device = node_logits_refined.device

        # Consistency Loss between Nodes and Edges
        # node_probs = F.softmax(node_logits_refined, dim=-1)  # Shape: (N, num_node_classes)
        # edge_probs = F.softmax(edge_logits_refined, dim=-1)  # Shape: (E, num_edge_classes)

        # Convert node logits to binary probabilities (Normal vs. Abnormal)
        node_probs = F.softmax(node_logits_refined, dim=-1)  # Shape: (N, 5)
        node_probs_binary = torch.stack([
            node_probs[:, 0],  # Normal
            1 - node_probs[:, 0]  # Abnormal
        ], dim=1)  # Shape: (N, 2)

        # Convert edge logits to probabilities
        edge_probs = F.softmax(edge_logits_refined, dim=-1)  # Shape: (E, 2)


        if batch_graph.edge_index.size(1) > 0:
            src, dst = batch_graph.edge_index

            # Node probabilities for source and destination nodes
            prob_node_normal_src = node_probs_binary[src, 0]  # Shape: (E,)
            prob_node_normal_dst = node_probs_binary[dst, 0]  # Shape: (E,)

            # Expected edge probabilities based on node probabilities
            # Normal Edge: Both nodes are normal
            expected_edge_normal = prob_node_normal_src * prob_node_normal_dst  # Shape: (E,)

            # Abnormal Edge: At least one node is abnormal
            expected_edge_abnormal = 1 - expected_edge_normal  # Shape: (E,)

            expected_edge_probs = torch.stack([
                expected_edge_normal,
                expected_edge_abnormal
            ], dim=1)  # Shape: (E, 2)

            # Clamp probabilities to avoid log(0)
            edge_probs = edge_probs.clamp(min=1e-9)
            expected_edge_probs = expected_edge_probs.clamp(min=1e-9)

            # Compute KL divergence
            edge_kl_loss = F.kl_div(
                edge_probs.log(),
                expected_edge_probs,
                reduction='batchmean'
            )
            node_edge_consistency_loss = edge_kl_loss * consistency_loss_weight

        else:
            node_edge_consistency_loss = torch.tensor(0.0, device=device)


        # 子图与节点的一致性损失；

            # Option 2: Mean Squared Error
            # mse_loss = F.mse_loss(node_preds, subgraph_pred_expanded)
            # consistency_loss += mse_loss

            # Option 3: Cross Entropy between node_preds and subgraph label
            # subgraph_label = outputs['subgraph_labels'][i]
            # node_consistency_loss = nn.CrossEntropyLoss()(node_preds, subgraph_label.expand(node_preds.size(0)))
            # consistency_loss += node_consistency_loss

        # Subgraph-Node Consistency Loss
        sub_graph_node_consistency_loss = 0.0
        for i in range(len(outputs_detection['subgraph_predictions'])):
            subgraph_logits = outputs_detection['subgraph_predictions'][i]  # Shape: (1, num_classes)
            node_logits_in_subgraph = outputs_detection['subgraph_node_predictions'][
                i]  # Shape: (num_nodes_in_subgraph, num_classes)

            if subgraph_logits.dim() == 1:
                subgraph_logits = subgraph_logits.unsqueeze(0)  # Shape: (1, num_classes)

            subgraph_prob = F.softmax(subgraph_logits, dim=1)
            node_probs_in_subgraph = F.softmax(node_logits_in_subgraph, dim=1)

            # Ensure probabilities are normalized
            subgraph_prob = subgraph_prob.clamp(min=1e-9)
            node_probs_in_subgraph = node_probs_in_subgraph.clamp(min=1e-9)

            # Compute KL divergence in the desired direction (e.g., KL(subgraph || node))
            kl_loss = F.kl_div(
                F.log_softmax(node_logits_in_subgraph, dim=1),  # log(p_node)
                subgraph_prob,  # q_subgraph
                reduction='batchmean'
            )
            sub_graph_node_consistency_loss += kl_loss

        # Average consistency loss over subgraphs
        if len(outputs_detection['subgraph_predictions']) > 0:
            sub_graph_node_consistency_loss /= len(outputs_detection['subgraph_predictions'])
        else:
            sub_graph_node_consistency_loss = torch.tensor(0.0, device=device)

        # toal consistency loss =  node_edge_consistency_loss  + sub_graph_node_consistency_loss
        consistency_loss  =  node_edge_consistency_loss + sub_graph_node_consistency_loss
        return node_edge_consistency_loss, sub_graph_node_consistency_loss,  consistency_loss
