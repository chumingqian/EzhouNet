            ...
            if self.audio_aug and random.random() > 0.5:
                ...
                if shift_samples > 0:  # Right shift (events move later)
                    if shift_samples >= audio_len:
                        audio_shifted = torch.zeros_like(audio)
                    else:
                        pad = torch.zeros(shift_samples, dtype=audio.dtype, device=audio.device)
                        audio_shifted = torch.cat([pad, audio[:-shift_samples]], dim=0)
                elif shift_samples < 0:  # Left shift (events move earlier)
                    k = -shift_samples
                    if k >= audio_len:
                        audio_shifted = torch.zeros_like(audio)
                    else:
                        pad = torch.zeros(k, dtype=audio.dtype, device=audio.device)
                        audio_shifted = torch.cat([audio[k:], pad], dim=0)
                else:
                ...
                for event in cur_audio_dict["events"]:
                    new_onset = max(0, min(event["onset"] + shift_seconds, audio_dur))
                    new_offset = max(0, min(event["offset"] + shift_seconds, audio_dur))
                ...
                # Update vad_timestamps
                updated_vad = []
                for vad in vad_timestamps:
                    new_start = max(0, min(vad["start"] + shift_seconds, audio_dur))
                    new_end = max(0, min(vad["end"] + shift_seconds, audio_dur))
                    if new_start < new_end:  # Keep only valid intervals
                        ...
                        updated_vad.append(updated_vad_entry)
                vad_timestamps = updated_vad
                cur_audio_dict["vad_timestamps"] = updated_vad

                # Update labels_df
                time_cols = ["onset", "offset"]  # Adjust if additional time columns exist
                for col in time_cols:
                    labels_df[col] = labels_df[col] + shift_seconds
                    labels_df[col] = labels_df[col].clip(lower=0, upper=audio_dur)
                labels_df = labels_df[labels_df["onset"] < labels_df["offset"]]  # Remove invalid rows
