...
#  v8-8,  直接使用 anchor + delta + offset logit; ( not work)

#  v8-9,  直接使用 anchor + offset logit;   并且此时， 去除所有其他非区间损失；
# 将节点类型相关的所有损失去除， 只保留区间类型的相关损失；


#  v8-10,  直接使用 anchor + offset logit;   + offset 类型的蒸馏损失；
# 将节点类型相关的所有损失去除， 只保留区间类型的相关损失；
...
class EnhancedTimeAwareFDR(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=3,
        ...
        # (Optional) You can keep an interval_generator for other outputs if needed, but not for center/width.
        ...
        for i in range(max_intervals):
            center = (i + 0.5) / max_intervals  # evenly spaced centers in [0,1]
            scale = scales[i % len(scales)]    # cycle through the three scales
            anchor_intervals.append([center, scale])
        anchor_intervals = torch.tensor(anchor_intervals)  # shape (max_intervals, 2)
        # Register as buffer so they are fixed.
        self.register_buffer('anchor_intervals', anchor_intervals)
        ...
        self.interval_conf_head = nn.Linear(node_embedding_dim , 1)
        ...
        self.interval_class_head = nn.Linear(node_embedding_dim , self.num_classes -1)
    ...
    def forward(
        ...
        # Instead of predicting delta offsets, we now directly use anchors.
        anchors = self.anchor_intervals.to(device)  # shape (max_intervals, 2): [center, width]
        pred_centers = anchors[:, 0]   # directly use anchor center
        pred_widths = anchors[:, 1]    # directly use anchor width

        # Compute candidate start and end boundaries from the anchors.
        starts = (pred_centers - pred_widths / 2).clamp(0, 1)
        ends = (pred_centers + pred_widths / 2).clamp(0, 1)
...
class Node_Edge_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur):
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.interval_pred = Node_Edge_cls_Module(adjusted_in_channels, hidden_channels, out_channels, node_num_classes=num_classes)
        ...
        if level == "node":
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                if num_nodes_sample > 1:
                    ...
                    dst_node_vad = sample_node_vad_fea[edge_index[1]]  # [E, feature_dim]
            ...
            pred_output = self.interval_pred(batch_graph, audio_durations)


            pred_intervals = pred_output[0]
            pred_interval_conf = pred_output[1]
            pred_interval_cls = pred_output[2]
            distill_loss = pred_output[3]
