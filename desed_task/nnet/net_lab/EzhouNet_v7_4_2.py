...
class Node_Edge_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, node_num_classes, edge_num_classes=2, subgraph_num_classes= 5):
       ...
       # Subgraph classifier: we'll produce a subgraph embedding of size subgraph_hidden
        subgraph_hidden = 128
        self.num_layers = 2  # we want a 2-layer GRU
        self.subgraph_gru = nn.GRU(
            input_size=out_channels,
            hidden_size=subgraph_hidden,
            num_layers=self.num_layers,
            batch_first=True
        )
        self.subgraph_classifier = nn.Linear(subgraph_hidden, subgraph_num_classes)
    ...
    def forward(self, data):
        ...
        for i in range(num_audios):
            ...
            # print(f"cur_graph.x shape: {cur_graph.x.shape}")
            # print(f"cur_graph.edge_index shape: {cur_graph.edge_index.shape}")
            # print(f"cur_graph.edge_attr shape: {cur_graph.edge_attr.shape}")
            ...
            # == 4) Edge classification + CRF smoothing
            ...
            if E > 0:
                ...
                # === CRF-based smoothing on edges ===
                # 1) Build adjacency among edges
                edge_adjacency = build_edge_adjacency_matrix(cur_graph.edge_index, device=device)

                # 2) Perform CRF inference
                #    Here, 'unary_potentials' = edge_logits (E, 2)
                smoothed_edge_probs = graph_crf_inference(
                    unary_potentials=edge_logits,  # shape: (E, 2)
                    adjacency_matrix=edge_adjacency,  # shape: (E, E)
                    num_iterations=5
                )  # (E, 2)

                # 3) Final predicted edge labels
                edge_pred_labels_smoothed = smoothed_edge_probs.argmax(dim=1).cpu().numpy()
            else:
            ...
            ## Step 5: Build subgraphs from edges predicted as "abnormal" (label=1)
            with torch.no_grad():

                # We also need node predictions if you want subgraph labels
                node_pred_probs = F.softmax(node_logits, dim=1)
                node_pred_labels = node_pred_probs.argmax(dim=1).cpu().numpy()

                # Build an Nx graph from abnormal edges
                G = nx.Graph()
                n_nodes = cur_nodes.shape[0]
                G.add_nodes_from(range(n_nodes))


                edges_np = cur_graph.edge_index.cpu().numpy().T  # shape (E, 2)
                # Mark edges predicted as abnormal => label=1
                abnormal_edge_indices = np.where(edge_pred_labels_smoothed == 1)[0]
                abnormal_edges = edges_np[abnormal_edge_indices]
                G.add_edges_from(abnormal_edges)

                if G.number_of_edges() == 0:
                    # No abnormal edges => each node is its own subgraph
                    # or you might treat the entire sample as one normal subgraph
                    subgraphs = [{n} for n in range(n_nodes)]
                else:
                    subgraphs = list(nx.connected_components(G))



            ## Step 6: Subgraph classification
            for sub_nodes in subgraphs:
                sub_nodes = list(sub_nodes)
                sub_nodes_tensor = torch.tensor(sub_nodes, dtype=torch.long, device=device)

                # Get subgraph embeddings => average of node embeddings
                subgraph_node_embeddings = node_embeddings[sub_nodes_tensor]
                # subgraph_embedding = subgraph_node_embeddings.mean(dim=0, keepdim=True)  # (1, out_channels)
                num_sub_nodes = subgraph_node_embeddings.size(0)

                # We'll treat this subgraph embedding as a single "sequence" for the GRU
                # But note: you can have multiple subgraphs; we want a batch dimension for them.
                # So we'll do something like:
                #   subgraph_node_embeddings => shape: [1, num_sub_nodes, out_channels]
                # "1" = batch size of 1 subgraph
                subgraph_node_embeddings = subgraph_node_embeddings.unsqueeze(0)

                # If you have multiple subgraphs in parallel, you can pack them all at once,
                # but in this snippet, let's do it subgraph by subgraph for clarity.

                # We'll create a length tensor: how many nodes are real (no padding)
                #  lengths = torch.tensor([num_sub_nodes], dtype=torch.long, device=device)
                lengths = torch.tensor([num_sub_nodes], dtype=torch.long, device="cpu")
                # Pack it
                packed_seq = pack_padded_sequence(
                    subgraph_node_embeddings,
                    lengths,
                    batch_first=True,
                ...
                _, hidden = self.subgraph_gru(packed_seq)

                # hidden is shape [num_layers, 1, subgraph_hidden], we can take the topmost layer
                # i.e. hidden[-1] => shape [1, subgraph_hidden]
                final_hidden = hidden[-1]  # shape: (1, subgraph_hidden)

                # Subgraph classification
                # final_hidden => shape [1, subgraph_hidden]
                subgraph_logits = self.subgraph_classifier(final_hidden)  # => [1, subgraph_num_classes]
                subgraph_predictions_list.append(subgraph_logits)

                # For a quick “label” for debugging, do majority vote of node_pred_labels
                node_labels_in_subgraph = node_pred_labels[sub_nodes_tensor.cpu().numpy()]
                if len(node_labels_in_subgraph) > 0:
                    labels, counts = np.unique(node_labels_in_subgraph, return_counts=True)
                    subgraph_label = labels[np.argmax(counts)]
                else:
                    subgraph_label = 0  # e.g. normal if no nodes?

                subgraph_label_tensor = torch.tensor([subgraph_label], dtype=torch.long, device=device)
                all_subgraph_labels.append(subgraph_label_tensor)

                # Store node indices & their logits (for consistency constraints, etc.)
        ...
        # Concatenate subgraph predictions and labels
        if subgraph_predictions_list:
            subgraph_predictions = torch.cat(subgraph_predictions_list, dim=0)
            subgraph_labels = torch.cat(all_subgraph_labels, dim=0)
        else:
            subgraph_predictions = torch.empty((0, self.subgraph_classifier.out_features), device=device)
            subgraph_labels = torch.empty((0,), dtype=torch.long, device=device)
