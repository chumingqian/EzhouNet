...
class EnhancedTimeAwareFDR(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=3,
        ...
        self.normal_threshold = 0.8 #nn.Parameter(torch.tensor(0.8)) # 该阈值用于判断当前音频是否为正常音频， 通过正常节点的占比率；
    ...
    def forward(self, node_embeddings, time_positions, batch_indices, node_pred, node_vad, duration_ratio=None):
        """
        Args:
        ...
        for i in range(valid_indices.numel()):
            ...
            refined_bounds.append(torch.stack([start, end]))
...
class Node_Edge_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, front_vad_logit):
        ...
        return [node_predictions,  edge_predictions, node_vad_pred_batch, event_bounds_list, total_distill_loss ]
