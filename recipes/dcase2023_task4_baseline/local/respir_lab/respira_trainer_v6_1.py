...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        if len(predicted_abnormal_indices) == 0:
            ...
            # Compute node-level loss only if there are correct detections
            if node_predictions is not None and node_labels is not None:
                # Map from sample indices to audio names
                sample_idx_to_audio_name = {idx: name for idx, name in enumerate(c_ex_mixtures_predicted_abnormal)}

                # Get indices of nodes corresponding to actual abnormal samples
                actual_abnormal_names_set = set(actual_abnormal_names)
                correct_detection_indices = [i for i, idx in enumerate(batch_indices.tolist())
                                             if sample_idx_to_audio_name[idx] in actual_abnormal_names_set]
                # 这里 sample_idx_to_audio_name[idx] in actual_abnormal_names_set，  只对预测为异常的样本，并且 真实值也为异常的样本，即TP 的情况；

                # Compute the node-level loss only for samples
                # that are both actually abnormal and predicted as abnormal.
                # note,  节点级别的分类损失， 仅计算实际异常和预测异常的样本的节点级损失。
                if len(correct_detection_indices) == 0:
                    # No correct detections
                    node_loss = torch.tensor(0.0, device=device)
                else:
                    # Extract the node_predictions and node_labels for correct detections
                    node_predictions_correct = node_predictions[correct_detection_indices]
                    node_labels_correct = node_labels[correct_detection_indices]

                    # Compute node-level loss
                    node_loss = self.supervised_loss(node_predictions_correct, node_labels_correct)
        ...
        if len(predicted_abnormal_indices) == 0:
            ...
            if node_predictions is not None and node_labels is not None:
                ...
                if len(correct_detection_indices) == 0:
                    ...
                    # Compute node-level loss
                    self.valid_node_loss = self.supervised_loss(node_predictions_correct, node_labels_correct)
    ...
    def validation_epoch_end(self, outputs):
        ...
        #     ground_truth,
        #     audio_durations,
        #     dtc_threshold= 0.15,  #0.7,
        #     gtc_threshold= 0.15,  #0.7,
        #     cttc_threshold=None,
        #     alpha_ct=0,
        #     alpha_st=1,
