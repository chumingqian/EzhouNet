...
# hf_lab1_5: 直接使用1层， without node  cls;
# 在原始的anchor interval 上， 使用1层 refine layer 直接学习 offset;
...
class Batch_Interval_Refine(nn.Module):
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):# [num_audios, num_intervals_scale, 2]
            ...
            expanded_ratio = torch.full_like(time_positions,duration_ratio) if (
                        duration_ratio is not None) else torch.ones_like(time_positions)

            time_feat = self.time_embed(torch.cat([time_positions, expanded_ratio], dim=-1))
            x = node_embeddings + time_feat  # [num_audios, num_nodes, emb_dim]
            ...
            abnormal_scores = 1 - smoothed_pred[:, :, self.normal_class_idx]  # [num_audios, num_nodes]

            # 3. Compute masks for this scale's intervals
            starts = anchor_intervals[:, :, 0].unsqueeze(2)  # [num_audios, num_intervals_scale, 1]
            ends = anchor_intervals[:, :, 1].unsqueeze(2)  # [num_audios, num_intervals_scale, 1]
            ...
            for i in range(num_audios):
                for j in range(num_intervals_scale):
                    mask = in_interval_mask[i, j, :]  # [num_nodes]
                    if mask.sum() > 0:
                        local_x = x[i, mask, :]
                        local_abnormal = abnormal_scores[i, mask].unsqueeze(-1)
                    else:
            ...
            local_abnormal_feature = h_local_abnormal.squeeze(0).squeeze(-1)[
                unsort_indices]  # [total_intervals]
            ...
            for layer in refinement_layers:
                query = torch.cat([
                    local_feat,
                    centers.unsqueeze(-1) / audio_len,
                    widths.unsqueeze(-1) / audio_len,
                    starts.unsqueeze(-1) / audio_len,
                    ends.unsqueeze(-1) / audio_len,
                    local_abnormal_feature.unsqueeze(-1)
                ], dim=-1)  # [num_audios, num_intervals_scale, hidden_dim + 5]

                logits = layer(query)  # [num_audios, num_intervals_scale, 2 * dist_bins]
            ...
            interval_conf_logits_scale = interval_conf_head(
                local_feat_full)  # [num_audios, num_intervals_scale, 1]
            interval_cls_logits_scale = interval_class_head(
                local_feat_full)  # [num_audios, num_intervals_scale, num_classes - 1]
...
class Node_Interval_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, anchor_intervals  ): # pass the batch data
        ...
        num_nodes_per_sample = node_embeddings.size(0) // num_audios
        ...
        # Uniform time positions for all samples (since duration is fixed)
        time_positions = torch.linspace(0, 1, num_nodes_per_sample, device=device).unsqueeze(-1)
        time_positions = time_positions.expand(num_audios, num_nodes_per_sample, 1)
        ...
        num_intervals_per_scale = [39, 19, 9]  # Example scales
        ...
        event_output = self.interval_refine(
            ...
            time_positions=time_positions,  # [num_audios, num_nodes, 1]
