...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='reorganize audio info' )):
            ...
            vad_timestamps = cur_dict.get("vad_timestamps", labels) # some example will be  empty;
            ...
            if len(gt_boxes) == 0 or gt_boxes.dim() < 2:
                ...
                for a_idx in range(len(anchor_boxes)):
                    ...
                    # if max_ious[a_idx] >= self.iou_threshold:
                        ...
                        # --- inside your assignment loop, when anchor a_idx matched gt_idx ---
                        anchor = anchor_boxes[a_idx]  # tensor([a_s, a_e]) in seconds
                        a_s, a_e = float(anchor[0]), float(anchor[1])
                        a_w = max(1e-6, a_e - a_s)
                        a_c = 0.5 * (a_s + a_e)

                        g_s, g_e = gt_intervals[gt_idx]
                        g_s, g_e = float(g_s), float(g_e)
                        g_w = max(1e-6, g_e - g_s)
                        g_c = 0.5 * (g_s + g_e)

                        t_c = (g_c - a_c) / a_w  # normalized center shift
                        t_w = math.log(g_w / a_w + 1e-12)  # log-width ratio
                        ...
                        if gt_label != "Normal":
                            ...
                            if max_iou >= threshold:
                                assignments.append({
                                    ...
                                    "box": gt_intervals[gt_idx],  # Ground truth box,  # for IoU auxiliary
                                    "t_c": t_c,
                                    "t_w": t_w
                                })
                            else:
    ...
    def __getitem__(self, idx, apply_general_aug= False):
        ...
        t_c = torch.tensor([a["t_c"] for a in assignments], dtype=torch.float)
        t_w = torch.tensor([a["t_w"] for a in assignments], dtype=torch.float)
