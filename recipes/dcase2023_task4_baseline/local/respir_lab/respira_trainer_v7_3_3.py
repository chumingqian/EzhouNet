...
# v7-2-1:   使用节点分类的  6 分类， 以及边分类的 2 分类；



# # v7-3-2:  边与节点的一致性损失更改为 KL 散度的形式，并且注意的两种类别上的维度匹配；
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        # Compute edge  loss
        edge_loss = self.cls_edge_loss_fn(edge_pred_refine, edge_labels)
        ...
        self.training_edge_preds.append(torch.argmax(edge_pred_refine, dim=1).cpu())
    ...
    def validation_step(self, batch_data, batch_indx):
        ...
        edge_pred_labels =  torch.argmax(edge_pred_refine, dim=1)  # Shape: (E,)
