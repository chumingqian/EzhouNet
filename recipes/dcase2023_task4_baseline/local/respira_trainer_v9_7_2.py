...
4.   区间的定位损失，替换为 smoth l1 loss( 消融实验)
5.   使用 AdamW 以及 余弦退火算法， 一起合作配合。
"""

# v9-7-2:
# 将区间的定位损失，IOU 损失替换为 smoth l1 loss;
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        # Define smooth L1 loss  for  the  interval  location  loss
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        triplets = self.generate_triplets(
            node_embeddings,
            node_type_labels,
            num_triplets_per_type=20,  # Start with 20 per type
        ...
        if fg_mask.sum() > 0:
            ...
            loc_loss = self.smooth_l1_loss(fg_pred_bounds, fg_box_targets)
        else:
    ...
    def configure_optimizers(self):
        ...
        lr_interval = 5e-3  # Lower learning rate for interval parameters
