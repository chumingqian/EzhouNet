...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        self.unify_heads =  nn.ModuleList([
            nn.Sequential(
                ...
                # outputs: conf(1) + cls(num_classes-1) + t_c(1) + t_w(1)
                nn.Linear(256, 1 + (num_classes - 1) + 2)
            )
        ...
        #     nn.Parameter(torch.linspace(-1.50, 1.50, dist_bins_list[i]))
        #     for i in range(self.num_scales)
        # ])
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            # Collect results
            final_bounds_scale = pred_bounds
            final_bounds_list.append(final_bounds_scale)
            interval_conf_logits_list.append(conf_logits)
            interval_cls_logits_list.append(cls_logits)

            t_c_pred_list.append(t_c_pred)
            t_w_pred_list.append(t_w_pred)
