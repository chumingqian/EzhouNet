...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.iou_threshold = 0.2    # change  for the  lab
        ...
        self.node_loss_weight_initial = 20.0
        ...
        self.node_vad_loss_weight = 2.0   # 时间属性


        self.interval_conf_loss_weight =  2.0
        self.interval_cls_loss_weight =   20.0
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        node_emb_vad_pred = outputs_detection['node_emb_vad']
        ...
        batch_audio_len  = batch_data_device["audio_dur"]
        pred_intervals = outputs_detection["pred_intervals"]


        distill_loss = outputs_detection['distill_loss']
        # Similarly, we assume your model now also returns:
        #  - "interval_conf_logits": List[Tensor], each shape (num_pred, 1)
        #  - "interval_cls_logits": List[Tensor], each shape (num_pred, num_abnormal_classes)
        interval_conf_logits_list = outputs_detection["pred_intervals_conf_logits"]  # length = batch_size
        interval_cls_logits_list = outputs_detection["pred_intervals_cls_logits"]  # length = batch_size
        ...
        all_conf_targets = []
        all_cls_targets = []
        all_box_targets = []  # for regression

        # 3. Build IoU-based GT (class & location) assignment for each sample in the batch
        for b_idx in range(batch_size):
            # 3.1. Prepare ground-truth intervals + sub-type labels
            gt_anno = vad_timestamps[b_idx]  # list of dicts: e.g. [{"start":..., "end":..., "event_label":...}, ...]
            gt_intervals = []
            gt_types = []  # store numeric IDs or "Normal"
            for anno in gt_anno:
                start_t = anno["start"]
                end_t = anno["end"]
                label_str = anno["event_label"]  # e.g. "Crackle"

                gt_intervals.append((start_t, end_t))
                gt_types.append(label_str)

            # 3.2. Predicted intervals for this sample
            preds_for_this_sample = pred_intervals[b_idx]  # shape (max_pred_interval_number, 2)
            conf_logits_this_sample = interval_conf_logits_list[b_idx]  # shape (num_pred, 1)
            cls_logits_this_sample = interval_cls_logits_list[b_idx]  # shape (num_pred, num_abnormal_classes)

            num_pred = preds_for_this_sample.size(0)

            # We'll create Tensors for confidence_targets and class_targets
            conf_targets = torch.zeros(num_pred, dtype=torch.float, device=device)  # 0 => normal, 1 => abnormal
            cls_targets = torch.full((num_pred,), -1, dtype=torch.long, device=device)  # -1 => ignore, else [0..3]
            box_targets = torch.zeros(num_pred, 2, dtype=torch.float, device=device)  # bounding-box GT if conf=1

            # 3.3. IoU-based matching
            for p_idx in range(num_pred):
                p_start, p_end = preds_for_this_sample[p_idx]
                best_iou = 0.0
                best_gt_idx = -1

                for g_idx, (g_start, g_end) in enumerate(gt_intervals):
                    iou_val = self.interval_iou((p_start.item(), p_end.item()), (g_start, g_end))
                    if iou_val > best_iou:
                        best_iou = iou_val
                        best_gt_idx = g_idx

                # If best IoU >= threshold => conf=1 => abnormal
                if best_iou >= self.iou_threshold and best_gt_idx != -1:
                    conf_targets[p_idx] = 1.0

                    # Convert the GT label to an abnormal sub-type ID
                    gt_label_str = gt_types[best_gt_idx]  # e.g. "Crackle"
                    # If you have "Normal" in your dataset, ensure those are separate or skip them here
                    # But presumably, your GT set doesn't have "Normal" intervals (?), or you do:
                    if gt_label_str == "Normal":
                        # Edge case if your data includes "Normal" intervals
                        # Usually we wouldn't have "Normal" in vad_timestamps, but let's handle it
                        conf_targets[p_idx] = 0.0  # override
                    else:
                        # It's an abnormal label => map to ID
                        class_id = interval_4labels[gt_label_str]  # e.g. 0..3
                        cls_targets[p_idx] = class_id

                    # bounding-box regression target
                    box_targets[p_idx, 0] = gt_intervals[best_gt_idx][0]
                    box_targets[p_idx, 1] = gt_intervals[best_gt_idx][1]

                else:
                    # conf=0 => normal => no sub-type => cls_targets=-1 => no box
                    conf_targets[p_idx] = 0.0
                    cls_targets[p_idx] = -1 # -1 => ignore 代表该区间的类型是正常的，不能参与异常分类损失
                    box_targets[p_idx] = 0.0 # 从而也代表该预测框也没有区间定位损失；

            # 3.4. Store in list for the entire batch
        ...
        # Example: Flatten
        conf_targets_cat = torch.cat(all_conf_targets, dim=0)  # shape (sum(num_pred),)
        cls_targets_cat = torch.cat(all_cls_targets, dim=0)  # shape (sum(num_pred),)
        box_targets_cat = torch.cat(all_box_targets, dim=0)  # shape (sum(num_pred), 2)
        ...
        for b_idx in range(batch_size):
            conf_preds_list.append(interval_conf_logits_list[b_idx].view(-1))  # shape (num_pred,)
            cls_preds_list.append(interval_cls_logits_list[b_idx])  # shape (num_pred, num_abnormal_classes)
            # If your bounding box predictions are also stored, gather them similarly
            # e.g. box_preds_list.append(...) => depends on how your model returns them

        conf_preds_cat = torch.cat(conf_preds_list, dim=0)  # shape (sum(num_pred),)
        cls_preds_cat = torch.cat(cls_preds_list, dim=0)  # shape (sum(num_pred), num_abnormal_classes)
        ...
        if fg_mask.sum() > 0:
            fg_cls_preds = cls_preds_cat[fg_mask]  # shape (num_fg, num_abnormal_classes)
            fg_cls_targets = cls_targets_cat[fg_mask]  # shape (num_fg,), note ,here need to check
            # interval_cls_loss = F.cross_entropy(fg_cls_preds, fg_cls_targets)
            interval_cls_loss = self.interval_cls_loss_fn(fg_cls_preds, fg_cls_targets)
        else:
            interval_cls_loss = torch.tensor(0.0, device=device)

        # 7. Localization Loss => only for intervals where conf=1
        #    Suppose your model returns "pred_bounds" in the same shape as "pred_intervals" => we gather them
        #    For demonstration, assume your "outputs_detection['pred_bounds']" is a list of Tensors:
        #        shape (num_pred, 2)
        #    So we can similarly gather them:
        pred_bounds_list = outputs_detection["pred_intervals"]  # List[Tensor]
        all_pred_bounds = torch.cat(pred_bounds_list, dim=0)  # shape (sum(num_pred),2)
        fg_pred_bounds = all_pred_bounds[fg_mask]
        fg_box_targets = box_targets_cat[fg_mask]

        if fg_mask.sum() > 0:
            #loc_loss = F.smooth_l1_loss(fg_pred_bounds, fg_box_targets)
        ...
        node_emb_vad_loss = self.vad_loss_fn(node_emb_vad_pred, node_vad_labels)  * self.node_vad_loss_weight
        ...
        self.training_node_vad_preds.append(torch.argmax(node_vad_pred, dim=1).cpu())
        self.training_node_vad_targets.append(node_vad_labels.cpu())
        ...
        # Total loss
        total_loss = (node_level_loss + edge_level_loss   +
                      node_vad_loss + node_emb_vad_loss  +
        ...
        self.log("train/node_emb_vad_loss", node_emb_vad_loss, prog_bar=True)
        ...
        # self.log("train/interval_distill_loss", distill_loss)
        ...
        self.training_step_outputs['node_emb_vad_loss'].append(node_emb_vad_loss)
    ...
    def training_epoch_end(self, outputs):
        ...
        if len(self.training_node_vad_preds) > 0:
            all_node_vad_preds = torch.cat(self.training_node_vad_preds)
            all_node_vad_targets = torch.cat(self.training_node_vad_targets)

            # Compute node_vad-level confusion matrix
            labels = list(bin_node_vad_labels.values())
            node_vad_cm = confusion_matrix(all_node_vad_targets.numpy(), all_node_vad_preds.numpy(), labels=labels)
            self.print_matrix_with_labels(node_vad_cm, bin_node_vad_labels,
                                          "node_vad-level training Confusion Matrix")

            # Compute ratio format of the confusion matrix
            with np.errstate(divide='ignore', invalid='ignore'):
                node_vad_cm_ratio = node_vad_cm.astype('float') / node_vad_cm.sum(axis=1)[:, np.newaxis]
                node_vad_cm_ratio = np.nan_to_num(node_vad_cm_ratio)  # Replace NaN with zero

            self.print_matrix_with_labels(node_vad_cm_ratio, bin_node_vad_labels,
                                          "node_vad-level training Confusion Matrix (Ratio Format)")

            # Compute weighted metrics
            weighted_precision = precision_score(all_node_vad_targets, all_node_vad_preds, average='weighted',
                                                 labels=labels, zero_division=0)
            weighted_recall = recall_score(all_node_vad_targets, all_node_vad_preds, average='weighted', labels=labels,
                                           zero_division=0)
            weighted_f1 = f1_score(all_node_vad_targets, all_node_vad_preds, average='weighted', labels=labels,
                                   zero_division=0)
            print(
                f"Weighted Metrics: Precision: {weighted_precision:.4f}, Recall: {weighted_recall:.4f}, F1 Score: {weighted_f1:.4f}\n")
        ...
        self.log("train/node_emb_vad_loss/epoch", epoch_metrics['node_emb_vad_loss'], on_epoch=True)
    ...
    def validation_step(self, batch_data, batch_indx):
        ...
        # Determine number of normal nodes to sample (e.g., equal to the number of abnormal nodes)
        num_abnormal_nodes = len(abnormal_node_indices)  #   每个batch 中正常节点的个数  最多为 64个；
        num_normal_nodes_to_sample = max(64, num_abnormal_nodes // 3)  # Adjust denominator based on desired ratio
        ...
        self.validation_node_vad_preds.append(torch.argmax(node_vad_pred, dim=1).cpu())
        self.validation_node_vad_targets.append(node_vad_labels.cpu())
        ...
        self.valid_node_emb_vad_loss = self.vad_loss_fn(node_emb_vad_pred, node_vad_labels)  * self.node_vad_loss_weight


        self.valid_total_loss =   (self.valid_node_loss   + self.valid_edge_loss
                                   +  self.valid_node_emb_vad_loss
        ...
        self.log("valid/node_emb_vad_loss", self.valid_node_emb_vad_loss, prog_bar=True)
        ...
        self.valid_step_outputs['node_emb_vad_loss'].append(self.valid_node_emb_vad_loss)
        ...
        # Now run batch inference
        # batch_results = self.inference_batch_interval(outputs_detection, conf_threshold=0.1, nms_iou_threshold=0.8)
        #
        # # 'batch_results' is a list of length B.
        # # For each sample b in the batch:
        # #   batch_results[b]["final_bounds"] => shape (K_b, 2)
        # #   batch_results[b]["final_conf"] => shape (K_b,)
        # #   batch_results[b]["final_subtypes"] => shape (K_b,)
        #
        # # You can now do anything you want with these final intervals,
        # # e.g., store them for evaluation, or compute metrics.
        #
        # # Example: print them
        # for b_idx, res in enumerate(batch_results):
        #     final_bounds = res["final_bounds"]
        #     final_conf = res["final_conf"]
        #     final_subtypes = res["final_subtypes"]
        #     # Possibly map subtypes from IDs to strings
        #     # e.g. {0:"Crackle",1:"Wheeze",2:"Stridor",3:"Rhonchi"}
        #     # ...
        #     # print or store them


        # Assume these are defined in your hyperparameters:
        # self.hparams["training"]["val_conf_thresholds"] = [0.80, 0.50, 0.30]
        # self.hparams["training"]["val_iou_NMS"] = [0.8, 0.50, 0.30]
    ...
    def validation_epoch_end(self, outputs):
        ...
        if len(self.validation_node_vad_preds) > 0:
            all_node_vad_preds = torch.cat(self.validation_node_vad_preds)
            all_node_vad_targets = torch.cat(self.validation_node_vad_targets)

            # Compute node_vad-level confusion matrix
            labels = list(bin_node_vad_labels.values())
            node_vad_cm = confusion_matrix(all_node_vad_targets.numpy(), all_node_vad_preds.numpy(), labels=labels)
            self.print_matrix_with_labels(node_vad_cm, bin_node_vad_labels, "node_vad-level Validation Confusion Matrix")
            ...
            self.print_matrix_with_labels(node_vad_cm_ratio, bin_node_vad_labels,
                                          "node_vad-level Validation Confusion Matrix (Ratio Format)")

            # Compute weighted metrics
            weighted_precision = precision_score(all_node_vad_targets, all_node_vad_preds, average='weighted', labels=labels, zero_division=0)
            weighted_recall = recall_score(all_node_vad_targets, all_node_vad_preds, average='weighted', labels=labels, zero_division=0)
            weighted_f1 = f1_score(all_node_vad_targets, all_node_vad_preds, average='weighted', labels=labels, zero_division=0)
            print(
                f"Weighted Metrics: Precision: {weighted_precision:.4f}, Recall: {weighted_recall:.4f}, F1 Score: {weighted_f1:.4f}\n")
        ...
        try:
            ...
            print(f"\n Using confidient & NMS thread-based to filter intervals, cur_conf_thread:{cur_thread}")
            ...
            print(f"\n Using confidient & NMS thread-based to filter intervals, cur_conf_thread:{cur_thread1}")
            ...
            print(f"\n Using confidient & NMS thread-based to filter intervals, cur_conf_thread:{cur_thread2}")
        ...
        self.log_counts(self.valid_epoch_edge_label_counts, "edge_label", "edge label")
        self.log_counts(self.valid_epoch_node_vad_label_counts, "node_vad", "Node vad label")
        ...
        self.log("valid/node_emb_vad_loss/epoch", epoch_metrics['node_emb_vad_loss'], on_epoch=True)
