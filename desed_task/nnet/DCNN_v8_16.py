...
class  DynamicFeatureExtractor(nn.Module):
        def __init__(self,
            ...
            def conv(i, normalization="batch", dropout=None, activ='relu'):
                ...
                # Ensure "same" padding
                padding = (kernel[i] - 1) // 2
                if DY_layers[i] == 1:
                    cnn.add_module(f"conv{i}",
                                   Dynamic_conv2d(in_dim, out_dim, kernel[i], stride=stride[i], padding=padding,
                                                  n_basis_kernels=n_basis_kernels, temperature=temperature,
                                                  pool_dim=pool_dim))
                else:
                    cnn.add_module(f"conv{i}", nn.Conv2d(in_dim, out_dim, kernel[i], stride=stride[i], padding=padding))
            ...
            if stage == "detect":
                # Adjust spec2node based on output shape (frames preserved)
                self.spec2node = nn.Sequential(
                    nn.Linear(n_filt[-1] * 84, node_fea_dim),  # Frames variable, flatten later
                    nn.LayerNorm(node_fea_dim),
                    nn.ReLU(True),
                )

        def forward(self, x):  # x: (batch, 3, frames, 84)
            x = self.dynamic_cnn(x)  # Output: (batch, n_filt[-1], frames, 84)
            # Flatten channels and freq, preserve frames
            x = x.view(x.size(0), x.size(2), -1)  # (batch, frames, n_filt[-1] * 84)
            node_fea = self.spec2node(x)  # (batch, frames, node_fea_dim)
            return node_fea
