...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.interval_conf_loss_fn = lambda inputs, targets: F.binary_cross_entropy_with_logits(inputs, targets,
                                                                                                reduction='mean')
        ...
        self.interval_distill_loss_weight = 50.00
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        conf_loss =  self.interval_conf_loss_fn(conf_preds_cat.squeeze(), conf_targets_cat)

        # 6. Sub-type classification => only for intervals where conf=1
        # Foreground mask
        #fg_mask = (conf_targets_cat == 1)
        # 2. Foreground mask for abnormal intervals
        # Select anchors where conf_targets_cat > 0 (i.e., assigned to abnormal intervals)
