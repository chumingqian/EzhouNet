...
from .utils import  batched_vad_node_decode_preds_v2,  compute_event_based_metrics, compute_event_based_metrics_ori
...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        alpha = [0.0214, 0.0935, 0.0261, 0.2236, 0.1673, 0.0128, 0.4609]
    ...
    def validation_step(self, batch_data, batch_indx):
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        scores_raw_node_pred, scores_postprocessed_node_pred, decoded_node_pred,decoded_abnormal_pred, = batched_vad_node_decode_preds_v2(
            node_predictions,
            filenames_synth,
            self.encoder,  #   note,  这里传入 异常类别的标签；
            batch_indices, #  此时的索引， 代表的是这些节点 位于当前预测为异常样本构成的 batch 中的索引；
            vad_timestamps = batch_audio_timestamp,
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': node_learing_params, 'lr': self.node_lr_initial, 'name': 'node_para'},
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            ...
            lr_lambda=[
               # lambda epoch: self.compute_lr_factor(epoch, 'classification'),
                lambda epoch: self.compute_lr_factor(epoch, 'node_para'),
