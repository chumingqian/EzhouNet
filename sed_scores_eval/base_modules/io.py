...
def parse_ground_truth(
    ...
    if not tagging:
        ...
        # this  will change the ground truth if neighbor two  event with same class  and  has  overlapping ,
        ground_truth =   adjust_overlapping_events(ground_truth)
...
def assert_non_connected_events(ground_truth):
    for clip_id, events in ground_truth.items():
        ...
        for event_class, class_events in per_class_events.items():
            ...
            class_events = sorted(class_events)
            previous_start_time = -1e6
            for event in class_events:
                # Ensure that the start time of the current event is greater than the start time of the previous event
                assert event[0] > previous_start_time, (
                    f'Events of type {event[2]} are not in order for clip {clip_id}: {class_events}'
                )
                previous_start_time = event[0]


from collections import defaultdict

def adjust_overlapping_events(ground_truth):
    adjustment_counts = defaultdict(int)  # To count adjustments per category
    for clip_id, events in ground_truth.items():
        per_class_events = defaultdict(list)
        for event in events:
            # event is a tuple: (start_time, end_time, event_class)
            per_class_events[event[2]].append(event)
        for event_class, class_events in per_class_events.items():
            # Sort events by their start times
            class_events.sort(key=lambda x: x[0])
            previous_end_time = -1e-6  # Initialize to a very small negative number
            for i, event in enumerate(class_events):
                start_time, end_time, event_label = event
                if start_time < previous_end_time:
                    # Adjust the start time to the end time of the previous event
                    adjustment_counts[event_class] += 1
                    new_start_time = previous_end_time
                    # Ensure the new start time is less than the end time
                    if new_start_time >= end_time:
                        # Adjust the end time to be slightly after the new start time
                        end_time = new_start_time + 1e-6
                        print(f"Warning: Adjusted end time for event {event} in clip {clip_id} to prevent zero or negative duration.")
                    # Update the event with the new start time
                    class_events[i] = (new_start_time, end_time, event_label)
                    start_time = new_start_time  # Update for the next iteration
                previous_end_time = end_time
            per_class_events[event_class] = class_events
        # Reconstruct the events list for the clip
        events = []
        for class_events in per_class_events.values():
            events.extend(class_events)
        ground_truth[clip_id] = events
    # Print out the adjustment counts
    for event_class, count in adjustment_counts.items():
        print(f"\n Adjusted overlapping events {count} times for category '{event_class}'.")
    return ground_truth
...
def read_ground_truth_events_w_NA(filepath):
    """read ground truth events from tsv file
    ...
    required_columns = ['filename', 'onset', 'offset', 'event_label', 'record_bin_label']
    if not all([name in list(file.columns) for name in required_columns]):
        raise ValueError(
            f'ground_truth events file must contain columns "filename", '
            f'"onset", "offset", "event_label", and "record_bin_label" but only columns '
            f'{list(file.columns)} were found.'
        )

    for filename, onset, offset, event_label, record_label in zip(
        file['filename'], file['onset'], file['offset'],
        file['event_label'], file['record_bin_label']
    ):
        example_id = filename.rsplit('.', maxsplit=1)[0]

        # Initialize with empty list for this file
        ground_truth[example_id] = []

        # Skip adding events if record_bin_label is Normal
        if record_label == 'Normal':
            continue

        # Only add events if record_bin_label is not Normal
        if isinstance(event_label, str):
            assert len(event_label) > 0
            ground_truth[example_id].append([
                float(onset), float(offset), event_label
