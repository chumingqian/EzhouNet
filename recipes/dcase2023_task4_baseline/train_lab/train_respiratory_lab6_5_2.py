...
from desed_task.dataio.datasets_resp_v6_1 import RespiraGnnSet
from desed_task.nnet.EzhouNet_v6_5_2 import GraphRespiratory
from local.respira_trainer_v6_5_2 import RespiraSED_lab
...
def calculate_sample_weights(dataset, class_weights):
    ...
    sample_weights = []
    for sample in dataset.examples.values():
        label = sample['record_bin_label']
        # Convert label string to index (Normal=0, Abnormal=1)
...
def single_run(
    ...
    if test_state_dict is None:
        ...
        binary_class_weights = calculate_binary_class_weights(train_dataset)
        device = next(sed_net.parameters()).device
        ...
        print(f" \n the sample weight length { len(sample_weights)}")
        batch_sampler = WeightedRandomSampler(
            weights=sample_weights,
            num_samples= len(sample_weights),  #config["training"]["batch_size"]
            replacement=True
        )
