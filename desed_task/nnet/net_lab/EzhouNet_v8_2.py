...
class FDR(nn.Module):
    """
    D-FINE style iterative refinement for sound event boundaries.
    Each refinement layer predicts a start/end time distribution
    and adjusts the boundary predicted by the prior layer.
    """
    def __init__(self,
                 num_classes,
                 in_channels=256,
        ...
        # ---- 1) Interval Count Predictor (Optional) ----
        self.interval_counter = nn.Sequential(
            nn.Linear(in_channels, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, max_intervals),
            nn.Softmax(dim=-1)
        )

        # ---- 2) Initial Boundary Prediction ----
        #   (No need to be super-accurate; just an initialization)
        self.initial_boundary = nn.Sequential(
            nn.Linear(in_channels, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, 2 * max_intervals)
        )

        # ---- 3) Multiple Refinement Layers for Start/End Distributions ----
        #
        # Instead of a single start_distribution / end_distribution,
        # create lists (or ModuleList) of small sub-networks.
        # Each layer i refines the boundary from layer i-1.
        #
        self.start_refine_layers = nn.ModuleList()
        self.end_refine_layers   = nn.ModuleList()
        for _ in range(num_refine_layers):
            self.start_refine_layers.append(
                nn.Sequential(
                    nn.Linear(in_channels, hidden_dim),
                    nn.ReLU(inplace=True),
                    nn.Linear(hidden_dim, dist_bins * max_intervals)
                )
            )
            self.end_refine_layers.append(
                nn.Sequential(
                    nn.Linear(in_channels, hidden_dim),
                    nn.ReLU(inplace=True),
                    nn.Linear(hidden_dim, dist_bins * max_intervals)
        ...
        # ---- 4) Learnable Weighting Function W(n) (D-FINE style) ----
        # This is used to map the predicted distribution into an offset.
        # We’ll keep the bins in [-0.5, 0.5], but let them be learnable.
        self.weight_params = nn.Parameter(
            torch.linspace(-0.5, 0.5, dist_bins),
            requires_grad=True
        )
    ...
    def forward(self, x):
        ...
        Returns a dictionary that includes:
          - all intermediate distributions (for self-distillation)
          - final refined boundaries
        """
        B = x.size(0)

        # (i) Interval count (if you want dynamic intervals)
        interval_probs = self.interval_counter(x)  # [B, max_intervals]
        # optional: you can interpret it with argmax or threshold
        num_intervals = interval_probs.argmax(dim=-1) + 1  # [B]

        # (ii) Initial boundary
        init_bounds = self.initial_boundary(x)  # [B, 2 * max_intervals]
        init_bounds = init_bounds.view(B, self.max_intervals, 2)

        # We will refine these boundaries across multiple layers
        current_start = init_bounds[..., 0:1]  # shape [B, max_intervals, 1]
        current_end   = init_bounds[..., 1:2]  # shape [B, max_intervals, 1]

        # Storage for all distributions at each layer (for knowledge distillation)
        all_start_distributions = []
        all_end_distributions   = []

        # ---- (iii) Iterative Refinement ----
        for layer_idx in range(self.num_refine_layers):
            # 1) Predict logit distributions for start/end from x
            start_logits = self.start_refine_layers[layer_idx](x)
            end_logits   = self.end_refine_layers[layer_idx](x)

            # reshape -> [B, max_intervals, dist_bins]
            start_logits = start_logits.view(B, self.max_intervals, self.dist_bins)
            end_logits   = end_logits.view(B, self.max_intervals, self.dist_bins)

            # 2) Convert to probability distributions
            start_dist = F.softmax(start_logits, dim=-1)  # [B, max_intervals, dist_bins]
            end_dist   = F.softmax(end_logits,   dim=-1)

            # store them for possible distillation
            all_start_distributions.append(start_dist)
            all_end_distributions.append(end_dist)

            # 3) Weighted-sum of bins => boundary offset
            start_offsets = torch.stack(
                [self.weighted_sum(d) for d in start_dist.unbind(dim=1)],
                dim=1  # -> shape [B, max_intervals, 1]
            )
            end_offsets = torch.stack(
                [self.weighted_sum(d) for d in end_dist.unbind(dim=1)],
                dim=1
            )

            # 4) Update boundaries
            #   This is a simple additive offset:
            refined_start = current_start + start_offsets
            refined_end   = current_end   + end_offsets

            # Optionally, you can clamp them in [0, T_max] for your time axis
            # refined_start = refined_start.clamp(min=0, max=some_value)
            # refined_end   = refined_end.clamp(min=0, max=some_value)

            # set them as the “current” boundary for the next iteration
            current_start = refined_start
            current_end   = refined_end

        # ---- (iv) Mask according to predicted number of intervals (optional) ----
        interval_mask = torch.arange(self.max_intervals, device=x.device)[None] < num_intervals[:, None]

        # final refined boundaries
        final_bounds = torch.cat([current_start, current_end], dim=-1)  # shape [B, max_intervals, 2]

        # Return everything needed
        return {
            'initial_bounds': init_bounds,             # (B, max_intervals, 2)
            'all_start_distributions': all_start_distributions,  # list of length num_refine_layers
            'all_end_distributions':   all_end_distributions,    # each item: (B, max_intervals, dist_bins)
            'final_bounds': final_bounds,               # (B, max_intervals, 2)
            'num_intervals': num_intervals,             # (B,)
            'interval_mask': interval_mask,             # (B, max_intervals) boolean
        }

    def weighted_sum(self, distribution):
        """
        Weighted-sum of distribution * learnable bin centers.
        distribution shape: [B, dist_bins],  but we unbind in forward(),
        so here shape: [dist_bins].
        """
        return (distribution * self.weight_params).sum(dim=-1, keepdim=True)

    # ---- (v) Example: Distillation Loss (KLD) ----
    def distillation_loss(self, all_start_dists, all_end_dists, T=1.0):
        """
        Example function you can call *outside* the forward pass to
        compute knowledge distillation from deeper distributions
        onto shallower ones. You might call this in your training loop.

        all_start_dists, all_end_dists:
          Lists of length = num_refine_layers,
          each entry is a probability distribution of shape [B, max_intervals, dist_bins].

        T: temperature factor for distillation (optional).
        """
        # Let the last layer's distribution be the “teacher”.
        start_teacher = all_start_dists[-1].detach()
        end_teacher   = all_end_dists[-1].detach()

        kd_loss = 0.0
        # Summation of KL over intermediate layers
        for i in range(self.num_refine_layers - 1):
            # shape [B, max_intervals, dist_bins]
            start_student = all_start_dists[i]
            end_student   = all_end_dists[i]

            # Optionally apply temperature scaling
            # Avoid log(0). Add small epsilon if needed
            kd_loss += self.kl_divergence(
                start_teacher, start_student, T
            )
            kd_loss += self.kl_divergence(
                end_teacher, end_student, T
            )
        return kd_loss

    @staticmethod
    def kl_divergence(p_teacher, p_student, temperature=1.0, eps=1e-8):
        """
        Standard KL-div with optional temperature scaling.
        p_teacher, p_student: [B, max_intervals, dist_bins]
        Return scalar loss (mean over batch).
        """
        # Adjust for temperature
        # p_teacher^(1/T) is typical in KD, but you can also do something simpler
        p_teacher = p_teacher**(1/temperature)
        p_teacher = p_teacher / (p_teacher.sum(dim=-1, keepdim=True) + eps)

        p_student = p_student**(1/temperature)
        p_student = p_student / (p_student.sum(dim=-1, keepdim=True) + eps)

        # kl_div wants log-prob for the first argument
        # kl_div(log(p_student), p_teacher) = sum( p_teacher * log(p_teacher/p_student) )
        # but we often do kl_div(log(p_student), p_teacher) in PyTorch reversed style,
        # so we can do the standard approach carefully:
        kl = F.kl_div(
            p_student.clamp(min=eps).log(),
            p_teacher,
            reduction='batchmean'
        )
        return kl
...
class DynamicTimestampGenerator(nn.Module):
    def __init__(self, input_dim, max_events=5, num_bins=100):
        super().__init__()
...
class Node_Edge_cls_Module(nn.Module):
    ...
    def forward(self, data):
        ...
        return node_predictions,  edge_predictions, node_vad_pred_batch
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.timestamp_generator = DynamicTimestampGenerator(node_fea_dim)
