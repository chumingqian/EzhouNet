...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        # Initialize counters for epoch accumulation
        self.train_true_abnormal = 0
        self.train_pred_abnormal = 0
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        batch_pred_intervals = outputs_detection["pred_intervals"]
        # Calculate true and predicted abnormal counts
        true_counts = self._get_true_abnormal_counts(batch_data_device)
        pred_counts = outputs_detection["pred_intervals_num"].sum().item()

        # Accumulate across batches
        self.train_true_abnormal += true_counts
        self.train_pred_abnormal += pred_counts
    ...
    def training_epoch_end(self, outputs):
        ...
        # Update threshold at end of epoch
        if self.train_true_abnormal > 0:
            hit_rate = self.train_pred_abnormal / self.train_true_abnormal
            self.sed_net.adjust_threshold(hit_rate, target=0.8)
            # self.model.adjust_threshold(hit_rate, target=0.8)
            self.log("train/hit_rate", hit_rate)
            self.log("train/abnormal_thresh", self.model.abnormal_thresh)
