...
# 直接作用于 anchor interval 进行学习；

# v8-15,  恢复 16 anchor interval 的方式，
# 且此时 refine layer 中直接使用的原始长度进行 offset refine;



# v8-15_2,  恢复 16 anchor interval 的方式，
# 且此时 refine layer 中直接使用归一化的长度进行 offset refine;
...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
                 dist_bins=100, kernel_size=5, max_intervals= 70 ):
        #super().__init__()
        ...
        self.num_scales = 1  # Three scales
    ...
    def forward(
        ...
        num_intervals = cur_anchor_intervals.size(0)



        # 1. Duration-aware time embedding
        expanded_ratio = torch.full_like(time_positions,duration_ratio) if duration_ratio is not None else torch.ones_like(time_positions)
        ...
        # 3. Compute masks for all intervals
        time_pos_1d = time_positions.squeeze(-1) * audio_len.item()
        centers = cur_anchor_intervals[:, 0].clone()
        widths = cur_anchor_intervals[:, 1].clone()


        # 5. Refine all intervals in parallel
        # Compute candidate start and end boundaries from the anchors.
        starts = (centers - widths / 2)
        ends = (centers + widths / 2)

        in_interval_mask = (time_pos_1d.unsqueeze(0) >= starts.unsqueeze(1)) & (
                    time_pos_1d.unsqueeze(0) <= ends.unsqueeze(1))  # (num_intervals, num_nodes)

        # 4. Extract local features for all intervals
        local_x_list = []
        ...
        for i in range(num_intervals):
            ...
            local_x_list.append(local_x) # here  show the  each local_x  has the differ length;
            local_abnormal_seq_list.append(local_abnormal)
            sequence_lengths.append(local_x.size(0)) # record the each local_x 's length;

        # Sort by sequence length for packing, from large to small
        sorted_indices = sorted(range(num_intervals), key=lambda i: sequence_lengths[i], reverse=True)
        ...
        # Pad and pack sequences, pad all the loca_x, and local_abnormal to the same length,  so that can avoid use for loop;
        max_len = max(sorted_lengths) # pad_local_x: (num_anchor_intervals,  max_length_for_local_x, node_emb_dim)
        ...
        # Process through GRUs,  with  all the  local interval x feature
        _, h_local = self.local_feat_rnn(packed_local_x)
        _, h_local_abnormal = self.local_abnormal_rnn(packed_local_abnormal)

        # Unsort to original order # (num_anchor_intervals, node_embed)
        ...
        normalized_starts =  starts/ audio_len.item()
        normalized_ends =  ends / audio_len.item()

        normalized_centers =  centers/ audio_len.item()
        normalized_widths =  widths / audio_len.item()


        # normalized_starts =  starts/ audio_len,
        # normalized_ends =  ends / audio_len,
        #
        # normalized_centers =  centers/ audio_len,
        # normalized_widths =  widths / audio_len,

        # use  normalized  starts,  ends  for the  refine the offset
        for layer in self.refinement_layers:
            query = torch.cat([  # (num_anchor_intervals, node_embed +5 )
                local_feat,
                normalized_starts.unsqueeze(-1) ,
                normalized_ends.unsqueeze(-1) ,
                normalized_centers.unsqueeze(-1) ,
                normalized_widths.unsqueeze(-1) ,
                local_abnormal_feature.unsqueeze(-1)
            ], dim=1)  # (num_intervals, node_embedding_dim + 5)

            logits = layer(query)  # (num_intervals, 2 * dist_bins)
            start_logits, end_logits = logits.chunk(2, dim=1)
            start_offsets = (F.softmax(start_logits, dim=1) * self.weight_params).sum(dim=1)
            end_offsets = (F.softmax(end_logits, dim=1) * self.weight_params).sum(dim=1)

            normalized_starts = ( normalized_starts + start_offsets).clamp(0,1)
            normalized_ends =   ( normalized_ends + end_offsets).clamp(0,1)

            normalized_centers = ( normalized_starts + normalized_ends) / 2
            normalized_widths =  normalized_ends - normalized_starts

        # 6. Final bounds and predictions
        final_bounds = torch.stack([normalized_starts * audio_len.item(), normalized_ends * audio_len.item()], dim=1)
        local_feat_full = torch.cat([local_feat, local_abnormal_feature.unsqueeze(-1)], dim=1)
        interval_conf_logits_all = self.interval_conf_head(local_feat_full)
        interval_cls_logits_all = self.interval_class_head(local_feat_full)
