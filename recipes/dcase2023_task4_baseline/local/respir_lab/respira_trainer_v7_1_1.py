...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        alpha = [0.0214, 0.0935, 0.0261, 0.2236, 0.1673, 0.0128, 0.4609] # this  for sample 10 normal every epoch;
        ...
        self.val_buffer_refine_pred = {  # 为不同阈值创建多个 DataFrame 缓冲区，用于存储合成数据和测试数据的验证结果。
            k: pd.DataFrame() for k in self.hparams["training"]["val_thresholds"]
        }
        ...
        self.epoch_node_label_counts = torch.zeros(7, dtype=torch.long)
        self.valid_epoch_node_label_counts = torch.zeros(7, dtype=torch.long)
    ...
    def training_step(self, batch_data, batch_indx):
        ...
        batch_label_counts = torch.bincount(node_labels, minlength=7)
        ...
        self.log("train/consis_loss", train_consistency_loss)
    ...
    def validation_step(self, batch_data, batch_indx):
       ...
       # self.valid_frame_loss =  self.frame_loss_weight * self.valid_frame_loss
        ...
        self.valid_total_loss =   self.valid_node_loss   + self.valid_edge_loss + self.consistency_loss
        ...
        for th in self.val_buffer_refine_pred.keys():
            self.val_buffer_refine_pred[th] = pd.concat(
                [self.val_buffer_refine_pred[th], refine_pred],
                ignore_index=True,
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        if len(self.validation_node_preds) > 0:
            ...
            self.print_matrix_with_labels(node_cm, classes_labels, "Node-level Validation Confusion Matrix")
            ...
            self.print_matrix_with_labels(node_cm_ratio, classes_labels,
