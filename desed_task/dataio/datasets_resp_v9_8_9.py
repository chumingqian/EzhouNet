...
from desed_task.utils.data_spec_aug import  standardize_rows, GroupSpecAugment_v2
...
#  v9-8-4,  修改了数据增强， 确保其不会影响声音事件的检测；


#
#  v9-8-7, 数据增强方式， 只使用Noise aug ,Loundness Aug 并且减小他们的幅度； 去除 vtlp,pitchAug


#  v9-8-8, 数据增强方式， 随机使用Noise aug ,Loundness Aug，包含vtlp,pitchAug；
#  但是确保他们在合理的范围内变化；


#  v9-8-9, 使用自定义的增强方式， 适用于呼吸音的 数据增强；
...
# ---- Custom precise noise augmenter ----
def add_noise(signal, noise_factor_range=(0.001, 0.01)):
    """Add Gaussian noise at a controlled amplitude level."""
    ...
    return np.clip(augmented, -1.0, 1.0)  # prevent clipping


# class FrequencyPreservingAugment:
#     def __init__(self, sample_rate):
#         self.sample_rate = sample_rate
#
#     def add_noise(self, waveform, noise_factor=(0.001, 0.01)):
#         """Add light broadband noise without masking target frequencies."""
#         factor = random.uniform(*noise_factor)
#         noise = torch.randn_like(waveform) * factor
#         return waveform + noise
#
#     def change_gain(self, waveform, gain_db=(-3, 3)):
#         """Adjust loudness in dB, preserving timing and spectral shape."""
#         gain = random.uniform(*gain_db)
#         return torchaudio.functional.gain(waveform, gain)
#
#     def mild_eq(self, waveform, center_freq=None, gain_db=(-1.5, 1.5), q=1.0):
#         """
#         Apply very mild EQ to simulate different stethoscope or chest wall acoustics.
#         center_freq: random pick in 200–1500 Hz range.
#         """
#         if center_freq is None:
#             center_freq = random.uniform(200, 1500)
#         gain = random.uniform(*gain_db)
...
#         )
#
#     def mild_pitch_shift(self, waveform, semitones=(-0.5, 0.5)):
#         """
#         Pitch shift without changing duration.
#         Keeps frequency ranges close to original.
#         """
#         shift = random.uniform(*semitones)
...
#         )
#
#     def mild_reverb(self, waveform, reverb_prob=0.3):
#         """
#         Add short IR-based convolution to simulate stethoscope position change.
#         Only applied randomly with small probability.
#         """
#         if random.random() > reverb_prob:
#             return waveform
#         # Simple impulse response (short)
#         ir = torch.zeros(int(0.03 * self.sample_rate))  # 30 ms
#         ir[0] = 1.0
#         ir[int(0.01 * self.sample_rate)] = 0.5  # tiny echo at 10ms
#         ir = ir.unsqueeze(0)  # match channel
#         return torch.nn.functional.conv1d(
#             waveform.unsqueeze(0),  # batch
#             ir.unsqueeze(1),        # kernel
#             padding=ir.shape[-1] // 2
#         ).squeeze(0)
#
#     def __call__(self, waveform):
#         """Apply a random selection of safe augmentations."""
#         aug_funcs = [
#             lambda x: self.add_noise(x),
#             lambda x: self.change_gain(x),
#             lambda x: self.mild_eq(x),
#             lambda x: self.mild_pitch_shift(x),
#             lambda x: self.mild_reverb(x)
#         ]
#         # Apply 1–3 augmentations per call
#         num_to_apply = random.randint(1, 3)
#         for func in random.sample(aug_funcs, num_to_apply):
...
class FrequencyPreservingAugment:
    ...
    def mild_reverb(self, waveform, reverb_prob=0.3):
        ...
        if random.random() > reverb_prob:
            return waveform
        ir = torch.zeros(int(0.03 * self.sample_rate))
        ir[0] = 1.0
        ir[int(0.01 * self.sample_rate)] = 0.5
        ir = ir.unsqueeze(0)
        return torch.nn.functional.conv1d(
    ...
    def __call__(self, waveform: torch.Tensor) -> torch.Tensor:
        # Assert input type for safety

        assert isinstance(waveform, torch.Tensor), "Input must be a torch.Tensor"
...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        test=False,
        # apply_general_aug=True
    ):
        super(RespiraGnnSet, self).__init__()
        # self.apply_data_aug = apply_general_aug
        self.gen_augmented = FrequencyPreservingAugment(sample_rate= fs)
    ...
    def __getitem__(self, idx,apply_general_aug=False ):
        ...
        if self.train_flag and binary_label == "Abnormal" and apply_general_aug:
                if contains_rare_event and random.random() > 0.5:
                    # print("\n call the 0.5 pro for (Stridor or Rhonchi) ")
                    audio = self.gen_augmented(audio)  # no numpy conversion needed
                elif not contains_rare_event and random.random() > 0.8:
                    audio = self.gen_augmented(audio)
