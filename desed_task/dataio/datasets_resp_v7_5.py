...
class RespiraGnnSet(Dataset):
    ...
    def __getitem__(self, idx):
       ...
       # check_array(audio)
        ...
        #audio = audio.to("cuda")
        img_save_path = '/media/respecting_god/S2/05_soundEventDetection/Respira_SED_LGNN/data/Spec_img'
        cur_audio_name = Path(cur_audio_path).stem
        spectrogram = self.spec_extract_features(audio, binary_label,cur_audio_name, img_save_path) #  此时，生成的单个样本是 （3, 84,656,）# (chan =3, n_mels=84,  frames= variable)
    ...
    #     plot_spectrogram(cur_gamma.squeeze(0), "Gamma Spectrogram (Amplitude to dB)")
    #
    ...
    def spec_extract_features(self, audio, binary_label, audio_name, save_path):
        """Extracts and saves spectrogram features."""
        # Create a folder for the current audio
        audio_folder = os.path.join(save_path, audio_name)
        os.makedirs(audio_folder, exist_ok=True)

        # ----------- Generate Mel Spectrogram Features ----------------------------
        mel_spec = self.mel_spec(audio)  # Raw Mel Spectrogram (n_filters=84, frames=626)
        save_spectrogram(mel_spec, "Raw Mel Spectrogram", audio_folder, "mel_raw")

        mel_spec = self.powerToDB(mel_spec)  # Mel Spectrogram (Amplitude to dB)
        save_spectrogram(mel_spec, "Mel Spectrogram (Amplitude to dB)", audio_folder, "mel_amplitude_to_db")

        mel_norm = standardize_rows(mel_spec)  # Mel Spectrogram (Standardized Rows)
        save_spectrogram(mel_norm, "Mel Spectrogram (Standardized Rows)", audio_folder, "mel_standardized")
        mel_norm = mel_norm.unsqueeze(0)  # (1, 84, 626)

        # ----------- Generate CQT Spectrogram Features ----------------------------
        cur_cqt = self.cqt_spectrogram(audio)  # Raw CQT Spectrogram (1, bins, frames=626)
        save_spectrogram(cur_cqt.squeeze(0), "Raw CQT Spectrogram", audio_folder, "cqt_raw")

        cur_cqt = self.amplitudeToDB(cur_cqt)  # CQT Spectrogram (Amplitude to dB)
        save_spectrogram(cur_cqt.squeeze(0), "CQT Spectrogram (Amplitude to dB)", audio_folder, "cqt_amplitude_to_db")

        cur_cqt = cur_cqt.squeeze(0)
        cqt_norm = standardize_rows(cur_cqt)  # CQT Spectrogram (Standardized Rows)
        save_spectrogram(cqt_norm, "CQT Spectrogram (Standardized Rows)", audio_folder, "cqt_standardized")
        cqt_norm = cqt_norm.unsqueeze(0)

        # ----------- Generate Gamma Spectrogram Features ----------------------------
        cur_gamma = self.gamma_spec(audio)  # Raw Gamma Spectrogram (1, n_filters, frames=626)
        save_spectrogram(cur_gamma.squeeze(0), "Raw Gamma Spectrogram", audio_folder, "gamma_raw")

        cur_gamma = self.powerToDB(cur_gamma)  # Gamma Spectrogram (Amplitude to dB)
        save_spectrogram(cur_gamma.squeeze(0), "Gamma Spectrogram (Amplitude to dB)", audio_folder,
                         "gamma_amplitude_to_db")

        cur_gamma = cur_gamma.squeeze(0)
        gamma_norm = standardize_rows(cur_gamma)  # Gamma Spectrogram (Standardized Rows)
        save_spectrogram(gamma_norm, "Gamma Spectrogram (Standardized Rows)", audio_folder, "gamma_standardized")
        gamma_norm = gamma_norm.unsqueeze(0)

        # Random augmentation for abnormal samples during training
        spec_Aug = random.random()
        if self.train_flag and binary_label == "Abnormal" and spec_Aug > 0.5:
            mel_norm = self.spec_aug(mel_norm)
            cqt_norm = self.spec_aug(cqt_norm)
        ...
        # Combine spectrograms into a hybrid spectrogram
        hybrid_spectrogram = torch.cat((cqt_norm, gamma_norm, mel_norm), dim=0)
...
def save_spectrogram(spectrogram, title, save_dir, filename):
