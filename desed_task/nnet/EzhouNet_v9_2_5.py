...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        self.interval_conf_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(node_embedding_dim + 3, 64),  # Hidden layer
                nn.ReLU(),
        ...
        self.interval_class_heads = nn.ModuleList([
            nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(0.2),  # Regularization
                nn.Linear(64, self.num_classes - 1)  # Output layer
            ) for _ in range(self.num_scales)
        ])
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            interval_refine_head = self.interval_refine_head[scale_idx]
            ...
            interval_refine_output_logit = interval_refine_head(shared_feature)

            start_logits = interval_refine_output_logit[:, :self.dist_bins[scale_idx]]
            end_logits =interval_refine_output_logit[:, self.dist_bins[scale_idx]:2 * self.dist_bins[scale_idx]]

            interval_conf_logits_scale =  interval_conf_head(shared_feature)
            interval_cls_logits_scale  =  interval_class_head(shared_feature)
