...
from desed_task.dataio.datasets_resp_v9_5_1 import RespiraGnnSet
from desed_task.nnet.EzhouNet_v9_6_3 import GraphRespiratory
from local.respira_trainer_v9_5_7 import RespiraSED_lab
...
# v9-5-1:  调整为双向 Bigru, 使用集成头的方式；
# 开始调整，每组中的帧数来进行实验，看看帧数构成节点特征的影响。
# 25 帧一组，  5 帧为位移；


# v9-5-2,  将置信度损失与 分类损失， 分阶段优化；


# v9-5-3,  frames = 25 frames;
# 1.将置信度损失与分类损失，分阶段优化；
# 2. 调整置信度的损失函数；
...
# v9-6   直接将位置信息加在后端， 即对节点和区间的进行分类的前一层中，加入位置信息；
# 而不将位置信息纳入到特征的节点特征更新中；
# 分别进行实验， 看加入位置信息后，对节点分类的影响以及对区间分类的影响

# v9-6-2,  对节点分类 和区间分类同时加入位置信息；
# 将位置信息加入到区间分类的前一层， 用于判断对节点的影响；


# v9-6-3,  只对区间特征加入 位置信息；
# 将位置信息加入到区间分类的前一层， 用于判断对节点的影响；
