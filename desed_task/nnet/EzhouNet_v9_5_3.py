...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, heads=2):
        ...
        self.gat = GATConv(in_channels, out_channels, heads=heads, concat=False, edge_dim= 16 )#16
...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        print("After gaussian init: smoothing_conv.weight requires_grad:",
              self.smoothing_conv.weight.requires_grad)
        ...
        print("start_weight_params requires_grad:",
              all(p.requires_grad for p in self.start_weight_params))
...
class Node_Interval_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop  ): # pass the batch data
        ...
        print("data.x before to(device):", data.x.shape, "grad_fn:", data.x.grad_fn, "requires_grad:", data.x.requires_grad)
        ...
        assert edge_index.shape[1] > 0, "Empty edge_index"
        assert x.shape[0] > 0, "Empty node features"

        # print("x:", x, "grad_fn:", x.grad_fn, "requires_grad:", x.requires_grad)
        # print("edge_index:", edge_index, "grad_fn:", edge_index.grad_fn if edge_index.is_tensor else "Not a tensor")
        # print("edge_attr:", edge_attr, "grad_fn:", edge_attr.grad_fn)
        # assert x.is_cuda, "x not on CUDA"
        # assert edge_index.is_cuda, "edge_index not on CUDA"
        # assert edge_attr.is_cuda, "edge_attr not on CUDA"

        # Log input information
        print("x shape:", x.shape, "grad_fn:", x.grad_fn, "requires_grad:", x.requires_grad)
        print("edge_index shape:", edge_index.shape, "grad_fn:",
              edge_index.grad_fn if isinstance(edge_index, torch.Tensor) else "Not a tensor")
        print("edge_attr shape:", edge_attr.shape, "grad_fn:",
              edge_attr.grad_fn if isinstance(edge_attr, torch.Tensor) else "Not a tensor")
        print("batch_indices shape:", batch_indices.shape, "num_audios:", num_audios)
        ...
        #print("node_embeddings:", node_embeddings, "grad_fn:", node_embeddings.grad_fn)

        node_preds = self.node_class_heads(node_embeddings) # (batch_nodes, normal confidence + 4 abnormal cls)
        #print("node_preds:", node_preds, "grad_fn:", node_preds.grad_fn)
        ...
        # print("node_predictions:", node_predictions, "grad_fn:", node_predictions.grad_fn)
        if node_predictions.grad_fn is None:
            print("Error: node_predictions lacks grad_fn")
            raise ValueError("node_predictions lacks grad_fn, check model parameters and input gradients")
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.edge_encoder = nn.Linear(2*2, edge_feature_dim) # *2, due to  combine two edge_proj
    ...
    def forward(self, batch_data, level):
        ...
        # Debug node_fea_generator parameters
        print("node_fea_generator parameters:")
        # for name, param in self.node_fea_generator.named_parameters():
        ...
        if level == "node":
            ...
            # Debug spectrograms and chunks
            # print("Spectrograms type:", type(spectrograms), "is_tensor:", isinstance(spectrograms[0], torch.Tensor))
            # print("Spectrograms shape:", spectrograms[0].shape, "device:",
            #       spectrograms[0].device if isinstance(spectrograms[0], torch.Tensor) else "N/A")

            # Ensure training mode
            self.node_fea_generator.train()
            self.node_edge_proj.train()
            # self.gat_model.train()
            # self.node_class_heads.train()
            print("node_fea_generator training:", self.node_fea_generator.training)
            print("node_edge_proj training:", self.node_edge_proj.training)
            # print("gat_model training:", self.gat_model.training)
            # print("node_class_heads training:", self.node_class_heads.training)
            ...
            # print("all_chunks_tensor:", all_chunks_tensor, "grad_fn:", all_chunks_tensor.grad_fn)
            ...
            # print("Spectrograms type:", type(spectrograms), "is_tensor:", isinstance(spectrograms[0], torch.Tensor))
            # print("First chunk type:", type(all_chunks[0]))

            node_vad_fea = self.node_edge_proj(node_features)
            # print("node_fea_generator output:", node_features, "grad_fn:", node_features.grad_fn)
            # print("node_vad_fea output:", node_vad_fea, "grad_fn:", node_vad_fea.grad_fn)
            ...
            print("batch_graph.x:", batch_graph.x, "grad_fn:", batch_graph.x.grad_fn)
