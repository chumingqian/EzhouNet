...
class RespiraSED_lab(pl.LightningModule):
    ...
    #def training_step(self, batch_data, batch_idx):
        ...
        # conf_optimization_epochs = 120
        # if self.current_epoch < conf_optimization_epochs:
        #     total_loss = node_conf_loss + conf_loss
        # else:
        #     total_loss =  node_conf_loss + conf_loss + node_level_loss  + interval_cls_loss + loc_loss

        transition_epochs = 40
        conf_optimization_epochs = 80
        if self.current_epoch < conf_optimization_epochs:
            total_loss = node_conf_loss + conf_loss
        elif self.current_epoch < conf_optimization_epochs + transition_epochs:
            alpha = (self.current_epoch - conf_optimization_epochs) / transition_epochs
    ...
    def configure_optimizers(self):
        ...
        scheduler_node_update_conf_cls = {
            'scheduler': torch.optim.lr_scheduler.MultiStepLR(
                optimizer_node_update_conf_cls,
                milestones=[80, 120, 250],  # Adjust based on your training length and needs
        ...
        # print("\nNode parameters:")
        # for param_group in optimizer_node_update_conf_cls.param_groups:
        #     for param in param_group['params']:
        #         if param.requires_grad:
        #             print(f"  - {param.shape}")
        #
        # print("\nInterval parameters:")
        # for param_group in optimizer_interval_conf_cls_refine.param_groups:
        #     for param in param_group['params']:
        #         if param.requires_grad:
        #             print(f"  - {param.shape}")
    ...
    def on_train_epoch_start(self):
        ...
        lr_interval_conf_cls_refine = optimizer[1].param_groups[0]['lr']  # classification
        # lr_shared = optimizer.param_groups[2]['lr']  # shared

        print(f"Epoch {self.current_epoch}: lr_node_conf_cls={lr_node_conf_cls}, "
              f"lr_interval_off={lr_interval_conf_cls_refine}, ")

        self.log('lr_node_conf_cls', lr_node_conf_cls, on_epoch=True)
        self.log('lr_interval_conf_cls_refine', lr_interval_conf_cls_refine, on_epoch=True)
