...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        num_additional_features = 2  # For node_vad_flags_tensor
        ...
        self.node_fea_proj2_vad =  nn.Sequential( nn.Linear(node_fea_dim, 64),
                                                     nn.ReLU(),
                                                     nn.Dropout(0.5),
                                                     nn.Linear(64, 2)
                                                )
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            #  node_features: (total_nodes, feature_dim=512), node_vad: (total_nodes, feature_dim=512)
            node_features = self.node_fea_generator(all_chunks_tensor)


            # should compute loss  with  node_vad_flags_all: (total_nodes ),
            # note, sigmoid output
            node_vad_logit =  self.node_fea_proj2_vad(node_features)  # tensors: (total_nodes, fea_dim=1)
            node_vad_prob = torch.sigmoid(node_vad_logit)


            if self.training:
                # print(f"\n it's in the trianing stage, Not using vad thread ")
                # Use continuous probabilities during training
                node_features = torch.cat([node_vad_prob, node_features], dim=1)
            else:
                ...
                # node_features = torch.cat([node_vad_judge, node_features], dim=1)
                node_features = torch.cat([node_vad_prob, node_features], dim=1)
