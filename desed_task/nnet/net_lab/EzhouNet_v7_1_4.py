...
# v7-1-4,  edge attr  是依据 边的两个节点特征生成。；
...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, heads=2):
        ...
        self.gat = GATConv(in_channels, out_channels,
                           ...
                           edge_dim=2,
                           add_self_loops= True # recover the automatic addition of self-loops
                           )
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.edge_mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256, 2)  # Output dimension for edge_features
        ...
        if level == "node":
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                if num_nodes_sample > 1:
                    ...
                    edge_features = self.edge_mlp(combined_node_features)  # [E, 2]



                else:
                    edge_index = torch.empty((2, 0), dtype=torch.long, device=device)
                    edge_features = torch.empty((0, 2), dtype=torch.float32, device=device)
