...
#  v9-3-10,  引入虚拟节点， 作为全局特征来表示每个音频的位置信息；



# v9-3-11:  调整为双向 Bigru,
# 保证该全局虚拟连接是单向的， 即该位置信息影响样本中的每个常规节点，
# 但是常规节点，却不应该改变该全局的位置信息， 因为此时每个样本的位置信息是固定的；
...
class Interval_Refine(nn.Module):
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            start_head = self.start_heads[scale_idx]
            end_head = self.end_heads[scale_idx]
            conf_cls_head = self.conf_cls_heads[scale_idx]
            ...
            start_logits = start_head(h_backward)  # Backward for start offset
            end_logits = end_head(h_forward)       # Forward for end offset
            ...
            conf_cls_logits = conf_cls_head(shared_feature)
            interval_conf_logits_scale = conf_cls_logits[:, 0]
            interval_cls_logits_scale = conf_cls_logits[:, 1:]
...
class Node_Interval_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop  ): # pass the batch data
        ...
        # Disable NumPy truncation
        # np.set_printoptions(threshold=np.inf)  # Show all elements
        # # Print full edge_index
        # print(edge_index.cpu().numpy())
        ...
        for i in range(num_audios):
            ...
            num_original_nodes = num_nodes - 1  # Exclude the virtual node
            node_indices = torch.arange(num_original_nodes, device=device) * 5
            if num_original_nodes > 1 and node_indices[-1] > num_frames - 5:
                node_indices[-1] = num_frames - 2.5
            time_positions = node_indices / num_frames
            time_positions = torch.cat([time_positions, torch.tensor([0.0], device=device)])
            time_positions = time_positions.unsqueeze(-1)  # Shape: (num_nodes, 1)
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        # Define global feature projector
        num_global_features = 0
        if self.include_gender:
            num_global_features += 2  # Gender one-hot size
        if self.include_location:
            num_global_features += 4  # Location one-hot size
        if num_global_features > 0:
            self.global_fea_proj = nn.Linear(num_global_features, node_fea_dim)
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            # Initialize lists
            all_chunks = []
            all_chunk_type_labels = []  # Regular node labels
            all_node_type_labels = []  # All node labels (regular + virtual)
            ...
            for sample_idx, (spec, labels, vad_intervals, audio_dur) in enumerate(
                ...
                for j in range(0, n_frames - chunk_size + 1, stride):
                    ...
                    all_node_type_labels.append(type_label)  # Add to all labels
            ...
            # this should  be as  global feature for  each audio, not to each node;
            # if self.include_gender:
            ...
            all_graphs = []
            start_idx = 0
            virtual_node_label_indices = []  # Track where to insert -2 labels
            current_label_idx = 0  # Track position in all_node_type_labels
            ...
            for sample_idx, num_nodes in enumerate(num_nodes_per_sample):
                ...
                if global_fea_parts:
                    ...
                    sample_x = torch.cat([sample_node_features, virtual_node_fea],
                                         dim=0)  # [num_nodes + 1, node_fea_dim]
                    virtual_node_idx = num_nodes
                    sample_y = torch.cat([sample_node_type_labels, torch.tensor([-2], dtype=torch.long, device=device)])
                    #all_node_type_labels.append(-2)  # Add virtual node label

                    # Insert -2 label after the regular node labels for this sample
                    virtual_node_label_indices.append(current_label_idx + num_nodes)
                else:
                    sample_x = sample_node_features
                    virtual_node_idx = None
                    sample_y = sample_node_type_labels

                # Update label index for the next sample
                current_label_idx += num_nodes
                ...
                if virtual_node_idx is not None:
                    ...
                    edge_index = torch.cat([original_edge_index, edges_from_virtual], dim=1)

                else:
                    edge_index = original_edge_index

                # Edge features
                sample_node_edge_fea = self.node_edge_proj(sample_x) # note, here sample x merge in the  position info
                ...
                graph_data = Data(x=sample_x, edge_index=edge_index,
                                edge_attr=edge_features, y=sample_y)
                graph_data.edge_y = edge_labels
                all_graphs.append(graph_data)

                # Update start_idx (original logic, correct for node_features without virtual nodes)
                start_idx = end_idx


            # Insert virtual node labels (-2) into all_node_type_labels at the correct positions
            for insert_idx in sorted(virtual_node_label_indices, reverse=True):
                all_node_type_labels.insert(insert_idx, -2)

            batch_graph = Batch.from_data_list(all_graphs).to(all_chunks_tensor.device)
            # Validation: Check for cross-sample leakage
            batch = batch_graph.batch
            same_batch = batch[batch_graph.edge_index[0]] == batch[batch_graph.edge_index[1]]
            assert same_batch.all(), "Cross-sample edges detected!"
            ...
            outputs = {
                ...
                'node_type_labels':  torch.tensor(all_node_type_labels, dtype=torch.long, device=device),  # Use all labels #node_type_labels,
