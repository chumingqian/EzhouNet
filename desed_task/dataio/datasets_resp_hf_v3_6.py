...
class RespiraGnnSet(Dataset):
    def __init__(
        ...
        for id, audio_name in enumerate(tqdm(self.examples,desc='reorganize audio info,' )):
            ...
            audio, onset_s, offset_s, padded_indx = self.read_audio(
                cur_audio_path, self.multisrc, self.random_channel, None, self.test, self.fs
            )
    ...
    def read_audio(self, file, multisrc, random_channel, pad_to=None, test=False, target_fs=8000):
        ...
        # Resample if the original sampling rate differs from the target
        if fs != target_fs:
            #print(f"the original audio is  sample rate at {fs },  now we are resample at {target_fs}")

            resampler = Resample(orig_freq=fs, new_freq=target_fs)
            mixture = resampler(mixture)
            fs = target_fs  # Update the sampling rate to the target
