...
from desed_task.dataio.datasets_resp_v9_1_1 import RespiraGnnSet
from desed_task.nnet.EzhouNet_v9_4_2 import GraphRespiratory
from local.respira_trainer_v9_4_4 import RespiraSED_lab


# # v8-18, self.gat_model  并行化；
# 1. refine layer,  interval confidence, inteval cls
#    这些层增加隐藏层以及dropout,  提高模型的学习容量， 并且降低过拟合；
# 2. 节点和区间参数的使用两个不同的学习率进行分开优化， 并对区间使用余弦退火算法
# 3. 将定位损失 添加 L1 距离损失作为可选项；


# # v8-18_1,  使用验证集；
# # v8-18_2,  使用测试集；
# # v8-18_3,  使用验证集 + 使用 time shif  数据增强；

#  # v8-18_4， 验证集，  试着将真实的标签替换为IOU的标签， 使用软标签
#   区间分类损失权重动态调整， 置信度过拟合现象减少；

#  v8-18_5,  使用 time shift +  置信度软标签

#  v8-18_6,  置信度软标签,
#  batch time shift 更改为使用 batch 级别的 time shift， 用于稳定训练；

#  v8-18_7,  置信度软标签, +  note, 置信度与区间分类参数，使用分阶段训练；
#  batch time shift 更改为使用 batch 级别的 time shift， 用于稳定训练；
...
#1.   网络： 时序编码，  更新 local  feature 局部特征的生成；

# 2. 区间的偏移生成，  置信度， 分类， 使用共同的特征作为输入，
#    并且使用公用头部进行输出， 之后，使用切片范式， 提取偏移量， 置信度，分类；  之后，使用不同的损失函数进行优化
# 3. 网络：  对节点的分类，使用同样的方式， 即置信度与分类使用同一个分类头，
#    但是，通过切片进行提取，使用不同的损失函数对置信度和分类进行优化；


##  v9-2-1,  节点的帧数调整为20 帧一个节点，  直接共同优化， 区间参数学习率保持不变；
#   并且以区间损失为主导损失；  此时，节点损失很小， 几乎起到微弱的作用；


##  v9-2-2,   调整优化器， 区间采用学习率使用余弦退火的方式优化
#   并且以区间损失为主导损失；  此时，节点损失起到辅助作用；



#  v9-2-3, 退回到 5 帧一个节点，  调整优化器， 区间采用学习率使用余弦退火的方式优化
#   并且以区间损失为主导损失；  此时，节点损失起到辅助作用； weight param = [-20, 20]


#  v9-3-4,对 node embedding  先进行 Linear 转换，
# 实验,   使用公用特征， 同时使用集成 head，  将区间置信度， 分类， 定位放在一起； weight param = [-1.0, 1.0]
...
#  v9-3-5_posi,  恢复原始的 node embeding +temporal,
#  并且加入呼吸音位置信息； note,  hard one hot coding


# v9-3-7:  调整为双向 Bigru,  从 gru 中 获取 logit,
# 然后与独立的 weight param 相乘获得 start, end offsets;

# v9-3-12:  调整为双向 Bigru,  并且此时，使用集成头的方式；


# v9-4-1:  调整为双向 Bigru,  并且此时，使用集成头的方式；
# 加入位置信息到 更新后的节点特征中，进行实验， 对位置信息进行软编码；


# v9-4-2:  双向 Bigru,集成头的方式；  此时调整，
# 将位置信息融入到 interval 级别，
# 而不是在 Node 级别， 避免与temporal encoding 产生冲突混淆；



# v9-4-3, group frames = 5  考虑位置信息；   将置信度损失与 分类损失， 分阶段优化；


# v9-4-4, group frames = 5  考虑位置信息；
# 1.将置信度损失与分类损失，分阶段优化；
# 2. 调整置信度的损失函数；
# 注意 time position  与chunk size 的变化；
...
def prepare_run(argv=None):
    ...
    parser.add_argument(
        ...
        default="./confs/rep_lab8_18.yaml",
