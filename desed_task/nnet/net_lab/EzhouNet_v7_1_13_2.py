...
class Node_Edge_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, node_num_classes, edge_num_classes=2):
        ...
        self.node_vad_classifier = nn.Linear(out_channels, 1)
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        adjusted_in_channels = node_fea_dim + node_vad_dim + num_additional_features

        self.node_vad_thread = 0.5


        self.node_vad_proj =  nn.Sequential( nn.Linear(node_vad_dim, 64),
    ...
    def forward(self, batch_data, level):
        ...
        if level == "node":
            ...
            # note, sigmoid output
            node_vad_logit =  self.node_vad_proj(node_vad)  # tensors: (total_nodes, fea_dim=1)
            ...
            if self.training:
                ...
                node_features = torch.cat([ node_vad_prob, node_vad, node_features,], dim=1)
            else:
