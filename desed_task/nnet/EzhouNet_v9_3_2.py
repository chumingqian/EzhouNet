...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
                  kernel_size=5, dist_bins_list= None, num_scales = None ):
        #super().__init__()
        ...
        self.num_scales = num_scales  # Three scales
        ...
        # self.weight_params = nn.ParameterList([
        #     nn.Parameter(torch.linspace(-100.0, 100.0, dist_bins_list[i]))
        #     for i in range(self.num_scales)
        # ])

        self.min_val = nn.Parameter(torch.tensor(-100.0))
        self.max_val = nn.Parameter(torch.tensor(100.0))
        self.weight_params = nn.ParameterList([
            nn.Parameter(torch.linspace(self.min_val.item(), self.max_val.item(), dist_bins_list[i]))
            for i in range(self.num_scales)
        ])
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            # here , to ensure  not to over valid range;
...
class Node_Interval_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
                 node_num_classes,
                 num_refine_layers = 1,
                 dist_bins_list=[100, 100, 100, 100, 100],
                 num_anchor_scales = 5
                 ):
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop  ): # pass the batch data
        ...
        for i in range(num_audios):
            ...
            event_output = self.interval_refine( # the instance of class EnhancedTimeAwareFDR
                ...
                num_intervals_per_scale = [10, 20, 20, 10, 10]  #[39,19, 9] # each scale's  num_anchor_intervals
