...
class RespiraSED_lab(pl.LightningModule):
    ...
    def __init__(
        ...
        self.interval_unify_head_params = list(
            self.sed_net.node_interval_pred.interval_refine.interval_unify_head.parameters())
        ...
        self.interval_conf_loss_weight =   60.0  # 40.0
        self.interval_cls_loss_weight =    10.0 #15.0    #50.0
    ...
    def configure_optimizers(self):
        ...
        # Scheduler for node parameters: Step-based exponential decay
        scheduler_node = {
            'scheduler': torch.optim.lr_scheduler.LambdaLR(
                optimizer_node,
                lr_lambda=lambda step: 0.99 ** (step / 126)  # Decay matches original step-wise rate
            ),
        ...
        scheduler_interval = {
            'scheduler': torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer_interval,
                T_max=50400,  # 400 epochs * 126 batches/epoch from original setup
                # eta_min=2e-4,  # Optional: Uncomment to set a minimum learning rate
            ),
            'interval': 'step',  # Update every step
            'frequency': 1,
            'name': 'interval_tasks_scheduler'
