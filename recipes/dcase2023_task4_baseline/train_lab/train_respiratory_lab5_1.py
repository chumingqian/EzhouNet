...
from desed_task.utils.encoder import ManyHotEncoder, RespiraHotEncoder
from local.classes_dict import classes_labels, binary_labels

#  v5,  去除加权采样和 加权损失的方式，
#  改用均衡采样， 即确保一个batch 中不同类型样本的个数大体相同

from desed_task.dataio.datasets_resp_v5_1 import RespiraGnnSet, StratifiedMultiLabelBatchSampler
from desed_task.nnet.EzhouNet_v3_2 import GNNRespiraModel
from local.respira_trainer_v5_1 import RespiraSED_lab
...
# def calculate_sample_weights(dataset, class_weights):
#     sample_weights = []
#     for audio_name in dataset.examples_list:
#         sample = dataset.examples[audio_name]
#         labels_df = sample['label_df']
#         if not labels_df.empty:
#             labels = labels_df['event_label'].tolist()
#             # Sum the class weights for all labels, including duplicates
#             sample_weight = sum([class_weights[label] for label in labels])
#         else:
#             # For samples without labels, assign the weight of the 'Normal' class or minimum weight
#             sample_weight = class_weights.get('Normal', min(class_weights.values()))
...
def single_run(
    ...
    #spec2node_generator = DynamicFeatureExtractor(**config["feats"])
    # spec2node_generator = DynamicFeatureExtractor(**config["spec2node"])#.cuda()
    sed_net = GNNRespiraModel(**config["net"])#.cuda()
    ...
    if test_state_dict is None:
        ...
        # sample_weights = calculate_sample_weights(train_dataset, class_weights)
        #
        ...
        batch_sampler = StratifiedMultiLabelBatchSampler(
            train_dataset,
            batch_size=None,  # Batch size is calculated inside the sampler
            samples_per_exception=  config["training"]["samples_per_exception"]   # self.samples_per_exception
        )

        ##### training params and optimizers ############
        epoch_len =   len(train_dataset) // (
                    config["training"]["batch_size"]    #  获取第 indx 个数据集的批大小
                    * config["training"]["accumulate_batches"] # 获取梯度累积步数
                ) #  计算一个epoch中，  batch 批次的个数



        opt = torch.optim.Adam(
            sed_net.parameters(), config["opt"]["lr"], betas=(0.9, 0.999)
        )
    ...
    if fast_dev_run:
        ...
        limit_train_batches = 0.005  #2 , bt =48;
    ...
    # ====== profiler the  cost time ===========
    # Initialize the profiler
    # torch_profiler = PyTorchProfiler(
    #     dirpath=Path("./profiler_output"),  # Directory to save profiling outputs
    #     filename="profiler_results",  # File name to save the results
    #     group_by_input_shapes=True,  # Group by input shapes to see which inputs are most expensive
    #     emit_nvtx=False,  # Set to True if you want to use with Nvidia's visual profiling tools
    #     export_to_chrome=True,  # Export results in a format that can be viewed in Chrome Tracing
    #     row_limit=10,  # Limit the number of rows in the report
    #     sort_by_key= "cpu_time_total" ,   #   "cuda_time_total",  # Sort by total CUDA time
    #     record_module_names=True,  # Record module names in the profile
