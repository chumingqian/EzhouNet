...
class Node_Interval_cls_Module(nn.Module):
    ...
    def forward(self, data, audio_dur, anchor_intervals  ): # pass the batch data
        ...
        for i in range(num_audios):
            ...
            # Compute time positions (assuming nodes are temporally ordered)
            num_nodes = mask.sum().item()
            time_positions = torch.linspace(0, 1, num_nodes, device=device).unsqueeze(-1)
            ...
            event_output = self.interval_refine( # the instance of class EnhancedTimeAwareFDR
                node_embeddings=cur_node_embeddings,
            ...
            node_predictions_list.append(cur_node_preds) # note,  need to justify
...
class GraphRespiratory(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        # print(node_labels.cpu().tolist())  # as plain Python list
