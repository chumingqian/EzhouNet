...
class EnhancedTimeAwareFDR(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=3,
        ...
        # Time embedding with duration awareness
        ...
        self.smoothing_conv = nn.Conv1d(
            ...
            padding=kernel_size // 2, groups=num_classes, bias=False
        )
        self._init_gaussian_weights()

        # Differentiable boundary detection
        self.boundary_detector = nn.Sequential(
            nn.Linear(node_embedding_dim + 1, 128),
            nn.ReLU(),
            nn.Linear(128, 2)
        )

        # Adaptive refinement layers
        self.refinement_layers = nn.ModuleList([
            nn.Sequential(
                nn.Linear(node_embedding_dim + 4, 256),
                nn.ReLU(),
                nn.Linear(256, 2 * dist_bins)
    ...
    def _init_gaussian_weights(self):
        """Initialize smoothing kernel with Gaussian distribution"""
        kernel = self.method_name()
        with torch.no_grad():
            for i in range(self.smoothing_conv.weight.shape[0]):
                self.smoothing_conv.weight[i] = kernel

    def method_name(self):
        kernel = torch.tensor([0.06136, 0.24477, 0.38774, 0.24477, 0.06136])
        return kernel

    def _differentiable_segment(self, x, time_positions, abnormal_scores, node_vad):
        """Differentiable interval detection with soft counting"""
        # Predict boundary probabilities
        boundary_logits = self.boundary_detector(
            torch.cat([x, time_positions], -1)
        )  # (nums_nodes, 2)
        boundary_probs = torch.sigmoid(boundary_logits) # (nums_nodes, 2)

        # Detect interval transitions
        transitions = F.conv1d(
            abnormal_scores.unsqueeze(0).unsqueeze(0), # (1,1, num_nodes)
            weight=torch.tensor([[[-1.0, 1.0]]], device=x.device),
            padding=1
        )

        # Soft interval counting，  max_intervals = 16s,  最大的个数应该限制为秒数 16秒；
        num_intervals = torch.sigmoid(transitions).sum(dim=-1)  # num_interval= 58 ,太多了， 需要想办法设定在一个区间中；
        return boundary_probs, num_intervals.round().long()

    def forward(self, node_embeddings, time_positions, batch_indices, node_pred=None, audio_durations=None):
        device = node_embeddings.device

        # 1. Duration-aware time embedding
        if self.true_duration_mode and audio_durations is not None:
            norm_times = time_positions * audio_durations[batch_indices].unsqueeze(1)
            duration_ratio = audio_durations[batch_indices] / audio_durations.mean()
        else:
            norm_times = time_positions
            duration_ratio = torch.ones_like(time_positions)

        time_feat = self.time_embed(torch.cat([norm_times, duration_ratio], -1)) # (115,256)
        x = node_embeddings + time_feat

        # 2. Learnable smoothing，  这里需要注意的是，平滑的时，是否是针对每个节点之间进行平滑， 还是对每个节点内部在五个类别上进行平滑， 还是各个节点之间在5个类别上的概率上进行平滑；
        smoothed_pred = F.softmax(self.smoothing_conv(node_pred.T).T, -1) #（num_nodes, cls=5,  ）
        normal_probs = smoothed_pred[:, self.normal_class_idx]
        abnormal_scores = 1 - normal_probs  # （nums_nodes）

        # 3. Global normality check
        if normal_probs.mean() > torch.sigmoid(self.normal_threshold):
            return self._empty_output(device)

        # 4. Differentiable interval detection
        boundary_probs, num_intervals = self._differentiable_segment(
            x, norm_times, abnormal_scores   #  norm_times,(nums_node=115, 1)
        )  # boundary_probs: (115, 2)
        if num_intervals == 0:
            return self._empty_output(device)

        # 5. Boundary initialization and refinement
        starts = (boundary_probs[:, 0] * norm_times.squeeze()).cumsum(0) / boundary_probs[:, 0].cumsum(0)
        ends = (boundary_probs[:, 1] * norm_times.squeeze()).cumsum(0) / boundary_probs[:, 1].cumsum(0)
        ...
        for i in range(num_intervals):
            ...
            for layer in self.refinement_layers:
                ...
                in_interval = (norm_times.squeeze() >= start) & (norm_times.squeeze() <= end)
                local_feat = x[in_interval].mean(dim=0) if in_interval.any() else x.mean(dim=0)
                ...
                logits = layer(query)
                start_adj = (F.softmax(logits[:self.dist_bins], 0) * self.weight_params).sum()
                end_adj = (F.softmax(logits[self.dist_bins:], 0) * self.weight_params).sum()

                start = (start + start_adj).clamp(0, 1)
                end = (end + end_adj).clamp(0, 1)
        ...
        interval_mask = torch.ones(len(refined_bounds), dtype=torch.bool, device=device)

        return {
            "final_bounds": final_bounds,
            "interval_mask": interval_mask,
            "num_intervals": len(refined_bounds)
        }
    ...
    def forward(self, data):
        ...
        for i in range(num_audios):
            ...
            event_output = self.time_aware_fdr(
                ...
                audio_durations=None  # You need to provide this if you have it
