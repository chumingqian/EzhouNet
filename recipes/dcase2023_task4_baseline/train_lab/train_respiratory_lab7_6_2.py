...
from desed_task.dataio.datasets_resp_v7_5_2 import RespiraGnnSet
from desed_task.nnet.EzhouNet_v7_5_7_3 import GraphRespiratory
from local.respira_trainer_v7_5_7 import RespiraSED_lab



# note,  开始使用子采样的方式， 每个epoch 中 只抽取10 个正常样本，
#  来构建 train_dataloader;
# 1. update ema =0.1;
...
# V7-5-4,   引入门控机制，并且门控的特征与原始的特征进行拼接作为， 后续图网络输入的特征；

# V7-5-5,   引入门控机制，  但是只保留门控的特征， 作为后续图网络输入的特征；
# V7-5-6,   引入门控机制， 将门控特征与原始的特征，使用残差的思想进行相加， 后续图网络输入的特征；


# V7-6-1,   开始使用 区域裁剪， 然后，使用线性插值， 之后使用归一化的语谱图 作为特征；
