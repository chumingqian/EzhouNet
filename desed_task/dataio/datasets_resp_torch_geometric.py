...
class RespiraGnnSet(Dataset):
    ...
    def get(self, idx):
        c_ex = self.examples[self.examples_list[idx]]
        audio, onset_s, offset_s, padded_indx = self.read_audio(
            c_ex["mixture"], self.multisrc, self.random_channel, None, self.test
        )
        # c_ex["mixture"], 代表音频文件名称，# padded_indx: 固定到统一长度时，使用的填充标志；
        # c_ex["events"]: 代表当前音频所包含的事件个数，以及详细标注信息；
        ...
        # todo: think about 语谱图的特征增强应该放在这里还是 collate_fn中；
       ...
       # check_array(audio)
        ...
        if not len(labels_df):
            ...
            frame_labels = frame_labels.transpose(0,1)



        # ============ 节点特征生成 以及对应的节点标签生成 =============================

        #  语谱图特征生成以及帧级别的标签生成之后，使用 node_generator,  使用 动态频率特征提取器，生成每5 帧所对应的节点特征，以及节点标签；
        n_channels, n_frames, n_mels = spectrogram.shape
        spectrogram = spectrogram.unsqueeze(0)  #  先扩充出一个 batch 维度， 方便后续处理；
        node_features = []

        for i in range(0, n_frames, 5):
            if i + 5 > n_frames:  # If last chunk has less than 5 frames, take the last 5 frames
                # chunk = spectrogram[:, :, -5:, :]
                # If last chunk has less than 5 frames, discard it
                break
            else:
                chunk = spectrogram[:, :, i:i + 5, :]
            # node_feature = cnn_extractor(chunk)
            node_feature = self.node_fea_generator(chunk)
            # 先创建一个用于调试
            #node_feature = torch.rand(256)
        ...
        for i in range(0, n_frames, 5):
            if i + 5 > n_frames:  # If last chunk has less than 5 frames, take the last 5 frames
                # label_chunk = frame_labels[:, -5:]
                # If last chunk has less than 5 frames, discard it
                break
            else:
                label_chunk = frame_labels[:, i:i + 5]
        ...
        #    返回当前 单个样本构成的图数据，   以及当前样本的对应的音频文件名称。
        out_args = [graph_data,c_ex["mixture"] ]

        # if self.return_filename: #  c_ex["mixture"], 代表音频文件名称，
        #     out_args.append(c_ex["mixture"])

        # print(f"the RespiraGNNset.get() method  has  been called: "
        #       f"spectrogram shape: {out_args[0]}, "
        #       f"node labels shape: {out_args[1]}")
