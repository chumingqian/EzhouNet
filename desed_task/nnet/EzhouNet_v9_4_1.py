...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, location_dim=4, heads=2):
        super(GATLayer, self).__init__()
        self.gat = GATConv(in_channels, out_channels, heads=heads, concat=False, edge_dim=16)
        # self.location_linear = nn.Linear(location_dim, out_channels)
        self.location_linear = nn.Linear(location_dim, in_channels)
        self.gate_mlp = nn.Sequential(
            nn.Linear(in_channels, 16),
            nn.ReLU(),
            nn.Linear(16, 1),
            nn.Sigmoid()
        )  # Learn a gate for each node

    def forward(self, x, edge_index, edge_attr, location_one_hot):
        device = x.device
        # Compute gate for each node based on its features
        gate = self.gate_mlp(x)  # Shape: [num_nodes, 1]
        # Transform location one-hot into an embedding
        location_embedding = self.location_linear(location_one_hot.to(device))  # Shape: [num_nodes, out_channels]
        # Scale location embedding by the gate
        modulated_location_embedding = gate * location_embedding  # Shape: [num_nodes, out_channels]
        # Combine node features with gated location information
        combined_x = x + modulated_location_embedding
        # Pass combined features to GAT layer
        out, attn_weights = self.gat(combined_x, edge_index, edge_attr=edge_attr, return_attention_weights=True)
        return out, attn_weights



class GATModel(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, location_dim):
        super(GATModel, self).__init__()
        self.gat1 = GATLayer(in_channels, hidden_channels, location_dim)
        self.gat2 = GATLayer(hidden_channels, out_channels, location_dim)

    def forward(self, x, edge_index, edge_attr, location_one_hot):
        device = next(self.parameters()).device
        x, edge_index, edge_attr, location_one_hot = x.to(device), edge_index.to(device), edge_attr.to(device), location_one_hot.to(device)

        # First GAT layer with location-aware attention
        x, attn_weights1 = self.gat1(x, edge_index, edge_attr, location_one_hot)
        x = torch.relu(x)
        # Second GAT layer with location-aware attention
        x, attn_weights2 = self.gat2(x, edge_index, edge_attr, location_one_hot)
        return x, (attn_weights1, attn_weights2)
...
class Node_Interval_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
                 node_num_classes, location_dim,
                 num_refine_layers = 1,
                 dist_bins_list=[80, 60, 80]
                 ):

        super(Node_Interval_cls_Module, self).__init__()
        self.gat_model = GATModel(in_channels, hidden_channels, out_channels, location_dim)
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop, location_one_hot  ): # pass the batch data
        ...
        node_embeddings, attn_weights = self.gat_model(x, edge_index, edge_attr, location_one_hot) # (batch_nodes, node_embeddings )
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
        ...
        self.node_interval_pred = Node_Interval_cls_Module(node_fea_dim, hidden_channels, out_channels, node_num_classes=num_classes, location_dim = 4)
