...
# v9-7-9;  去除节点和区间中的 位置信息,   修改区间 weight param = [-1.5, 1.5]


# v10-1-3;  同时使用 中心点偏移量， 以及区间起止位置偏移， 两种的损失共同计算；
...
class GATLayer(nn.Module):
    def __init__(self, in_channels, out_channels, heads=2):
        ...
        self.gat = GATConv(in_channels, out_channels, heads=heads, concat=False, edge_dim= 32 )#16
...
class GATModel(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_conf, attn_weights1 = self.gat1(x, edge_index, edge_attr)
        x2_in = torch.relu(x_conf)
        x_cls, attn_weights2 = self.gat2(x2_in, edge_index, edge_attr)
        return x_cls,  (attn_weights1, attn_weights2)
...
class Interval_Refine(nn.Module):
    def __init__(self, node_embedding_dim, num_classes=5, num_refine_layers=1,
        ...
        # Scale-specific local feature BiGRUs
        ...
        self.unify_heads =  nn.ModuleList([
            nn.Sequential(
                ...
                # outputs: start_bins(D) + end_bins(D) + conf(1) + cls(num_classes-1) + t_c(1)
                nn.Linear(256, 2 * dist_bins_list[i] + 1 + (num_classes - 1) + 1)
            )
    ...
    def forward(
        ...
        for scale_idx in range(self.num_scales):
            ...
            shared_feature = torch.cat([
                ...
                # cur_audio_location[:num_local_node, :],
            ], dim=1)
            ...
            # conf_logits = unify_output[:, 0]
            # cls_logits = unify_output[:, 1: 1 + (self.num_classes - 1)]
            # t_c_pred = unify_output[:, -2] #  预测中心点的偏移量
            # t_w_pred = unify_output[:, -1] #  预测对宽度的缩放比率
            #
            # starts = anchor_intervals[:, 0]
            # ends = anchor_intervals[:, 1]
            ...
            # pred_bounds = torch.stack([s, e], dim=1)
            ...
            interval_cls_logits_scale = unify_output[:, 2 * self.dist_bins[scale_idx] + 1:  2*self.dist_bins[scale_idx] + 1 + (self.num_classes - 1)]
            t_c_pred = unify_output[:, -1]  # center offset (normalized by anchor width)


             # Offset branch (original  method way)
            ...
            starts_o = torch.clamp(starts + start_offsets, 0, audio_len.item())
            ends_o = torch.clamp(ends + end_offsets, 0, audio_len.item())

            starts_a = anchor_intervals[:, 0]
            ends_a = anchor_intervals[:, 1]
            a_w = (ends_a - starts_a).clamp(min=1e-6)
            a_c = 0.5 * (starts_a + ends_a)


            # Center branch
            c_pred = a_c + t_c_pred * a_w
            starts_c = (c_pred - 0.5 * a_w).clamp(min=0.0, max=float(audio_len))  # using anchor width (simple & stable)
            ends_c = (c_pred + 0.5 * a_w).clamp(min=0.0, max=float(audio_len))

            # Fuse with confidence
            alpha = torch.sigmoid(interval_conf_logits_scale) # .unsqueeze(-1)  # (N,1) in (0,1)
            s = alpha * starts_o + (1.0 - alpha) * starts_c
            e = alpha * ends_o + (1.0 - alpha) * ends_c
            fused_bounds = torch.stack([s, e], dim=1)



            # Collect results
            final_bounds_scale = fused_bounds
            ...
            t_c_pred_list.append(t_c_pred)
            t_w_pred_list.append(t_c_pred)
        ...
        t_c_preds   = torch.cat(t_c_pred_list, dim=0)
        t_w_preds  = torch.cat(t_w_pred_list, dim=0)

        a_c_bt  = torch.cat(a_c_list, dim=0)
        a_w_bt  = torch.cat(a_w_list, dim=0)

        return {
            "final_bounds": final_bounds,
            "distill_loss": 0 if distill_loss else None,
            "interval_conf_logits": interval_conf_logits,
            "interval_cls_logits": interval_cls_logits,
            "t_c_pred": t_c_preds, "t_w_pred": t_w_preds,
            "a_c": a_c_bt, "a_w": a_w_bt
        }
...
class Node_Interval_cls_Module(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
        ...
        self.edge_class_heads =  nn.Sequential(
                nn.Linear(edge_feature_dim, edge_feature_dim//2 ),  # Hidden layer
                nn.ReLU(),
                nn.Dropout(0.2),  # Regularization
                nn.Linear(edge_feature_dim//2, 1)  # Output layer
            )
    ...
    def forward(self, data, audio_dur, anchor_intervals, fs, frame_hop, bt_resp_loc  ): # pass the batch data
        ...
        # class the  edge  first, then  update the node embedding;
        edge_pred = self.edge_class_heads(edge_attr)
        ...
        # node_preds = self.node_class_heads(node_emb_w_loc) # (batch_nodes, normal confidence + 4 abnormal cls)
        ...
        for i in range(num_audios):
            ...
            event_output = self.interval_refine( # the instance of class EnhancedTimeAwareFDR
                ...
                # cur_audio_location=cur_rep_loc
            ...
            cur_t_c_pred = event_output["t_c_pred"]
            cur_w_c_pred = event_output["t_w_pred"]
            cur_a_c      = event_output["t_w_pred"]
            cur_a_w      = event_output["t_w_pred"]
            ...
            t_c_pred_lt.append(cur_t_c_pred)
            t_w_pred_lt.append(cur_w_c_pred)
            a_c_lt.append(cur_a_c)
            a_w_lt.append(cur_a_w)
...
class GraphRespiratory(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
                 ...
                 include_location=  False,  # note,  put on  the triger
                 ):
        ...
        if level == "node":
            ...
            for num_nodes in num_nodes_per_sample:
                ...
                if num_nodes > 1:
                    ...
                    edge_features = torch.empty((0, 32), dtype=torch.float32, device=device)
            ...
            pred_output = self.node_interval_pred(batch_graph, audio_durations, anchor_intervals, self.fs, self.frame_hop, bt_resp_loc=None)
            ...
            node_embeddings = pred_output[7]

            t_c_pred   = pred_output[8]
            t_w_pred   = pred_output[9]
            a_c       = pred_output[10]
            a_w      = pred_output[11]
...
if __name__ == "__main__":
    ...
    # Prepare batch data
    batch_data = {
        "spectrograms": [torch.randn(3, 1000, 84) for _ in range(2)],  # 3 channels, 1000 frames, 84 mel bands
        "frame_labels": [torch.zeros(1000, 5) for _ in range(2)],  # Dummy labels for 5 classes
        "c_ex_mixtures": ["audio1.wav", "audio2.wav"],
        "vad_timestamps": [[{"start": 0.0, "end": 5.0}, {"start": 6.0, "end": 9.0}] for _ in range(2)],
        # "genders": [0, 1],  # Integer labels for male/female
        # "chest_loc": [0, 1],  # Integer labels for 4 possible locations
        # "audio_dur": [16.0, 16.0],
        "genders": [torch.tensor(label, dtype=torch.long) for label in [0, 1]],  # 整数标签，使用 long 类型
        "chest_loc": [torch.tensor(label, dtype=torch.long) for label in [0, 1]],  # 位置标签，使用 long 类型
        "audio_dur": [torch.tensor(dur, dtype=torch.float) for dur in [16.0, 16.0]],  # 浮点数，使用 float 类型

        "anchor_intervals": [torch.zeros(70, 2) for _ in range(2)],  # 70 intervals per sample
        "assignments": [torch.zeros(70, dtype=torch.long) for _ in range(2)]  # Dummy assignments
    }

    # Forward pass
    outputs = sed_net(batch_data, level="node")
    print("Node predictions shape:", outputs["node_predictions"].shape)
    print(f"\n output.shape{outputs.keys()} ")
