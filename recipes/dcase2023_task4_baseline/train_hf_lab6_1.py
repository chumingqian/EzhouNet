...
from desed_task.dataio.datasets_resp_hf_v3_9 import RespiraGnnSet
from desed_task.nnet.EzhouNet_hf_v5_1 import GraphRespiratory
from local.respira_trainer_hf_v6_1 import RespiraSED_lab
...
# hf_lab1_6: 直接使用1层refine layer 直接学习 offset;
# 节点损失参与计算；
...
# hf_lab3_1: 节点损失参与计算；   +  time shift   数据增强；
#  且此时， self.gat_model,  interval_refine 全部实现并行化处理
#  数据集：修改先验区间的持续时间为 ， 并且保证尺度2的个数最多，
#  Scale 1: 0.5s, Scale 2: 0.8s, Scale 3: 1.5s
# 2. 网络， 对于粗粒度区间，同样分配更多的bins，  从而可以进行更大范围的偏移学习；
...
# hf_lab3_2:  调整为集成头，  weight para 分为 [-1.0, 1.0];

# hf_lab3_3:  调整为集成头，  weight para 分为 [-1.0, 1.0];
#   由于采样率较低， 需要增加节点个数， 因此 重新设置 hop = 64,
#   且 mel freq = 64, 因为此时 fmax = 2000较低,  mel 此时过度，会产生无意义的特征；
# gat  并行，  interval refine  仍然串行， 因为，此时每个样本中的帧数，仍然存在不同的情况；



# hf_lab3_:  调整为sepe头， weight para 分为 [-0.50, 0.50];
# 并且调整学习率， 使其与 epoch 保持相似的衰减率；


# hf_lab3_4: 调整基于 step 的方式， 更新学习率；


# hf_lab3_7:  sep head 保留 steth 听诊的数据， 去除 HF 多通道设备的音频数据，
# 由于 epoch 中， batch 的减少，因此需要调整对应的 step 学习率；



# hf_lab3_8:  inte head;  and  add posi feature 该数据集没有位置信息；


# hf_lab3_9:  调整为双向 Bigru,  从 gru 中获取 logit,
# 然后与独立的 weight param 相乘获得 start, end offsets;

# hf_lab3_9_3:  调整为双向 Bigru, 将 node pred 提前，
# # 并使用节点分类的结果 生成边的特征；


# hf_lab4_1:  恢复基于 epoch 的学习率调整方式；
# chunk size = 5, stride = 5


# hf_lab6_1:  恢复基于 epoch 的学习率调整方式；
# chunk size = 5, stride = 5
#  但是损失函数的设计，  此时一起考虑， 不再使用分阶段的方式
...
def single_run(
    ...
    if fast_dev_run:
        ...
        limit_train_batches = 0.006  #2 , bt =48;
        limit_val_batches = 0.02    #2 , bt=12, 2500 +158= 2658;  2658/bt = 220个批次batch, 220 *0.2 =44 个batch ; 得到 # 使用limit_val_batches = 0.2 ，则仅处理 20% 的验证批次，但每个批次将包含 12 个样本（由val_batch_size设置）。
    ...
    trainer = pl.Trainer(
        ...
        #strategy="single_device" if accelerator == "gpu" else None,  # Set strategy only for GPU
...
def prepare_run(argv=None):
    ...
    parser.add_argument(
        "--conf_file",
        default="./confs/hf_rep_lab5_1.yaml",
